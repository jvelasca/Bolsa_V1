import { ensureProjectDatabase, findDockerExe, isPostgresReady } from './lib/docker.mjs';
...
const keepFromCli = cliKeep >= 0 ? Number.parseInt(process.argv[cliKeep + 1] ?? '', 10) : undefined;
...
async function main() {
  ...
  if (docker && !isPostgresReady(docker)) {
    ...
    if (!ensure.ok) {
      ...
      writeAgentLog('db-dump', { status: 'failed', step: 'ensure', error: ensure.error });
      process.exit(1);
    }
  }

  const keep = resolveKeep({ envKeep: keepFromCli !== undefined ? String(keepFromCli) : undefined });

  try {
    const { file, bytes, gzipped } = pgDumpToFile({ docker, gzip: wantGzip });
    logInfo(
      'db-dump',
    ...
    if (pruned > 0) {
      logInfo('db-dump', `Podados ${pruned} backup(s) más antiguo(s) (keep=${keep})`);
    }
    ...
    logError('db-dump', error instanceof Error ? error.message : 'Error en el dump');
    writeAgentLog('db-dump', { status: 'failed', step: 'dump', error: String(error) });
    process.exit(1);
  }
}
