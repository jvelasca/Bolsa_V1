"""
Campaign 3 — IBEX 35 MACD family (human presets + MACD signal-cross grid).

Same methodology as SMA/RSI campaigns: list-scoped, ledger K, lab notebook afterwards.
"""
...
MACD_PRESETS = ("macd_signal_cross", "macd_zero_line")
...
async def _main() -> int:
    parser = argparse.ArgumentParser(description="IBEX35 Campaign 3 — MACD family")
    parser.add_argument("--list-id", default="ibex35")
    parser.add_argument("--presets", nargs="+", default=list(MACD_PRESETS))
    ...
    async with factory() as session:
        ...
        print(
            f"Campaign 3 MACD · {detail.name} · {len(detail.instrument_ids)} instruments · "
            f"presets={args.presets} · grid_max={args.max_grid_trials}"
        )

        if not args.grid_only:
            for idx, instrument_id in enumerate(detail.instrument_ids, start=1):
        ...
        if not args.human_only:
            print("\nMACD signal-cross grid…")
            for idx, instrument_id in enumerate(detail.instrument_ids, start=1):
                inst = await instruments.get_by_id(instrument_id)
                symbol = inst.symbol if inst else instrument_id[:8]
                try:
                    bars = await ohlcv.get_bars(
                        instrument_id, timeframe=TimeFrame.D1, limit=args.bar_limit
                    )
                    if len(bars) < 50:
                        human_skip += 1
                    ...
                    grid_trials = run_macd_signal_cross_grid(
                        inputs,
                        initial_cash=args.initial_cash,
                    ...
                    result_payload = {
                        ...
                        "trials": [
                            {
                                "fastPeriod": t.fast_period,
                                "slowPeriod": t.slow_period,
                                "signalPeriod": t.signal_period,
                    ...
                    for trial in grid_trials:
                        await trials.insert_trial(
                            ...
                            preset_key="macd_signal_cross",
                            strategy_name="macd_signal_cross",
                            params={
        ...
        print(
            f"\nCampaign 3 done: human_ok={human_ok} human_skip={human_skip} "
