"""V1.96 — Golden HTTP: T2 fill-link certification + opening veto.

  Confirm OPEN → T1 → T2 → EXIT → drain
...
async def _insert_account(
    ...
    *,
    ...
    account_id = f"lc-g196-{uuid4().hex[:12]}"
    legacy_id = f"pf-g196-{uuid4().hex[:12]}"
    inv_pf_id = f"ip-g196-{uuid4().hex[:12]}"
    ledger_id = f"le-g196-{uuid4().hex[:12]}"
    now = _now()
    ...
    async with factory() as session:
        ...
        session.add(
            LedgerEntryRow(
                ...
                description="Depósito inicial golden v196",
                executed_at=now,
                created_at=now,
            )
        )
...
async def _insert_instrument(
    ...
    *,
    ...
    now = _now()
    sym = symbol or f"G196{instrument_id[-6:].upper()}"
    async with factory() as session:
...
def auth_secret(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("APP_PASSWORD", "lifecycle-golden-v196-password")
    monkeypatch.setenv("APP_AUTH_SECRET", "lifecycle-golden-v196-secret-key-32b")
    monkeypatch.setenv("JWT_SIGNING_KEY", "lifecycle-golden-v196-secret-key-32b")
...
@pytest.mark.asyncio
async def test_v196_golden_t2_integrity_then_opening_veto(auth_secret: None) -> None:
    instrument_id = f"inst-g196-{uuid4().hex[:8]}"
    instrument_b = f"inst-g196b-{uuid4().hex[:8]}"
    ...
    async with lifespan(app):
        ...
        acc_a = await _insert_account(factory, user_id=user_a, name="Golden V196 A")
        token_a = await _jwt(factory, user_a)
        decision_id = f"dec-g196-{uuid4().hex[:10]}"
        plan = _triggered_plan(instrument_id, decision_id=decision_id)
        session_id = f"DSS-g196-{uuid4().hex[:10]}"
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            ...
            t2_data = t2_resp.json()["data"]
            t2_trade = t2_data.get("trade") or {}
            assert t2_trade.get("status") == "executed", (
                f"t2 rejected: {t2_trade.get('reason')} full={t2_data!r}"
            )
            t2_tx = t2_trade["transactionId"]
            assert isinstance(t2_tx, str) and t2_tx.strip()
            ...
            async with factory() as session:
                result = await session.execute(
                    delete(LedgerEntryRow).where(
                        ...
                        LedgerEntryRow.reference_id == t2_tx,
                        LedgerEntryRow.reference_type == "transaction",
                    )
                )
                assert result.rowcount >= 1, "expected T2 ledger row to delete"
                await session.commit()
            ...
            assert any(
                "T2_EXECUTED" in (i.get("detail") or "")
                for i in (payload2.get("fillLinkIssues") or [])
            ), payload2

            decision_b = f"dec-g196b-{uuid4().hex[:10]}"
            session_b = f"DSS-g196b-{uuid4().hex[:10]}"
