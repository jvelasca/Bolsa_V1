"""HTTP V1.46 — Paper Desk cycle + daily report (PAPER_D gate)."""
...
class PaperDeskCycleRequestDto(BaseModel):
    ...
    default_mark_price: float | None = Field(default=None, alias="defaultMarkPrice")
    mark_prices: dict[str, float] | None = Field(default=None, alias="markPrices")
...
async def paper_desk_cycle(
    ...
    """V1.46 — un ciclo EntryTick + PositionTick. dryRun default true."""
    ...
    result = await uc.execute(
        PaperDeskCycleInput(
            ...
            mark_prices=dict(body.mark_prices or {}),
            default_mark_price=body.default_mark_price,
...
async def paper_desk_daily_report(
    ...
    default_mark_price: float | None = Query(default=None, alias="defaultMarkPrice"),
) -> dict[str, Any]:
    """V1.46 — cycle dry-run + report; execute=true exige PAPER_D."""
    dry_run = not execute
    if execute and not paper_d_execute_allowed():
        raise HTTPException(
            status_code=403,
            detail={"code": "paper_auto_env_blocked", "message": "PAPER_D_EXECUTE off"},
        )
    ...
    uc = get_paper_desk_cycle_use_case(
        session, execution_policy_id=execution_policy_id, dry_run=dry_run
    )
    cycle = await uc.execute(
        PaperDeskCycleInput(
            ...
            default_mark_price=default_mark_price,
        )
    )
