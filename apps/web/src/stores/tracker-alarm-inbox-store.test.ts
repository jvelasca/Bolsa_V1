...
const sampleResult = (scanId: string): ScanRunResultDto =>
  ({
    ...
    alarmRoute: {
      policyId: 'pol-1',
      mode: 'alert',
      actions: [{ instrumentId: 'inst-1', signalKind: 'entry_long', status: 'alert_dispatched' }],
    },
  }) as ScanRunResultDto;
...
describe('tracker-alarm-inbox-store', () => {
  ...
  it('acks and scopes by account', () => {
    ...
    store.ack(id);
    expect(unreadCountForAccount(useTrackerAlarmInboxStore.getState().items, 'acc-demo')).toBe(0);
    expect(itemsForAccount(useTrackerAlarmInboxStore.getState().items, 'acc-other')).toHaveLength(0);
  });

  it('stores strategyDefinitionId from hit signal', () => {
    useTrackerAlarmInboxStore.getState().pushFromScan(sampleResult('scan-c'), 'acc-demo');
  ...
  it('rejects non-safe execution modes', () => {
    ...
    expect(useTrackerAlarmInboxStore.getState().pushFromScan(unsafe, 'acc-demo')).toBe(0);
