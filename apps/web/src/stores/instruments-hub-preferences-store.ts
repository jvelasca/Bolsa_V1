...
export const useInstrumentsHubPreferencesStore =
  create<InstrumentsHubPreferencesState>()(
    persist(
      (set) => ({
        columnLayout: DEFAULT_INSTRUMENTS_HUB_COLUMN_LAYOUT.map((c) => ({
          ...c,
        })),
        ...
        setColumnLayout: (columnLayout) =>
          set({
            columnLayout: normalizeInstrumentsHubColumnLayout(columnLayout),
          }),
        ...
        setDetailSectionsOpen: (detailSectionsOpen) =>
          set({
            detailSectionsOpen: normalizeDetailSections(detailSectionsOpen),
          }),
      ...
      }),
      {
        ...
        merge: (persisted, current) => {
          const stored = (persisted ??
            {}) as Partial<InstrumentsHubPreferencesState> & {
            /** v1 flat fields */
            listWidthPct?: number;
            stackHeightPct?: number;
            detailPanelOpen?: boolean;
          ...
          return {
            ...current,
            ...stored,
            columnLayout: normalizeInstrumentsHubColumnLayout(
              stored.columnLayout,
            ),
            favoriteColumnIds: stored.favoriteColumnIds?.length
            ...
            scopeFilter:
              ...
              stored.scopeFilter === "list"
                ...
                : current.scopeFilter,
            scopeListId:
              typeof stored.scopeListId === "string" ||
              stored.scopeListId === null
                ? stored.scopeListId
                : current.scopeListId,
            favoriteBuiltinFilters: normalizeFavoriteBuiltinFilters(
            ...
            wideSplit: normalizeWideSplit(
              stored.wideSplit ?? wideFromV1 ?? current.wideSplit,
            ),
