...
const PREFERRED_EXCHANGES = ['NMS', 'NYQ', 'NASDAQ', 'NYSE', 'NGM', 'BME', 'MCE', 'MAD'];
export function rankCatalogInstrument(
  item: { symbol: string; yahooSymbol: string; name: string; isin?: string | null },
  query: string,
): number {
  const q = query.trim().toLowerCase();
  const qIsin = normalizeIsin(query);
  let score = 0;
  if (item.isin && normalizeIsin(item.isin) === qIsin && qIsin.length >= 10) score += 200;
  else if (item.isin && normalizeIsin(item.isin).includes(qIsin) && qIsin.length >= 4) score += 120;
...
export function rankExternalSearchHit(hit: ExternalInstrumentSearchHitDto, query: string): number {
  const q = query.trim().toLowerCase();
  let score = 0;
