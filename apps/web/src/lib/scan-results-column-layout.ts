...
export function normalizeScanResultsLayout(
  ...
  options: { full: boolean; hasRating: boolean; hasBreakdown: boolean; hasDataQuality: boolean },
): ScanResultsColumnLayoutItem[] {
...
export function buildScanResultsGridTemplate(visibleColumns: ScanResultsColumnLayoutItem[]): string {
...
export function resizeScanResultsColumn(
  ...
  return layout.map((column) =>
    column.id === columnId ? { ...column, width: clampScanResultsColumnWidth(width) } : column,
  );
}
...
export function toggleScanResultsFavoriteColumn(
  ...
  return BASE_DEFAULT_LAYOUT.map((column) => column.id)
    .filter((id) => id !== 'actions' && current.has(id));
}
...
export function isNumericScanResultsColumn(columnId: ScanResultsColumnId): boolean {
  return (
    columnId === 'rating' ||
...
export function scanResultsColumnAlign(columnId: ScanResultsColumnId): 'left' | 'right' | 'center' {
...
export function qualityFromScore(score: number): {
  ...
  if (score >= 75) {
    return { label: 'Alta', className: 'bg-emerald-500/15 text-emerald-700 dark:text-emerald-400' };
  }
  if (score >= 60) {
    return { label: 'Media-alta', className: 'bg-sky-500/15 text-sky-700 dark:text-sky-300' };
  }
  if (score >= 45) {
    return { label: 'Media', className: 'bg-amber-500/15 text-amber-800 dark:text-amber-300' };
  }
  return { label: 'Baja', className: 'bg-muted text-muted-foreground' };
}

function sortableValue(hit: ScanHitDto, columnId: ScanResultsColumnId): string | number | null {
  switch (columnId) {
    case 'symbol':
      return hit.symbol;
    case 'name':
