...
const apiLists = [
  ...
  { id: CATALOG_IBEX_LIST_ID, name: 'IBEX 35', source: 'catalog', itemCount: 35 },
...
describe('reconcileCarouselListIds', () => {
  ...
  it('does not prune when api lists are not loaded yet', () => {
    ...
    expect(reconcileCarouselListIds(['stale-id'], ['IBEX 35'], [])).toEqual(['stale-id']);
  ...
  it('recovers IBEX when only stale ids remain without stored names', () => {
    ...
    expect(reconcileCarouselListIds(['old-ibex-uuid'], undefined, apiLists)).toEqual([
