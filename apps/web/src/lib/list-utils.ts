import type { InstrumentWithMetaDto, ListColumnId, ListSortState } from '@bolsa/shared';
...
export function getListCellDisplay(
  ...
  switch (column) {
    ...
    case 'lastClose':
      return {
        text: item.meta.lastClose != null ? formatPrice(item.meta.lastClose) : '—',
        className: 'text-right tabular-nums',
      };
    case 'changePct':
      return {
        text: item.meta.changePct != null ? formatPct(item.meta.changePct) : '—',
        className: `text-right tabular-nums ${
          item.meta.changePct != null && item.meta.changePct >= 0 ? 'text-success' : 'text-destructive'
        }`,
      };
    case 'isin':
      return {
    ...
    case 'syncStatus': {
      const status = item.meta.lastSync?.status ?? (item.meta.barCount > 0 ? 'success' : 'pending');
      const label =
...
function sortValue(item: InstrumentWithMetaDto, column: ListColumnId): string | number {
  switch (column) {
    case 'symbol':
