...
export function TradingDiaDReplayPanel() {
  ...
  const clearGateDecisions = useDiaDTradingSessionStore(
    (s) => s.clearGateDecisions,
  );
  ...
  const [pendingTrade, setPendingTrade] = useState<BacktestTradeDto | null>(
    null,
  );
  const [resumeNonce, setResumeNonce] = useState(0);
  const [layout, setLayout] = useState<DiaDVerifyLayoutPrefs>(() =>
    loadDiaDVerifyLayout(),
  );
  const [focusTimestamp, setFocusTimestamp] = useState<string | null>(null);
  const [iaNarrative, setIaNarrative] = useState<
    DiaDSessionEvidenceV1["paragraphs"] | null
  >(null);
  ...
  const setReportOpen = useCallback(
    (updater: boolean | ((v: boolean) => boolean)) => {
      setLayout((prev) => {
        const nextOpen =
          typeof updater === "function" ? updater(prev.reportOpen) : updater;
  ...
  const rawAutoDetail: BacktestRunDetailDto | null =
  ...
  const sessionEvidence = useMemo(() => {
    ...
    const ok = session.gateDecisions.filter(
      (d) => d.action === "accept",
    ).length;
    const ko = session.gateDecisions.filter(
      (d) => d.action === "reject",
    ).length;
  ...
  const reconciliation = useMemo(() => {
    ...
    const expSlot = getDiaDExperimentTop1(
      session.instrumentId,
      tf,
      session.diaD,
    );
    ...
    let counterfactual = null as ReturnType<
      typeof buildCounterfactualOos
    > | null;
    if (!prod1) {
      ...
      counterfactual = buildCounterfactualOos({
        ...
        productionTradeCount:
          detail?.tradeCount ?? autoDetail?.tradeCount ?? null,
        productionMaxDrawdownPct:
          detail?.maxDrawdownPct ?? autoDetail?.maxDrawdownPct ?? null,
        identity: "same_id",
      });
  ...
  const saveEvidenceMutation = useMutation({
    mutationFn: async () => {
      if (!session || !sessionEvidence)
        throw new Error("Sin Evidence de sesión");
  ...
  const importArchiveFile = useCallback(
    async (file: File | null) => {
      ...
      try {
        ...
        setArchiveImportMsg(
          err instanceof Error ? err.message : "Import falló",
        );
      } finally {
  ...
  const busy =
    ...
    (Boolean(session.autoRunId) && detailQuery.isLoading);
  const err =
  ...
  const accepted = session.gateDecisions.filter(
    (d) => d.action === "accept",
  ).length;
  const rejected = session.gateDecisions.filter(
    (d) => d.action === "reject",
  ).length;
          ...
          <div className="flex flex-wrap items-center gap-1">
            ...
            <span
              className={cn(
                ...
                sessionEvidence.band === "adverse" &&
                  "bg-destructive/15 text-destructive",
                sessionEvidence.band === "mixed" &&
                  "bg-amber-500/15 text-amber-900 dark:text-amber-100",
                sessionEvidence.band === "incomplete" &&
                  ...
                  return (
                    <li key={item.id} className="flex items-center gap-0.5">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          if (previewArchiveId === item.id)
                            setPreviewArchiveId(null);
  ...
  const reportPanelProps = {
    ...
    onResizeDragEnd: () =>
      persistLayout({ reportWidthPct: pendingReportW.current }),
  };
  ...
  return (
    <div
      ...
      data-testid={
        fullBleed ? "dia-d-replay-full-bleed" : "dia-d-replay-embedded"
      }
    >
      <div className="flex shrink-0 flex-wrap items-center gap-2 border-b border-border/50 px-3 py-1.5 text-[11px]">
        ...
        <span className="text-muted-foreground">
          ...
          {session.diaD} → {session.endDate} · {DIA_D_MODE_LABELS[session.mode]}
        </span>
        {report ? (
          <span className="tabular-nums text-muted-foreground">
            Ret. {formatPct(report.totalReturnPct)} · DD{" "}
            {formatPct(report.maxDrawdownPct)} · {report.tradeCount} ops · fin{" "}
            {formatPrice(report.finalEquity)}
        ...
        {session.mode === "manual" ? (
          <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] text-muted-foreground">
            Manual: ▶ / pasos · pausa en señales → Aceptar/Rechazar (reescribe
            equity)
          </span>
        ) : null}
        {session.mode === "semi" ? (
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        <div
          ref={movieRowRef}
          className={cn(
            "flex min-h-[180px] overflow-hidden",
        ...
        >
          <div className="min-h-0 min-w-0 flex-1 overflow-hidden p-2">
            ...
            !ohlcvQuery.isLoading ? (
              <p className="text-[11px] text-muted-foreground">
                Sin barras OHLCV para la película.
              </p>
            ) : null}
          </div>
        ...
        <PanelResizeHandle
          ...
          onDrag={(dy) => {
            ...
            if (!shell) return;
            const next = clampMovieHeightPct(
              pendingMovieH.current + pxToPct(dy, shell.clientHeight),
            );
            pendingMovieH.current = next;
            setLayout((prev) => ({ ...prev, movieHeightPct: next }));
          }}
          onDragEnd={() =>
            persistLayout({ movieHeightPct: pendingMovieH.current })
          }
        />
        ...
        <div
          ref={bottomRowRef}
          className={cn(
            "flex min-h-[160px] overflow-hidden border-t border-border/50",
            isWideBottom ? "flex-row" : "flex-col",
          )}
          style={{ flex: "1 1 auto" }}
        >
          <section
          ...
          >
            <h3 className="shrink-0 text-[11px] font-medium text-foreground">
          ...
          <PanelResizeHandle
            ...
            onDrag={(delta) => {
              ...
              const next = clampEquityWidthPct(
                pendingEquityW.current + pxToPct(delta, size),
              );
              pendingEquityW.current = next;
              setLayout((prev) => ({ ...prev, equityWidthPct: next }));
            }}
            onDragEnd={() =>
              persistLayout({ equityWidthPct: pendingEquityW.current })
            }
