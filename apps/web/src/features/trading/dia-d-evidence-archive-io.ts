...
export function formatDiaDArchiveRowLabel(
  item: DiaDEvidenceArchiveItem,
): string {
  const band =
...
export function diaDEvidenceExportFilename(
  item: DiaDEvidenceArchiveItem,
): string {
 ...
 */
export function parseDiaDEvidenceImport(
  raw: unknown,
): DiaDEvidenceImportResult {
  ...
  if (
    Array.isArray(itemRaw.narrativeParagraphs) &&
    itemRaw.narrativeParagraphs.length >= 3
  ) {
    narrativeParagraphs = [
  ...
  const item: DiaDEvidenceArchiveItem = {
    id:
      String(itemRaw.id ?? "").trim() ||
      `dde-import-${Date.now().toString(36)}`,
    instrumentId,
    symbol,
    ...
    researchEvidenceId:
      itemRaw.researchEvidenceId != null &&
      String(itemRaw.researchEvidenceId).trim()
...
export function parseDiaDEvidenceImportText(
  text: string,
): DiaDEvidenceImportResult {
  try {
