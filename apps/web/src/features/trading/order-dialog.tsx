...
export function OrderDialog() {
  ...
  const { needsFx, fxLabel, notionalAccount, isFxLoading, yahooSymbol } =
    useTradeNotional(instrumentCurrency, accountCurrency, qty, executionPrice);
  ...
  function validateOrder(): string | null {
    ...
    if (needsFx && isFxLoading)
      return "Espera el tipo de cambio para confirmar.";
      ...
      <div className="mb-4 space-y-2 rounded border border-border bg-muted/20 p-3">
        {accountName && (
          <p className="text-[10px] text-muted-foreground">
            Cuenta: <span className="text-foreground">{accountName}</span> (
            {accountCurrency})
          </p>
        )}
        <TradeFeeBreakdown
        ...
        {!confirmBeforeTrade && (
          <p className="text-[10px] text-amber-600/90">
            Confirmación desactivada — la orden se ejecutará al pulsar
            Compra/Venta.
          </p>
        )}
      </div>
