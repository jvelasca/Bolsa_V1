...
export function InstrumentAnalysisSummary({ instrumentId, symbol }: Props) {
  ...
  const topSlots =
    topQuery.data?.data?.slots?.filter((s) => Boolean(s?.strategyDefinitionId)) ?? [];
            ...
            {status === 'ready' && card ? (
              <>
                <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                  ...
                  >
                    {card.scoreDisplay100 != null ? `${card.scoreDisplay100}/100` : '—'}
                  </span>
                ...
                <KeyValueList>
                  <KeyValueRow label="PER">{fmt(card.facts?.trailingPe, 1)}</KeyValueRow>
                  <KeyValueRow label="ROE">
                    {fmt(card.facts?.roe != null ? card.facts.roe * 100 : null, 1)}%
                  </KeyValueRow>
                  ...
                  <KeyValueRow label="Beneish M">{fmt(card.derived?.beneishM)}</KeyValueRow>
          ...
          <section className="min-w-0 space-y-1.5">
            ...
            {composite ? (
              <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                ...
                <span className="text-[10px] text-muted-foreground">
                  {composite.metadata?.paperDUnlocked ? 'Paper D ok' : 'Paper D bloqueado'}
                </span>
              </div>
            ) : null}
            <KeyValueList>
              <KeyValueRow label="Pierna TA">
                {techLeg ? `${fmt(techLeg.score)} · ${techLeg.method ?? '—'}` : '—'}
              </KeyValueRow>
              <KeyValueRow label="Finalistas 1d">{topSlots.length || '—'}</KeyValueRow>
            </KeyValueList>
            {topSlots.slice(0, 3).map((slot, i) => (
              ...
              >
                #{i + 1} {slot.label ?? slot.strategyDefinitionId ?? 'estrategia'}
              </p>
            ))}
            {!composite && !compositeQuery.isLoading && status === 'ready' ? (
              <p className="text-muted-foreground">Composite no disponible aún.</p>
            ) : null}
            {faBusy ? (
              <p className="text-muted-foreground">Composite tras completar FA…</p>
            ) : null}
          </section>
        </div>
      </div>
