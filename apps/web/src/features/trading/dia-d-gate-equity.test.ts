...
describe('applyGateFills', () => {
  const trades = [
    trade({ id: 'b1', type: 'buy', timestamp: '2024-01-02', price: 100, quantity: 10 }),
    trade({ id: 's1', type: 'sell', timestamp: '2024-01-10', price: 110, quantity: 10 }),
    trade({ id: 'b2', type: 'buy', timestamp: '2024-01-15', price: 105, quantity: 10 }),
    trade({ id: 's2', type: 'sell', timestamp: '2024-01-20', price: 108, quantity: 10 }),
  ];
  ...
  it('auto: reject buy drops entry; orphan sell skipped', () => {
    const out = applyGateFills(trades, [{ tradeId: 'b1', action: 'reject' }], 'auto');
    expect(out.map((t) => t.id)).toEqual(['b2', 's2']);
  });

  it('auto: reject sell keeps position; next buy skipped; later sell closes', () => {
    const out = applyGateFills(trades, [{ tradeId: 's1', action: 'reject' }], 'auto');
...
describe('rebuildEquityCurve', () => {
  it('marks to market between fills', () => {
    ...
    const trades = [
      trade({ id: 'b1', type: 'buy', timestamp: '2024-01-02', price: 100, quantity: 10 }),
      trade({ id: 's1', type: 'sell', timestamp: '2024-01-04', price: 110, quantity: 10 }),
    ];
...
describe('computeGatedSessionMetrics', () => {
  it('reject entry leaves cash flat vs full auto', () => {
    ...
    const autoTrades = [
      trade({ id: 'b1', type: 'buy', timestamp: '2024-01-02', price: 100, quantity: 10 }),
      trade({ id: 's1', type: 'sell', timestamp: '2024-01-10', price: 120, quantity: 10 }),
    ];
