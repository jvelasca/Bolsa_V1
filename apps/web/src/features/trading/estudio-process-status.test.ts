...
const basePrefs = (
  over: Partial<EstudioSupervisionPrefs> = {},
...
describe("estudio-process-status", () => {
  it("resolveLaneState empty / ok / stale / running", () => {
    ...
    expect(resolveLaneState("2026-08-06T10:00:00Z", 1440, false, now)).toBe(
      "ok",
    );
    expect(resolveLaneState("2026-08-04T10:00:00Z", 1440, false, now)).toBe(
      "stale",
    );
    expect(resolveLaneState("2026-08-04T10:00:00Z", 1440, true, now)).toBe(
      "running",
    );
  });
