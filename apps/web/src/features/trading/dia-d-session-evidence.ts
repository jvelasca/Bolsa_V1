...
function buildClaims(
  ...
  if (input.mode !== "auto") {
    ...
    claims.push(
      `Trayectoria Auto (#1 congelada) · capital fin ${input.gated.finalEquity.toFixed(2)}`,
    );
  }
  if (band === "incomplete") {
...
export function buildDiaDSessionEvidence(
  ...
  if (
    input.gated.tradeCount === 0 &&
    input.mode === "auto" &&
    input.auto.tradeCount === 0
  ) {
