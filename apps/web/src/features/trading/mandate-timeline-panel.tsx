...
function TenureRow({
  ...
  const tradeCount = cashflow?.tradeCount ?? listTradeLinksForMandate(t.id).length;
  return (
    <li
      className={cn(
...
export function MandateTimelinePanel({
  ...
  const pnl = useMemo(() => {
    ...
    const txs = (txQuery.data?.data ?? []).filter((t) => t.instrumentId === instrumentId);
  ...
  return (
    <div className={cn('space-y-1.5 text-[11px]', className)} data-testid="mandate-timeline">
      <p className="font-semibold text-foreground">Mandato operativo</p>
      {tenures.length === 0 ? (
        <p className="text-muted-foreground">
          Sin historial. Al <strong className="text-foreground">Adoptar</strong> un Finalista
          (Checklist) se abre el mandato vigente.
        </p>
      ) : (
