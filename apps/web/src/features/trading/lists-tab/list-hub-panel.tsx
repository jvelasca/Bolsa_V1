...
const LIST_HUB_FAMILY_ORDER: ListHubFamilyId[] = [
  "sistema",
  "indices",
  "personales",
];
...
function groupListsByFamily(
  ...
  return LIST_HUB_FAMILY_ORDER.filter((id) => buckets[id].length > 0).map(
    (id) => ({
      id,
...
export function ListHubPanel() {
  ...
  const [newInstrumentIds, setNewInstrumentIds] = useState<Set<string>>(
    new Set(),
  );
  ...
  const pendingBuyCount = pendingOrders.filter(
  ...
  const chartListMembership = useWorkspaceStore(
    (state) => state.chartListMembership,
  );
  ...
  async function exportList(list: InstrumentListSummaryDto) {
    const columns = visibleListColumns(
      resolveListColumnLayout(listConfig, list.id),
    );
    ...
    if (list.id === VIRTUAL_LIST_VISUALIZATION) {
      ...
      const openEntries = openTabs.flatMap((tab) => {
        ...
        seen.add(id);
        const fromStore = visualizationEntries.find(
          (e) => e.instrumentId === id,
        );
        return [
          fromStore ?? {
      ...
      if (openEntries.length === 0) {
        window.alert(
          "No hay valores en Visualizados (ninguna pestaña abierta).",
        );
        return;
      }
      exportInstrumentsCsv(
        openEntries.map((entry) =>
          visualizationEntryToListItem(entry, catalog),
        ),
        columns,
        list.name,
      );
    ...
    try {
      ...
      window.alert(
        err instanceof ApiError ? err.message : "No se pudo exportar",
      );
    }
  }
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err) => {
      setError(
        err instanceof ApiError ? err.message : "No se pudo eliminar la lista",
      );
    },
  });
  ...
  const createMutation = useMutation({
    ...
    onError: (err) => {
      setError(
        err instanceof ApiError ? err.message : "No se pudo crear la lista",
      );
    },
  });
  ...
  const freezeCopyMutation = useMutation({
    ...
    onError: (err) => {
      setError(
        err instanceof ApiError ? err.message : "No se pudo congelar la copia",
      );
    },
  });
  ...
  const sortedLists = sortListSummaries(
    allLists,
    resolveListHubSort(listConfig),
    metaById,
  );
  ...
  return (
    <ListHubColumnLayoutProvider>
      <div className="flex h-full min-h-0 flex-col">
        ...
        {error && (
          <p className="shrink-0 px-2 py-1 text-xs text-destructive">{error}</p>
        )}
      </div>
    </ListHubColumnLayoutProvider>
  );
