...
describe('fetchIoByInstrumentIds', () => {
  it('uses seed cache and skips network for those ids', async () => {
    const queryFundamentals = vi.fn(async () => ({ data: [] as FundamentalChipDto[] }));
    const queryComposite = vi.fn(async () => ({ data: [] as CompositeChipDto[] }));
    ...
    const io = await fetchIoByInstrumentIds(
      ['a', 'b'],
      { queryFundamentals, queryComposite, yieldBetweenChunks: async () => undefined },
      { fa, ta },
    );
  ...
  it('fetches composite in small sequential chunks', async () => {
    ...
    const ids = Array.from({ length: IO_SORT_COMPOSITE_CHUNK * 2 + 1 }, (_, i) => `id-${i}`);
  ...
  it('applies FA distress floor to IO', async () => {
    const io = await fetchIoByInstrumentIds(
      ...
      {
        fa: new Map([['z', { scoreDisplay100: 20, isStale: false, distress: true }]]),
        ta: new Map([['z', { technicalDisplay100: 90, compositeDisplay100: 90 }]]),
      },
    );
