...
function resolveVisualState(item: InstrumentWithMetaDto): SyncVisualState {
  ...
  if (freshness === "current" && item.meta.lastSync?.status === "partial")
    return "partial";
  return freshness;
}
 ...
 */
export function ListSyncStatusCell({
  item,
  className,
}: ListSyncStatusCellProps) {
    ...
    return (
      ...
      >
        <RefreshCw
          ...
          strokeWidth={2.5}
          aria-label={
            state === "partial" ? "Sincronización parcial" : "Datos desfasados"
          }
        />
      </span>
    );
