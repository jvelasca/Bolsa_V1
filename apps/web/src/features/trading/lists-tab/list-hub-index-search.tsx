...
type ProgressStep = { id: string; label: string; detail?: string; status: StepStatus };
...
export function ListHubIndexSearch({ apiLists, onError }: Props) {
  ...
  function initSteps(displayName: string) {
    setExtraNotes([]);
    setSteps([
      { id: 'resolve', label: `Resolver constitutivos de ${displayName}`, status: 'active' },
      { id: 'count', label: 'Contar miembros del índice', status: 'pending' },
      { id: 'hydrate', label: 'Comprobar BD e importar faltantes', status: 'pending' },
      { id: 'result', label: 'Materializar lista (altas / bajas)', status: 'pending' },
    ]);
  }
  ...
  function finishFromSyncResult(data: SubscribeMarketIndexResultDto) {
    ...
    const isPartial = data.status !== 'ready' || missingAfter > 0 || progress.failed.length > 0;
    patchStep('hydrate', {
    ...
    patchStep('result', {
      ...
      detail: [
        ...
        leaveN ? `−${leaveN} bajas (salen del índice; NO se borran de BD)` : null,
    ...
    const notes: string[] = [];
    if (missingAfter > 0) notes.push(`Falta vincular ${missingAfter} constitutivos.`);
    ...
    if (!isPartial) notes.push('Listo: puedes usar la lista en el carrusel / watchlist.');
    setExtraNotes(notes);
  ...
  async function pollJob(jobId: string): Promise<IndexSubscribeJobDto> {
    for (;;) {
      ...
      const current = typeof job.result?.currentSymbol === 'string' ? job.result.currentSymbol : null;
      if (job.status === 'processing' || job.status === 'pending') {
        patchStep('hydrate', {
          ...
          detail: [
            already || imported ? `${already} en BD · +${imported} importados` : null,
    ...
    try {
      ...
      if (useAsync) {
        ...
        if (job.status === 'failed') {
          throw new ApiError(job.error || 'Falló la suscripción del índice', 500);
        }
        const instrumentIds = (job.result?.instrumentIds as string[] | undefined) ?? [];
      ...
      setExtraNotes([]);
      onError(err instanceof ApiError ? err.message : 'No se pudo suscribir el índice');
    } finally {
      setBusyKey(null);
    }
  }
  ...
  return (
    <section className="shrink-0 space-y-1.5 border-b border-border bg-muted/10 p-2">
      <p className="text-[11px] font-medium text-foreground">Índices mundiales</p>
      <p className="text-[10px] leading-snug text-muted-foreground">
        Catálogo estándar + búsqueda. Suscribir usa la misma tubería para todos (IBEX, S&P,
        DAX…): constitutivos → import faltantes → lista vinculada.
      </p>
      {catalog.length > 0 ? (
        <ul className="flex flex-wrap gap-1">
          {catalog.map((entry) => {
            const already = apiLists.some((l) => l.id === entry.listId && l.source === 'catalog');
            ...
            return (
              <li key={entry.code}>
                <Button
                  type="button"
                  size="sm"
                  variant={entry.constituentReady ? (already ? 'outline' : 'default') : 'ghost'}
                ...
                >
                  {isBusy ? '…' : already ? `${entry.displayName} · Sync` : entry.displayName}
                </Button>
              </li>
            );
          })}
        ...
        <ul className="max-h-36 space-y-1 overflow-auto rounded-md border border-border/70 bg-background p-1">
          {hits.map((hit) => {
            ...
            const already = Boolean(
              listId && apiLists.some((l) => l.id === listId && l.source === 'catalog'),
            );
            ...
            return (
              ...
              >
                <div className="min-w-0">
                  <p className="truncate font-medium text-foreground">{hit.displayName}</p>
                  <p className="truncate text-[10px] text-muted-foreground">
                    {hit.yahooSymbol}
                    {hit.region ? ` · ${hit.region}` : ''}
                    {hit.constituentReady ? ' · Suscribir activo' : ' · constitutivos pronto'}
                  </p>
                </div>
                <Button
                  type="button"
            ...
            {steps.map((step) => (
              <li key={step.id} className="flex gap-1.5">
                ...
                <span>
                  ...
                  {step.detail ? (
                    <span className="mt-0.5 block opacity-80">{step.detail}</span>
                  ) : null}
                </span>
              </li>
            ))}
