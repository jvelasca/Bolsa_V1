...
describe("orderInstrumentIdsByIo", () => {
  it("puts higher IO to the left (first)", () => {
    ...
    expect(
      orderInstrumentIdsByIo(["a", "b", "c"], io, { a: "A", b: "B", c: "C" }),
    ).toEqual(["b", "c", "a"]);
  });
  ...
  it("ranks missing IO last", () => {
    ...
    expect(orderInstrumentIdsByIo(["x", "y"], io, { x: "X", y: "Y" })).toEqual([
      "y",
      "x",
    ]);
  });
...
describe("compareIoQuality", () => {
  it("sorts descending by io", () => {
    ...
    expect([...rows].sort(compareIoQuality).map((r) => r.instrumentId)).toEqual(
      ["b", "a"],
    );
  });
});
