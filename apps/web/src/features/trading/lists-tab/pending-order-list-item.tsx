...
export function PendingOrderListItem({ order, isChartActive, onOpenChart }: PendingOrderListItemProps) {
  ...
  return (
    ...
    >
      <button type="button" className="min-w-0 flex-1 text-left" onClick={onOpenChart}>
        <div className="flex items-baseline justify-between gap-1">
          <div className="min-w-0 truncate">
            <span className="text-xs font-semibold">{order.symbol}</span>
            <span className="ml-1 text-[10px] text-muted-foreground">
              {sideLabel} limitada · {order.quantity} @ {formatPrice(order.limitPrice)}
            </span>
          </div>
