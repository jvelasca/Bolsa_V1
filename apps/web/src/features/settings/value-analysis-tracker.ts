...
export const VALUE_ANALYSIS_YOU_ARE_HERE = {
  ...
  body:
    'FIE cerrado en código: valoración F0–F2.8 + Composite v1.1 + Screener FA + Paper D + CAPM visibles. Fase actual: smoke UI / checklist APP.',
  steps: [
...
export const VALUE_ANALYSIS_FA_INVENTORY = [
  { id: 'f1', label: 'Tarjeta Valor + chip', detail: 'fund_card_v1 · scoreDisplay100 · confidence' },
  { id: 'f1b', label: 'Copiloto', detail: 'Ollama/heurística; solo facts precalculados' },
  { id: 'f20', label: 'Gate rico', detail: 'PE, ROE, D/E, Altman, FCF, márgenes, growth…' },
  { id: 'f21', label: 'Piotroski', detail: 'piotroski_f_annual_v1 · null si incompleto' },
  { id: 'f22', label: 'Sector bands', detail: 'fund_sector_bands_v1' },
  { id: 'f23', label: 'Graham + DCF', detail: 'graham_number_v1 · dcf_fcf_2stage_wacc_v1' },
  { id: 'f24', label: 'WACC sector', detail: 'fund_wacc_sector_v1 (fallback sin beta)' },
  { id: 'f25', label: 'DCF escenarios', detail: 'dcf_scenarios_v1 bear/base/bull' },
  { id: 'f26', label: 'CAPM + ADV', detail: 'fund_capm_v1 · capmRf/capmErp en Tarjeta · adv_usd_v1 → Composite' },
  { id: 'f27', label: 'ROIC', detail: 'roic_nopat_ic_v1 · gate minRoic' },
  { id: 'f28', label: 'Beneish M', detail: 'beneish_m_annual_v1 · M>−1.78 → fund.solvency=distress · Score_FUND' },
  { id: 'f2b', label: 'Filings + SEC + RAG', detail: 'Disco · EDGAR · TF-IDF ask (≠ Score_FUND)' },
  { id: 'f3', label: 'Composite', detail: 'composite_score_v1_1 · etiquetas liquidez UI · paperDUnlocked' },
  { id: 'f4', label: 'Screener FA', detail: 'fund_screener_v1 · whitelist snapshot' },
  { id: 'pd', label: 'Paper D', detail: 'paper_d_propose_v2 · PAPER_D_EXECUTE' },
  { id: 'cron', label: 'Cron FA→D', detail: 'fa_weekly_pipeline_v1 · FA_WEEKLY_CRON_ENABLED' },
] as const;
...
export const VALUE_ANALYSIS_PRINCIPLE = {
  ...
  body:
    'Los hechos (PE, RSI, VIX, noticias) no son órdenes. Los Assessments son lecturas tipadas (score/facts). Solo el DecisionRuntime fusiona con WeightRules y propone. El Gate permite o veta. El LLM no pondera ni compra ni calcula ratios.',
} as const;
...
export const VALUE_ANALYSIS_DB = [
  { what: 'Hechos Yahoo / derived FA', where: 'instruments.profile_snapshot.fundamentals' },
  { what: 'Filings (fuera de Score_FUND)', where: 'data/filings/{instrumentId}/' },
  { what: 'Whitelist FA', where: 'instrument_lists kind=snapshot' },
  { what: 'DecisionSession (razonamiento)', where: 'decision_sessions.payload' },
  ...
  { what: 'Prediction (en Session)', where: 'decision_sessions.payload.predictions' },
] as const;
