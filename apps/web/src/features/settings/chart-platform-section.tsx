...
export function ChartPlatformSection({ compact = false }: { compact?: boolean }) {
  const body = (
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Resumen de estado (Ayuda). No es un backlog editable. Sync {CHART_TRACKER_SYNC.asOf}.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-4 text-sm md:grid-cols-2">
          <div className="rounded-md border border-border bg-muted/20 p-3">
            ...
            <ul className="mt-2 list-inside list-disc space-y-1 text-muted-foreground">
              <li>
                <span className="text-foreground">General</span> — cuenta activa, workspace, plantilla
                de gráficos nuevos
              </li>
              <li>
                <span className="text-foreground">Otras</span> — sync automática Yahoo y estado de
                proveedores
              </li>
              <li>Comisiones, confirmaciones; notificaciones / sonidos / atajos (pendientes)</li>
            </ul>
          </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Datos en <code className="text-xs">chart-platform-tracker.ts</code> · alineado con{' '}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Diseño de referencia — no usar fases antiguas del ADR como backlog</CardDescription>
        </CardHeader>
        <CardContent className="text-sm text-muted-foreground">
          <p>
            Diseño en <code className="text-xs">{CHART_TRACKER_SYNC.adr}</code>. Detalle de barras:{' '}
            <code className="text-xs">{CHART_TRACKER_SYNC.chartDataBar}</code>. Este resumen en Ayuda
            es la fuente de verdad del estado actual en la app.
          </p>
        </CardContent>
      </Card>
    </>
