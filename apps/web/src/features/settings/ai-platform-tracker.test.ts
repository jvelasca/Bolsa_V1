...
describe("ai-platform-tracker · Mapa IA", () => {
  ...
  it("marks goals done vs partial; NEXT and FROZEN lists", () => {
    ...
    expect(AI_PRODUCT_GOALS.find((g) => g.id === "goal-coach-at")?.status).toBe(
      "done",
    );
    expect(AI_PRODUCT_GOALS.find((g) => g.id === "goal-lab-at")?.status).toBe(
      "done",
    );
    expect(AI_PRODUCT_GOALS.find((g) => g.id === "goal-core-r")?.status).toBe(
      "done",
    );
    ...
    expect(AI_PRODUCT_FROZEN.join(" ")).toMatch(
      /Belief|P3–P9|PAPER_D|multi-dispositivo/i,
    );
