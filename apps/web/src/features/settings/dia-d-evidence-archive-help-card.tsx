  ...
  return (
    <Card className="border-border/70">
      <CardHeader className="pb-2">
        ...
        <CardDescription>
          localStorage · últimos {HELP_ARCHIVE_MAX} · export JSON · sandbox ≠ DEMO
        </CardDescription>
      </CardHeader>
            ...
            {preview ? (
              <div className="space-y-0.5 rounded border border-border/50 bg-muted/20 px-2 py-1.5 text-[11px] leading-snug text-muted-foreground">
                <p className="font-medium text-foreground">
                  {preview.symbol} · {DIA_D_EVIDENCE_BAND_LABELS[preview.evidence.band]}
                  {preview.researchEvidenceId
                    ? ` · Fase 2 ${preview.researchEvidenceId.slice(0, 8)}…`
                    : ' · solo local'}
                </p>
                {(preview.narrativeParagraphs ?? preview.evidence.paragraphs).map(
                  (p, idx) => (
