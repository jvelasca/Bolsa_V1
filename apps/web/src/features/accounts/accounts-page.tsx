        ...
        <div className="flex shrink-0 flex-col items-end gap-0.5">
          {isWorking && account.status === "active" && (
            <span
              ...
              title="La app opera con esta cuenta"
            >
              Activa
            </span>
          )}
...
export function AccountsPage() {
  ...
  const filteredAccounts = useMemo(() => {
    ...
    if (hubFilter === "demo")
      return accounts.filter((a) => a.type === "simulated");
    return accounts;
  }, [accounts, hubFilter]);
  ...
  const demoActiveCount = useMemo(
    () =>
      accounts.filter((a) => a.type === "simulated" && a.status === "active")
        .length,
    [accounts],
  );
  const paperActiveCount = useMemo(
    () =>
      accounts.filter((a) => a.type === "paper" && a.status === "active")
        .length,
    [accounts],
  );
          ...
          {closedAccounts.length > 0 && (
            <div className="pt-1">
              ...
              >
                {showClosed
                  ? "Ocultar cerradas"
                  : `Mostrar cerradas (${closedAccounts.length})`}
              </button>
              {showClosed && (
