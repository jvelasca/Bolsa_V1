...
import type { ChartTimeframe, DataFreshnessStatus, InstrumentDataStatusDto } from '@bolsa/shared';
...
export function ChartDataStatusBadge({
  ...
  const key: DataFreshnessStatus = syncing ? 'syncing' : (status?.freshnessStatus ?? 'empty');
  ...
  if (syncing) {
    return (
      ...
      >
        <span className="hidden sm:inline">Actualizando y guardando en BD…</span>
  ...
  if (flash && status && status.barCount > 0) {
    return (
      ...
      >
        <span className="hidden font-medium sm:inline">Actualizados y guardados en BD</span>
