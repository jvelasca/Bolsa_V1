...
export function pitchforkRays(
  ...
  return [median, upper, lower].filter(
    (seg): seg is NonNullable<typeof seg> => seg != null,
  );
}

/** Ratios Gann clásicos (precio × tiempo) respecto a la línea 1×1. */
export const GANN_FAN_RATIOS: ReadonlyArray<
  readonly [priceMult: number, timeMult: number]
> = [
  [1, 8],
  [1, 4],
  [1, 3],
...
export function hitTestGannFan(
  ...
  return gannFanRays(drawing, toPixel).some(
    (seg) =>
...
export function drawingVertices(
  drawing: ChartDrawing,
): Array<"p1" | "p2" | "p3"> {
  if (drawing.type === "channel" || drawing.type === "pitchfork")
    return ["p1", "p2", "p3"];
  if (
    drawing.type === "hline" ||
    drawing.type === "hray" ||
...
export function markerDirectionFromDelta(
  dx: number,
  dy: number,
): ChartMarkerDirection {
...
export function hitTestFibTimeZone(
  ...
  return fibTimeZoneLineXs(drawing.p1, drawing.p2, toPixel).some(
    (x) => Math.abs(px - x) <= threshold,
  );
}
...
export function hitTestGannGrid(
  ...
  if (hitTestRectBounds(left, top, right, bottom, px, py, threshold))
    return true;
...
export function hitTestPitchfork(
  ...
  return pitchforkRays(drawing, chart, series, toPixel).some(
    (seg) =>
      distanceToSegment(px, py, seg.x1, seg.y1, seg.x2, seg.y2) <= threshold,
  );
}
...
export function hitTestBrushStroke(
  ...
  for (let i = 1; i < drawing.points.length; i += 1) {
    ...
    if (!a || !b) continue;
    if (distanceToSegment(px, py, a.x, a.y, b.x, b.y) <= hitThreshold)
      return true;
  }
  return false;
}
...
export function hitTestDrawing(
  ...
  if (drawing.type === "text-label") {
    const content = drawing.label?.trim() || drawing.text?.trim() || "Texto";
...
export function findDrawingVertexHit(
  ...
  if (options.priorityDrawingId) {
    const preferred = drawings.find(
      (drawing) => drawing.id === options.priorityDrawingId,
    );
    if (preferred) {
