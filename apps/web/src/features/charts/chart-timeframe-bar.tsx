...
import { CHART_BAR_ZONE_ROW_CLASS, CHART_BAR_ZONE_SCROLL_ROW_CLASS } from '@/features/charts/chart-bar-zone-styles';
...
) as Record<ChartTimeframe, { id: ChartTimeframe; label: string; hint: string }>;

function ChartTimeframeZoomControls({
  chartSyncId,
}: {
  chartSyncId?: string;
}) {
...
export function ChartTimeframeBar({
  ...
  const { favorites, toggleFavorite, isFavorite } = useChartTimeframeFavorites();

  const zoomControls = showZoom ? <ChartTimeframeZoomControls chartSyncId={chartSyncId} /> : null;

  if (!showTimeframe) {
    return zoomControls ? (
      <div className={cn(CHART_BAR_ZONE_ROW_CLASS, className)} title="Zoom del gráfico">
