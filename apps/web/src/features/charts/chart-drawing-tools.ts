...
export function findDrawingToolDefinition(
  id: string,
...
export function activeToolInGroup(
  ...
  return DRAWING_TOOL_CATALOG.find(
    (item) => item.id === tool && item.group === group,
  );
}

export function toolBelongsToGroup(
  tool: ChartDrawTool | string,
  group: DrawingToolGroupId,
): boolean {
  return DRAWING_TOOL_CATALOG.some(
    (item) => item.id === tool && item.group === group,
  );
}

export function availableToolsInGroup(
  group: DrawingToolGroupId,
): DrawingToolDefinition[] {
  return DRAWING_TOOL_CATALOG.filter(
    (item) =>
      item.group === group && item.available && isActiveDrawTool(item.id),
  );
}

export function groupForDrawTool(
  tool: ChartDrawTool | string,
): DrawingToolGroupId | null {
  return DRAWING_TOOL_CATALOG.find((item) => item.id === tool)?.group ?? null;
}

/** Grupos de la barra sin Cursor (Cursor va fijado arriba). */
export const DRAWING_TOOL_GROUPS_WITHOUT_CURSOR = DRAWING_TOOL_GROUPS.filter(
  (g) => g.id !== "cursor",
);
...
export function resolveGroupRailIconTool(
  ...
  return (
    (availableToolsInGroup(groupId)[0]?.id as ChartDrawTool | undefined) ?? null
  );
}
...
export function extraFavoriteToolsForRail(
  favorites: ChartDrawTool[],
): ChartDrawTool[] {
...
export function drawingRailFamilyBlocks(
  favorites: ChartDrawTool[],
): DrawingRailFamilyBlock[] {
