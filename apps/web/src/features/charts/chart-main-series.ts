...
export function resolveMainSeriesEngine(seriesType: ChartSeriesType): MainSeriesEngineKind {
  ...
  switch (kind) {
    ...
    case 'line-markers':
      return chart.addSeries(
        LineSeries,
        lineOptions(colors, { pointMarkersVisible: true, pointMarkersRadius: 3 }),
      );
    case 'line-step':
      return chart.addSeries(
        LineSeries,
...
export function setMainPriceSeriesData(
  ...
  switch (kind) {
    ...
    case 'area':
      ...
      return;
    case 'area-hlc':
      (series as ISeriesApi<'Area'>).setData(barsToTypicalPriceLineSeries(bars));
      return;
    case 'baseline': {
      const baseline = series as ISeriesApi<'Baseline'>;
      const basePrice = bars[Math.floor(bars.length / 2)]?.close ?? bars[0]!.close;
    ...
    case 'hollow-candlestick':
      ...
      return;
    case 'heikin-ashi':
      (series as ISeriesApi<'Candlestick'>).setData(barsToHeikinAshiSeries(bars));
      return;
    case 'volume-candlestick':
...
export function applyMainPriceSeriesColors(
  ...
  switch (kind) {
    ...
    case 'high-low-bar':
      ...
      return;
    case 'hollow-candlestick':
      (series as ISeriesApi<'Candlestick'>).applyOptions(hollowCandleOptions(colors));
      return;
    case 'heikin-ashi':
      (series as ISeriesApi<'Candlestick'>).applyOptions(solidCandleOptions(colors));
      return;
    case 'volume-candlestick':
    case 'renko':
    case 'line-break':
