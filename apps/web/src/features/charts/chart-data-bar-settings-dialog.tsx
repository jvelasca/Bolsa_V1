...
import type { ChartToolbarChartOverrides, ChartToolbarGlobalConfig } from '@bolsa/shared';
import {
...
export function ChartDataBarSettingsDialog() {
  ...
  const updateChartToolbar = useWorkspaceStore((s) => s.updateChartToolbarForChart);
  const resetChartToolbar = useWorkspaceStore((s) => s.resetChartToolbarForChart);
  ...
  return (
    <Dialog
      ...
      className="max-w-lg"
    >
      <DataBarWorkspaceDefaultsSection draft={globalDraft} onChange={setGlobalDraft} />
        ...
        <section className="mb-4 mt-6 space-y-3 border-t border-border pt-4">
          <div>
            ...
            <p className="mt-1 text-[11px] text-muted-foreground">
              Personaliza solo la pestaña activa. Si usas los defaults del workspace, hereda la
              sección anterior.
            </p>
          </div>
          ...
          <label className="flex items-center gap-2 text-xs font-medium">
            ...
            />
            Usar valores por defecto del workspace (sin personalizar este gráfico)
          </label>
            ...
            <>
              <section className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  ...
                  >)
                    .filter((key) => key !== 'settingsButton' && key !== 'overlayIndicators')
                    .map((key) => (
                      <label key={key} className="flex items-center gap-2 text-xs">
                        <input
                          type="checkbox"
                          className={checkboxClassName}
                          checked={chartDraft.visibility?.[key] ?? resolvedPreview.visibility[key]}
                          onChange={() =>
                            setChartDraft((prev) => ({
                              ...prev,
                              visibility: {
                                ...
                                ...prev.visibility,
                                [key]: !(prev.visibility?.[key] ?? resolvedPreview.visibility[key]),
                              },
                            }))
                          }
                        />
              ...
              <section className="space-y-2">
                <p className="text-xs font-medium text-foreground">Distribución</p>
                <label className="flex items-center gap-2 text-xs">
                  <input
                    type="checkbox"
                    className={checkboxClassName}
                    checked={chartDraft.layout?.wrapRows ?? resolvedPreview.layout.wrapRows}
                    onChange={() =>
                      setChartDraft((prev) => ({
                        ...prev,
                        layout: {
                          ...
                          ...prev.layout,
                          wrapRows: !(prev.layout?.wrapRows ?? resolvedPreview.layout.wrapRows),
                        },
                      }))
                    }
                  />
      ...
      <section className="mt-6 border-t border-border pt-4">
        <FavoritesWorkspaceSection draft={globalDraft} onChange={setGlobalDraft} />
      </section>

      <div className="mt-4 flex flex-wrap justify-end gap-2">
        <Button type="button" variant="outline" onClick={handleResetWorkspaceDefaults}>
          Restaurar defaults del workspace
        </Button>
        {activeTab && !chartDraft.useGlobalDefaults && (
          <Button type="button" variant="outline" onClick={handleResetChartOverrides}>
