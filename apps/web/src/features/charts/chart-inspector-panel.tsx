...
import { requestChartReflow, formatPct, formatPrice } from '@/features/charts/chart-utils';
import {
  DATA_STATUS_COLORS,
  DATA_STATUS_LABELS,
...
const CONFIG_SECTIONS: {
  ...
  { id: 'series', shortcutId: 'series', label: 'Estilo', icon: ChartCandlestick },
  { id: 'objects', shortcutId: 'objects', label: 'Objetos', icon: Shapes },
  { id: 'styles', shortcutId: 'styles', label: 'Canvas', icon: Palette },
  { id: 'context', shortcutId: 'context', label: 'Selección', icon: MousePointer2 },
];
            ...
            <div key={id} className="relative shrink-0">
              ...
              >
                ...
                {id === 'layers' && layersCount > 0 && (
                  <span className="tabular-nums text-[8px] opacity-70">{layersCount}</span>
                )}
                {id === 'objects' && objectsCount > 0 && (
                  <span className="tabular-nums text-[8px] opacity-70">{objectsCount}</span>
                )}
              </button>
              <button
                type="button"
...
export function ChartInspectorPanel({
  ...
  const [configSection, setConfigSection] = useState<ChartInspectorConfigSection>('layers');
  ...
  const setSelectedIndicatorId = useUiStore((s) => s.setSelectedIndicatorInstanceId);
  ...
  const setIndicatorInstanceParameters = useWorkspaceStore((s) => s.setIndicatorInstanceParameters);
  ...
  const updateChartSeriesTypeParams = useWorkspaceStore((s) => s.updateChartSeriesTypeParams);
  const { isFavorite: isBarShortcutFavorite, toggleFavorite: toggleBarShortcutFavorite } =
  ...
  }, [selectedIndicatorId, selectedDrawingId, selectedIndicator, selectedDrawing]);
      ...
      <div className="min-h-0 flex-1 overflow-auto p-2">
          ...
          <div className="space-y-4">
            ...
            <section id="inspector-data-chart">
              ...
              <dl>
                ...
                <DetailRow label="Velas cargadas" value={barsCount.toLocaleString('es-ES')} />
                <DetailRow
                  label="Plantilla ind."
                ...
                />
                <DetailRow label="Objetos gráficos" value={String(drawings.length)} />
                <DetailRow
                  label="Cursor"
                  value={chartConfig.cursor.mode === 'magnet' ? 'Imán' : 'Libre'}
                />
                <DetailRow
                  label="Rejilla"
                  value={
                    chartConfig.grid.showHorizontal || chartConfig.grid.showVertical
                      ? 'Visible'
                      : 'Oculta'
                  }
                />
            ...
            <section id="inspector-data-candle">
              ...
              {activeBar ? (
                <dl>
                  ...
                  <DetailRow label="Apertura" value={formatPrice(activeBar.open)} />
                  <DetailRow label="Máximo" value={formatPrice(activeBar.high)} />
                  <DetailRow label="Mínimo" value={formatPrice(activeBar.low)} />
                  <DetailRow label="Cierre" value={formatPrice(activeBar.close)} />
                  <DetailRow
                    label="Rango H-L"
                  ...
                  <DetailRow
                    ...
                    value={
                      activeBar.open
                        ? formatPct(((activeBar.close - activeBar.open) / activeBar.open) * 100)
                        : '—'
                    }
                  />
                  <DetailRow
            ...
            <section id="inspector-data-alerts">
              ...
              <p className="text-xs text-muted-foreground">
                Las alertas de precio e indicador se configurarán aquí (próximamente).
              </p>
            </section>
          </div>
        )}
            ...
            {selectedIndicator && selectedDefinition ? (
              <div className="space-y-3">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{instanceLabel(selectedIndicator)}</p>
                  <p className="mt-1 text-[10px] text-muted-foreground">
                    {selectedDefinition.panel === 'sub' ? 'Panel inferior' : 'Sobre el precio'}
                  </p>
                </div>
                ...
                {selectedDefinition.parameters.length > 0 && (
                  <IndicatorParametersForm
                    ...
                    onChange={(next) => {
                      setIndicatorInstanceParameters(selectedIndicator.instanceId, next, chartId);
                      requestChartReflow();
                    }}
                  />
                )}
                <button
                  ...
                  onClick={() => {
                    removeIndicatorInstance(selectedIndicator.instanceId, chartId);
              ...
              <div className="space-y-3">
                <div>
                  <p className="font-medium">{drawingTypeTitle(selectedDrawing)}</p>
                  {selectedDrawing.text && (
                    <p className="text-xs text-muted-foreground">{selectedDrawing.text}</p>
                  )}
                </div>
                <ChartDrawingPropertiesPanel
                  mode="instance"
                  ...
                  templates={templates}
                  compact
                  onUpdateDrawing={(patch) => updateDrawing(selectedDrawing.id, patch, chartId)}
                  onApplyTemplate={(templateId) =>
                    applyDrawingTemplate(selectedDrawing.id, templateId, chartId)
                  }
                />
                <button
                  type="button"
              ...
              <p className="text-xs text-muted-foreground">
                Selecciona un indicador en el gráfico o en la pestaña Capas, o un objeto gráfico con
                el puntero.
              </p>
            )}
          </div>
        ...
        {mode === 'config' && configSection === 'layers' && (
          <div className="space-y-3" id="inspector-config-layers">
            <section className="space-y-1.5">
              ...
              {onOpenSync && (
                <ActionButton onClick={onOpenSync}>Sincronizar histórico 1D</ActionButton>
              )}
            </section>
            {indicatorInstances.length === 0 ? (
              <p className="text-xs text-muted-foreground">Sin indicadores en este gráfico.</p>
            ) : (
              <>
                <div id="inspector-layers-overlay">
                  ...
                  {overlayLayers.length === 0 ? (
                    ...
                    <ul className="space-y-0.5">{overlayLayers.map(renderLayerRow)}</ul>
                  )}
                </div>
                <div id="inspector-layers-sub">
                  ...
                  {subLayers.length === 0 ? (
                    <p className="text-xs text-muted-foreground">Sin paneles inferiores.</p>
                  ) : (
                    <ul className="space-y-0.5">{subLayers.map(renderLayerRow)}</ul>
                  )}
                </div>
              </>
            )}
        ...
        {mode === 'config' && configSection === 'objects' && (
          <div id="inspector-config-objects">
            {drawings.length === 0 ? (
              <p className="text-xs text-muted-foreground">Sin objetos gráficos.</p>
            ) : (
                  ...
                  <li key={drawing.id}>
                    <button
                      ...
                      className={cn(
                        ...
                        selectedDrawingId === drawing.id && 'bg-accent text-primary',
        ...
        {mode === 'config' && configSection === 'series' && (
          <ChartSeriesStylePanel
            ...
            onParamsChange={(patch) => updateChartSeriesTypeParams(patch, chartId)}
          />
        )}
