...
export function AiIndicatorsPanel({
  ...
  const createAiVariant = useWorkspaceStore(
    (state) => state.createAiIndicatorVariant,
  );
  const forkPresetToPersonal = useWorkspaceStore(
    (state) => state.forkPresetToPersonal,
  );
  const removePreset = useWorkspaceStore(
    (state) => state.removeIndicatorPreset,
  );
  const save = useWorkspaceStore((state) => state.save);

  const [selectedMotorId, setSelectedMotorId] = useState(
    AI_INDICATOR_DEFINITIONS[0]?.id ?? "",
  );
  ...
  function handleCreateVariant() {
    onMessage(null);
    const definition = AI_INDICATOR_DEFINITIONS.find(
      (item) => item.id === selectedMotorId,
    );
    ...
    <section className="mb-4 space-y-4 rounded-lg border border-primary/20 bg-primary/5 p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-xs text-muted-foreground">
            Scores compuestos deterministas (0–100) alineados con rastreadores
            híbridos. Crea variantes con pesos o parámetros distintos, añádelas
            al gráfico con ★ y publícalas como personal cuando quieras editarlas
            libremente.
          </p>
        </div>
      ...
      <div className="grid gap-3 lg:grid-cols-3">
        {AI_INDICATOR_DEFINITIONS.map((definition) => {
          const preset = systemAiPresets.find(
      ...
      {userAiVariants.length > 0 && (
        <div className="rounded-md border border-border bg-background/80 p-3">
          <p className="text-xs font-medium">
            Tus variantes IA ({userAiVariants.length})
          </p>
