...
describe('chart-time-sync', () => {
  it('compara rangos lógicos con tolerancia', () => {
    expect(logicalRangesEqual({ from: 0, to: 100 }, { from: 0, to: 100 })).toBe(true);
    expect(logicalRangesEqual({ from: 10.0001, to: 50 }, { from: 10, to: 50.0001 })).toBe(true);
    expect(logicalRangesEqual({ from: 0, to: 100 }, { from: 5, to: 100 })).toBe(false);
  });
  ...
  it('zoom sin ancla se centra en el rango', () => {
    ...
    expect(zoomed.to - zoomed.from).toBeCloseTo(100 * CHART_HORIZONTAL_ZOOM_FACTOR, 6);
  });
