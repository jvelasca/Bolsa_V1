...
import { ChevronDown, ChevronUp, Copy, Eye, EyeOff, Save, Trash2 } from 'lucide-react';
...
import { Dialog, DialogTabs, FieldRow, checkboxClassName, inputClassName } from '@/components/ui/dialog';
...
export function IndicatorInstanceConfigDialog() {
  ...
  const duplicateInstance = useWorkspaceStore((s) => s.duplicateIndicatorInstance);
  const forkInstanceToPersonalPreset = useWorkspaceStore((s) => s.forkInstanceToPersonalPreset);
  const forkPresetToPersonal = useWorkspaceStore((s) => s.forkPresetToPersonal);
  const swapChartInstanceToPreset = useWorkspaceStore((s) => s.swapChartInstanceToPreset);
  const removeInstance = useWorkspaceStore((s) => s.removeIndicatorInstance);
  const reorderInstances = useWorkspaceStore((s) => s.reorderIndicatorInstances);
  const save = useWorkspaceStore((s) => s.save);

  const chartId = target?.chartId ?? activeTab?.id;
  const instance =
    target && activeTab
      ? activeTab.indicatorInstances.find((item) => item.instanceId === target.instanceId)
      : undefined;
  const definition = instance ? findIndicatorDefinition(instance.definitionId) : undefined;
  const timeframe = activeTab?.timeframe ?? '1d';

  const [tab, setTab] = useState<ConfigTab>('data');
  const [draft, setDraft] = useState<ChartIndicatorInstance | null>(instance ?? null);
  ...
  const subIndex = isSubPanel
    ? subIndicators.findIndex((item) => item.instanceId === activeDraft.instanceId)
    : -1;
  const canMoveUp = isSubPanel && subIndex > 0;
  const canMoveDown = isSubPanel && subIndex >= 0 && subIndex < subIndicators.length - 1;
  ...
  function patchColor(hex: string) {
    setDraft((prev) =>
      prev
        ? {
            ...prev,
            parameters: normalizeParameters(definition!, { ...prev.parameters, color: hex }),
          }
        : prev,
    );
  }
  ...
  function toggleHiddenNow() {
    ...
    const nextId = updateInstance(activeDraft.instanceId, { visible: nextVisible }, activeChartId);
    if (!nextId) return;
    setDraft((prev) => (prev ? { ...prev, visible: nextVisible, instanceId: nextId } : prev));
    setSelectedIndicator(nextId);
    save();
    requestChartReflow();
  }
  ...
  function saveAsPersonalPreset() {
    const name = window.prompt('Nombre del indicador personal:', instanceLabel(activeDraft));
    if (!name?.trim()) return;
    ...
    if (
      ...
      const nextId = swapChartInstanceToPreset(activeDraft.instanceId, newPresetId, activeChartId);
  ...
  function deleteNow() {
    if (!window.confirm(`¿Eliminar ${instanceLabel(activeDraft)} del gráfico?`)) return;
  ...
  function moveNow(direction: 'up' | 'down') {
    ...
    if (!swapWith) return;
    reorderInstances(activeDraft.instanceId, swapWith.instanceId, activeChartId);
    save();
    requestChartReflow();
  }
  ...
  return (
    <Dialog
      ...
      className="max-w-md"
    >
      <DialogTabs tabs={TABS} active={tab} onChange={(id) => setTab(id as ConfigTab)} />
      ...
      {tab === 'data' && (
        <div className="mt-4 space-y-3">
          <p className="text-[11px] text-muted-foreground">
            ...
            ). Al cambiar la escala en la barra superior, todos los indicadores se actualizan.
          </p>
        ...
        <div className="mt-4 space-y-4">
          ...
          <FieldRow label="Grosor de línea">
            <select
              ...
              onChange={(event) => patch({ lineWidth: Number(event.target.value) })}
            >
              {[1, 2, 3, 4].map((width) => (
        ...
        <div className="mt-4 space-y-3">
          ...
          <label className="flex items-center gap-2 text-sm">
            <input
              ...
              onChange={(event) => patch({ showLastValue: event.target.checked })}
            />
            Etiqueta del último valor en la escala de precios
          </label>
          <p className="text-[11px] text-muted-foreground">
            Si ocultas el indicador, deja de dibujarse pero permanece en la lista bajo el precio
      ...
      <div className="mt-4 space-y-3 border-t border-border pt-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          <Button type="button" variant="outline" size="sm" onClick={toggleHiddenNow}>
            {activeDraft.visible ? (
          ...
          <Button type="button" variant="outline" size="sm" onClick={saveAsPersonalPreset}>
            <Save className="mr-1.5 h-3.5 w-3.5" />
            Guardar como personal
          </Button>
          <Button type="button" variant="outline" size="sm" onClick={duplicateNow}>
