...
const MENU_OPTIONS = Object.fromEntries(
  (Object.keys(CHART_CURSOR_FIELD_LABELS) as ChartCursorBarField[]).map((id) => [
    id,
    { id, label: CHART_CURSOR_FIELD_LABELS[id], hint: CHART_CURSOR_FIELD_TIPS[id] },
  ]),
) as Record<ChartCursorBarField, { id: ChartCursorBarField; label: string; hint: string }>;
...
const CURSOR_PRICE_VALUE_CLASS = 'inline-block w-[3.5rem] truncate text-right tabular-nums text-foreground';
const CURSOR_CHANGE_VALUE_CLASS =
  'inline-block w-[8.25rem] truncate text-right font-medium tabular-nums';
const CURSOR_VOLUME_VALUE_CLASS = 'inline-block w-[2.25rem] truncate text-right tabular-nums text-foreground';
...
function resolveActiveBar(
  ...
  if (instrumentId && hoveredInstrumentId === instrumentId && hoveredBar) return hoveredBar;
...
function renderCursorChip(field: ChartCursorBarField, bar: OhlcvBarDto) {
  switch (field) {
    ...
    case 'changePct': {
      ...
      if (!change) {
        return <span className="inline-block w-full truncate text-muted-foreground">—</span>;
      }
      return (
        <span
          className={cn(
    ...
    case 'volume':
      return (
        <span className="inline-flex w-full items-baseline justify-between gap-0.5 tabular-nums text-muted-foreground">
          ...
          <span className={CURSOR_VOLUME_VALUE_CLASS}>{formatVolume(bar.volume)}</span>
        </span>
      );
    default:
      return null;
...
export function ChartCursorZone({ instrumentId, bars, className }: ChartCursorZoneProps) {
  const hoveredInstrumentId = useChartCursorStore((s) => s.instrumentId);
  const hoveredBar = useChartCursorStore((s) => s.hoveredBar);
  const { favorites, toggleFavorite, isFavorite, anchor } = useChartCursorFieldFavorites();
  ...
  if (!activeBar) {
    return (
      <div className={cn(CHART_BAR_ZONE_ROW_CLASS, className)} title={cursorHint}>
