    ...
    <div className="flex items-center gap-2">
      ...
      <label className="flex items-center gap-2 text-xs">
        <input
          ...
          onChange={(event) => onChange(event.target.checked ? 'transparent' : '#1e293b')}
        />
        {transparentLabel}
      </label>
    </div>
...
export function FavoritesWorkspaceSection({
  ...
  return (
    <section className="space-y-3">
      <div>
        <p className="text-xs font-medium text-foreground">Accesos directos (favoritos)</p>
        <p className="mt-1 text-[11px] text-muted-foreground">
          El icono de cada zona muestra el valor activo (p. ej. 1D, Velas). Usa la{' '}
          <strong>estrella</strong> en el menú para fijar chips de acceso rápido opcionales.
        </p>
      </div>
      ...
      <div className="space-y-2 rounded-md border border-border bg-muted/10 p-2 text-[11px]">
        ...
        <p className="text-muted-foreground">
          {draft.instrumentFieldFavorites
            ...
            .map((id) => CHART_INSTRUMENT_FIELD_LABELS[id as ChartInstrumentBarField])
            .join(' · ') || 'Solo ancla'}
        </p>
      ...
      <Button type="button" variant="outline" size="sm" onClick={resetFavorites}>
...
export function DataBarWorkspaceDefaultsSection({
  ...
  return (
    <section className="space-y-3">
      <div>
        <p className="text-xs font-medium text-foreground">Defaults del workspace</p>
        <p className="mt-1 text-[11px] text-muted-foreground">
          Valores que heredan los gráficos nuevos y los tabs con «usar valores por defecto».
        </p>
      </div>
      ...
      <FieldRow label="Timeframe inicial (Escala)">
        <DefaultTimeframeSelect
          ...
          onChange={(defaultTimeframe) => onChange({ ...draft, defaultTimeframe })}
        />
      </FieldRow>
      ...
      <FieldRow label="Estilo inicial (tipo de barra)">
        <DefaultSeriesTypeSelect
          ...
          onChange={(defaultSeriesType) => onChange({ ...draft, defaultSeriesType })}
        />
      </FieldRow>
      ...
      <div className="space-y-2">
        <p className="text-xs font-medium text-foreground">Distribución por defecto</p>
        <label className="flex items-center gap-2 text-xs">
          <input
            type="checkbox"
            className={checkboxClassName}
      ...
      <div className="space-y-2">
        <p className="text-xs font-medium text-foreground">Visibilidad por defecto</p>
