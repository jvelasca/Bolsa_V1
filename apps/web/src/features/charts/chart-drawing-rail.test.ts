...
describe("chart drawing rail favorites", () => {
  ...
  it("does not duplicate active extra favorite on group icon", () => {
    expect(
      resolveGroupRailIconTool("lines", "ray", { lines: "ray" }, [
        ...favorites,
      ]),
    ).toBe("line");
  });
  ...
  it("shows active non-extra tool on group icon", () => {
    expect(
      resolveGroupRailIconTool("lines", "line", { lines: "line" }, [
        ...favorites,
      ]),
    ).toBe("line");
  });
  ...
  it("shows menu-only tool on group icon when active", () => {
    ...
    expect(resolveGroupRailIconTool("lines", "vline", {}, [...favs])).toBe(
      "vline",
    );
  });
  ...
  it("activates primary or last non-extra tool from group button", () => {
    expect(
      resolveGroupRailActivateTool("lines", { lines: "ray" }, [...favorites]),
    ).toBe("line");
    expect(
      resolveGroupRailActivateTool("lines", { lines: "line" }, [...favorites]),
    ).toBe("line");
    expect(
      resolveGroupRailActivateTool("lines", { lines: "vline" }, [...favorites]),
    ).toBe("vline");
  });
  ...
  it("builds unified family blocks with primary included in bracket count", () => {
    ...
    const cursor = drawingRailFamilyBlocks(favs).find(
      (block) => block.groupId === "cursor",
    );
    const lines = drawingRailFamilyBlocks(favs).find(
      (block) => block.groupId === "lines",
    );
    const fibonacci = drawingRailFamilyBlocks(favs).find(
      (block) => block.groupId === "fibonacci",
    );
