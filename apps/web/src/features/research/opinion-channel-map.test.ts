...
describe('mapOpinionToChannel', () => {
  it('maps buy ★4+ to alarma and buy ★2–3 to aviso', () => {
    expect(mapOpinionToChannel({ stance: 'buy', dictamenStars: 5 })).toBe('alarma');
    expect(mapOpinionToChannel({ stance: 'buy', dictamenStars: 4 })).toBe('alarma');
    expect(mapOpinionToChannel({ stance: 'buy', dictamenStars: 3 })).toBe('aviso');
    expect(mapOpinionToChannel({ stance: 'buy', dictamenStars: 1 })).toBe('silent');
  });

  it('maps sell/reduce ★≥3 to alarma', () => {
    expect(mapOpinionToChannel({ stance: 'sell_exit', dictamenStars: 3 })).toBe('alarma');
    expect(mapOpinionToChannel({ stance: 'reduce', dictamenStars: 2 })).toBe('aviso');
  });

  it('silences hold_watch / no_trade', () => {
    expect(mapOpinionToChannel({ stance: 'hold_watch', dictamenStars: 5 })).toBe('silent');
    expect(mapOpinionToChannel({ stance: 'no_trade', dictamenStars: 1 })).toBe('silent');
  });
});
...
describe('buildOpinionChannelItems', () => {
  it('drops silent and sorts alarmas first', () => {
    const items = buildOpinionChannelItems([
      { symbol: 'IBE', opinion: opinion({ instrumentId: '1', stance: 'hold_watch', dictamenStars: 4 }) },
      { symbol: 'SAN', opinion: opinion({ instrumentId: '2', stance: 'buy', dictamenStars: 5 }) },
      { symbol: 'TEF', opinion: opinion({ instrumentId: '3', stance: 'overbought', dictamenStars: 3 }) },
    ]);
