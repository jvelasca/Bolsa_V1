...
function metricNum(metrics: Record<string, number | string | null> | undefined, key: string): number | null {
  if (!metrics) return null;
  const v = metrics[key];
  return typeof v === 'number' && Number.isFinite(v) ? v : null;
}

function formatMetric(v: number | null, kind: 'ratio' | 'pct' | 'money' | 'raw' = 'ratio'): string {
  if (v == null) return '—';
...
export function ResearchTrialResultBlock({
  ...
  const preset = trial?.presetKey ?? (typeof trial?.params?.presetKey === 'string' ? trial.params.presetKey : null);
  ...
  const maxDd = metricNum(m, 'maxDrawdownPct') ?? fallback?.maxDrawdownPct ?? null;
  const pnl = metricNum(m, 'totalReturnPct') ?? fallback?.totalReturnPct ?? null;
  ...
  const commissionBps = metricNum(m, 'commissionBps') ?? fallback?.commissionBps ?? null;
  const slippageBps = metricNum(m, 'slippageBps') ?? fallback?.slippageBps ?? null;
  ...
  return (
    <div className={cn('rounded-lg border border-border bg-muted/20 p-3 space-y-3', className)}>
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Research</p>
          <p className="mt-0.5 font-mono text-sm text-foreground">
            trialId: {id ?? '—'}
            {id && (
              <Button type="button" variant="ghost" size="sm" className="ml-1 h-7 px-2" onClick={() => void copyId()}>
                Copiar
              </Button>
            )}
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            K contribution: <span className="text-foreground font-medium">{k}</span>
      ...
      <div>
        <p className="mb-2 text-xs font-medium text-foreground">Performance (IS)</p>
      ...
      <div>
        <p className="mb-1 text-xs font-medium text-foreground">Costes aplicados</p>
        <p className="text-xs text-muted-foreground">
          Commission: {commissionBps ?? 0} bps · Slippage: {slippageBps ?? 0} bps · Spread:{' '}
          {spreadBps ?? 0} bps
        </p>
      </div>
    </div>
