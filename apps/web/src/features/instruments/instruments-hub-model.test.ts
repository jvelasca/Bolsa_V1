...
describe('instruments-hub-model', () => {
  const catalog = [
    inst({ id: '1', symbol: 'SAN', name: 'Banco Santander', meta: { changePct: 1.2, lastClose: 4.5, barCount: 200, lastSync: null, lastBarDate: null } }),
    inst({ id: '2', symbol: 'ACS', name: 'ACS', isin: 'ES0167050915', meta: { changePct: -0.5, lastClose: 41, barCount: 50, lastSync: null, lastBarDate: null } }),
    inst({ id: '3', symbol: 'IBE', name: 'Iberdrola', sector: 'Utilities', meta: { changePct: 0.1, lastClose: 12, barCount: 300, lastSync: null, lastBarDate: null } }),
  ];
  ...
  it('sorts by listCount and filters portfolio', () => {
    ...
    expect(
      instrumentMatchesHubQuery(catalog[1]!, 'watch', membershipsByInstrument.get('2')),
    ).toBe(true);
  });
