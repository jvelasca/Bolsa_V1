    ...
    >
      <div className="space-y-4 text-sm">
        <p className="text-muted-foreground">
          Descargaremos hasta 5 años de velas diarias (OHLCV) y las guardaremos localmente, igual
          que en la ficha de instrumento.
        </p>

        {syncMutation.isError && (
          <p className="text-destructive">{formatSyncError(syncMutation.error)}</p>
        )}

        {syncMutation.isSuccess && syncMutation.data?.data.status === 'partial' && (
          <p className="text-amber-400">
            Sincronización parcial: {syncMutation.data.data.error ?? 'sin barras nuevas'}
          </p>
        )}
        ...
        <div className="flex flex-wrap justify-end gap-2 border-t border-border pt-4">
          ...
          <Button type="button" disabled={syncMutation.isPending} onClick={() => void handleSync()}>
