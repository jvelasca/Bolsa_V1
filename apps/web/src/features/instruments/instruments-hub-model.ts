...
export type InstrumentsHubScopeFilter =
  | "all"
  | "estudio"
  | "portfolio"
  | "list";
...
export function compareInstrumentsForHub(
  ...
  switch (key) {
    case "name":
      return (
        cmpString(a.name, b.name, dir) || cmpString(a.symbol, b.symbol, "asc")
      );
    ...
    case "listCount": {
      ...
      return (
        cmpNullableNumber(ac, bc, dir) || cmpString(a.symbol, b.symbol, "asc")
      );
    }

    case "unrealizedPnl": {
      const ap =
        enrichment?.positionsByInstrument?.get(a.id)?.unrealizedPnl ?? null;

      const bp =
        enrichment?.positionsByInstrument?.get(b.id)?.unrealizedPnl ?? null;

      return (
        cmpNullableNumber(ap, bp, dir) || cmpString(a.symbol, b.symbol, "asc")
      );
    }
    ...
    case "scoreFa": {
      ...
      return (
        cmpNullableNumber(af, bf, dir) || cmpString(a.symbol, b.symbol, "asc")
      );
    }

    case "scoreTa": {
      const at =
        enrichment?.taByInstrument?.get(a.id)?.technicalDisplay100 ?? null;

      const bt =
        enrichment?.taByInstrument?.get(b.id)?.technicalDisplay100 ?? null;

      return (
        cmpNullableNumber(at, bt, dir) || cmpString(a.symbol, b.symbol, "asc")
      );
    }
    ...
    case "scoreIo": {
      ...
      return (
        cmpNullableNumber(ai, bi, dir) || cmpString(a.symbol, b.symbol, "asc")
      );
    }
