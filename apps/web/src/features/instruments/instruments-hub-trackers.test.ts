...
describe("instruments-hub-trackers", () => {
  ...
  it("covers pin and list memberships", () => {
    const memberships = new Map([
      ["inst-a", [{ listId: "list-1", listName: "IBEX", source: "catalog" }]],
      ["inst-b", [{ listId: "list-1", listName: "IBEX", source: "catalog" }]],
    ]);
    ...
    expect(
      map
        ...
        ?.map((c) => c.trackerId)
        .sort(),
    ).toEqual(["t-list", "t-pin"]);
    expect(
      map.get("inst-a")?.find((c) => c.trackerId === "t-pin")?.coverage,
    ).toBe("pin");
