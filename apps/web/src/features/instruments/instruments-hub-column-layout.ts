...
export const INSTRUMENTS_HUB_COLUMN_LABELS: Record<
...
export function normalizeInstrumentsHubColumnLayout(
  ...
  for (const def of defaults) {
    ...
    const insertAt = prevId ? ordered.findIndex((c) => c.id === prevId) + 1 : 0;
...
export function toggleInstrumentsHubFavoriteColumn(
  ...
  return DEFAULT_INSTRUMENTS_HUB_COLUMN_LAYOUT.map(
    (column) => column.id,
...
export function isNumericInstrumentsHubColumn(
  columnId: InstrumentsHubColumnId,
): boolean {
  return (
    columnId === "price" ||
...
export function isSortableInstrumentsHubColumn(
  columnId: InstrumentsHubColumnId,
): boolean {
...
export function cycleInstrumentsHubColumnSort(
  ...
  if (current?.columnId !== columnId) {
    return {
      columnId,
      direction:
        isNumericInstrumentsHubColumn(columnId) || columnId === "lastBar"
          ? "desc"
          : "asc",
    };
  }
...
export function fitInstrumentsHubColumnsToContent(
  ...
  return layout.map((column) => {
    ...
    for (const sample of samples) {
      ...
      contentW = Math.max(
        contentW,
        measureInstrumentsHubTextWidth(sample) + FIT_CELL_PAD,
      );
    }
    // La cabecera cuenta como muestra más (por si el label es más largo que las celdas).
    contentW = Math.max(
      contentW,
      measureInstrumentsHubTextWidth(header) + FIT_CELL_PAD,
    );
    const floor =
      column.id === "actions"
        ? 120
        : column.id === "scoreFa" ||
          ...
          ? 52
          : MIN_WIDTH;
    const width = clampInstrumentsHubColumnWidth(
