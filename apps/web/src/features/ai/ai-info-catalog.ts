...
export const AI_INFO_SURFACES: Record<AiInfoSurfaceId, AiInfoSurface> = {
  fa_copilot: {
    ...
    does:
      'Explica Score_FUND, pilares y facts ya calculados (narrativa). Puede usar Ollama o heurística local.',
    doesNot:
      'No recalcula ratios, Piotroski, DCF ni Score_FUND. No escribe en BD ni envía órdenes.',
    engineHint: 'Proxy First → Ollama / heurística · scoreVersion fund_score_v1',
  },
  fa_filings: {
    id: 'fa_filings',
    title: 'Filings · resumir / preguntar',
    does:
      'Resume o responde sobre documentos 10-K/u.o. con RAG local (TF-IDF) + narrativa opcional.',
    doesNot:
      'No altera Score_FUND, Composite ni el gate FA. No es asesoramiento legal.',
    engineHint: 'Filings en disco · summarize/ask · sin embedding cloud obligatorio',
  },
  backtest_coach: {
    id: 'backtest_coach',
    title: 'Coach de backtesting',
    does:
      'El ranking TOP ★ es local y determinista. El LLM (si está activo) solo narra o audita en adversario; no sustituye el TOP.',
    doesNot:
      'No calcula PnL, no guarda Finalistas solo, no ejecuta paper. Lab Optimize es determinista (no LLM).',
    engineHint: 'rankTechnicalRecommendations local · LLM opcional (prefs Coach)',
  },
  strategy_draft: {
    id: 'strategy_draft',
    title: 'Borrador de estrategia (prompt)',
    does:
      'Interpreta lenguaje natural → borrador de StrategyDefinition para revisar y guardar.',
    doesNot:
  ...
  chart_propose: {
    ...
    does:
      'Propose determinista (DecisionRuntime) con assessments TA/FA/… → cola Supervisado F3. Humano confirma.',
    doesNot:
  ...
  lab_optimize: {
    ...
    doesNot: 'No usa LLM para puntuar. No escribe Finalistas (vuelve el Coach²).',
