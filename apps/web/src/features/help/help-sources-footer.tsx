import { HELP_CONTENT_AS_OF, helpSectionMeta, type HelpRegistrySectionId } from '@/features/help/help-registry';
...
export function HelpSourcesFooter({ sectionId }: { sectionId: HelpRegistrySectionId }) {
      ...
      <ul className="mt-1.5 space-y-0.5">
        {meta.sources.map((src) => (
          <li key={src.path} className="font-mono break-all">
            <span className="text-muted-foreground/70">[{ROLE_LABEL[src.role]}]</span> {src.path}
            {src.note ? <span className="font-sans text-muted-foreground"> — {src.note}</span> : null}
          </li>
        ))}
      </ul>
    </div>
