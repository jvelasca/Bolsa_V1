    ...
    <div className="space-y-4" data-testid="notifications-settings">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Correo y toasts para Alarmas del Estudio (Asesor). Un operador por
            navegador hasta multiusuario. El envío de email requiere SMTP en el
            servidor (<code className="text-[10px]">SMTP_HOST</code> +{" "}
            <code className="text-[10px]">SMTP_FROM</code> en{" "}
            <code className="text-[10px]">.env</code>, luego reiniciar la API).
            Sin eso, «Enviar ahora» y el digest EOD responderán «SMTP
            incompleto».
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-5 text-sm">
          <label className="flex items-start gap-3">
            <input
              ...
              onChange={(e) =>
                setPrefs({ alarmaToastEnabled: e.target.checked })
              }
            />
            <span>
          ...
          <label className="flex items-start gap-3">
            <input
              ...
              onChange={(e) =>
                setPrefs({ alarmaEmailEnabled: e.target.checked })
              }
            />
            <span>
              <span className="font-medium">Email de Alarmas</span>
              <p className="mt-1 text-xs text-muted-foreground">
                Tras recalcular EOD (manual o cron), envía resumen de Alarmas al
                correo indicado.
              </p>
            </span>
          </label>
          ...
          <label className="flex items-start gap-3">
            ...
            <span>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Suscripción al digest del día (Asesor → Diario). Tras eod-batch
                o «Enviar ahora» manda HTML al mismo correo. Requiere SMTP
                servidor.
              </p>
            </span>
          </label>
          ...
          <label className="flex items-start gap-3">
            ...
            <span>
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                R4 — PDF ligero (Helvetica) junto al HTML. También puedes
                descargarlo en Diario sin email. Flag servidor opcional:{" "}
                <code className="text-[10px]">
                  DAILY_OPS_DIGEST_PDF_ENABLED
                </code>
                .
              </p>
            </span>
          ...
          <p className="rounded-md border border-border/70 bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
            Multiusuario: este buzón pasará al perfil de usuario. Hoy es
            preferencia local del navegador (no sustituye cuentas DEMO /
            inversión).
          </p>
        </CardContent>
      </Card>
