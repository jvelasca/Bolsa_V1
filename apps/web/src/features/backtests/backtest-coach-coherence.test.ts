...
function catalogForInstrument(
  symbol: string,
  lateByType: Record<string, number>,
): ExplorePresetRow[] {
  const defs: Array<{
...
describe("coach coherence · periodReturns path", () => {
  ...
  it("soft fallback loses to stamped thirds when comparable", () => {
    ...
    expect(top[0]?.usedSoftFallback).toBe(false);
    expect(
      top.find((t) => t.row.strategyType === "sma_crossover")?.usedSoftFallback,
    ).toBe(true);
  });
});
...
describe("coach coherence · diversify + quality", () => {
  ...
  it("TOP-3 diversifies categories when possible", () => {
    ...
    expect(
      top.filter((t) => t.row.category === "trend").length,
  ...
  it("does not crown severely bad late+excess when better options exist", () => {
    ...
    expect(
      top.find((t) => t.row.strategyType === "golden_cross")?.qualityFlagged,
    ).toBe(true);
  });
...
describe("coach coherence · multi-instrument battery", () => {
  ...
  it("TOP #1 changes with instrument late-window profile (not sticky alphabetical)", () => {
    ...
    expect(tops.find((t) => t.symbol === "ACS")?.types[0]).toBe(
      "sma_crossover",
    );
    ...
    // IBE momentum
    expect(tops.find((t) => t.symbol === "IBE")?.types[0]).toBe(
      "macd_signal_cross",
    );
  });
      ...
      lookup: {
        ...
        createFromPreset: async (input) => {
          ...
          return {
            ...
            name: input.name,
            presetKey:
              input.presetKey as StrategyDefinitionSummaryDto["presetKey"],
            origin: "preset",
            timeframe: "1d",
            kind: "rules",
