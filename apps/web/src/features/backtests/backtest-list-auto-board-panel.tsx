...
function PhaseIcon({ phase }: { phase: ListAutoRowPhase }) {
  if (phase === 'running') {
    return <Loader2 className="h-3.5 w-3.5 animate-spin text-sky-500" aria-hidden />;
  }
  if (phase === 'saved') {
    return <Check className="h-3.5 w-3.5 text-emerald-600 dark:text-emerald-400" aria-hidden />;
  }
  if (phase === 'omitted') {
    return <ZapOff className="h-3.5 w-3.5 text-violet-600 dark:text-violet-300" aria-hidden />;
  }
  if (phase === 'same') {
    return <Equal className="h-3.5 w-3.5 text-muted-foreground" aria-hidden />;
  }
  if (phase === 'skipped') {
    return <SkipForward className="h-3.5 w-3.5 text-amber-600 dark:text-amber-400" aria-hidden />;
  }
  if (phase === 'aborted') {
...
function RowStatus({ row }: { row: ListAutoBoardRow }) {
  return (
    <span className="inline-flex items-center gap-1.5">
      ...
      <span
        className={cn(
          row.phase === 'running' && 'font-medium text-sky-700 dark:text-sky-300',
      ...
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="min-w-0 space-y-0.5">
          ...
          <p className="text-[10px] leading-snug text-muted-foreground">
            {progress.doneCount}/{progress.total} · {progress.changedCount} cambiaron ·{' '}
            {progress.omittedCount} omitidos (frescos) · {progress.skippedCount} skip Lab
            {reevalAttention > 0 ? ` · ${reevalAttention} a revisar (CORE-R)` : ''}
          </p>
        </div>
        <p
          className={cn(
        ...
        <table className="w-full border-collapse text-left text-[11px]">
          <thead className="sticky top-0 z-[1] bg-muted/95 backdrop-blur-sm">
            <tr className="border-b border-border/60 text-[10px] uppercase tracking-wide text-muted-foreground">
              ...
              {!compact && <th className="px-2 py-1.5 font-medium">Últ. búsqueda</th>}
          ...
          <tbody>
            {board.rows.map((row) => {
              ...
              const selected = Boolean(
                selectedInstrumentId && row.instrumentId === selectedInstrumentId,
              );
              const actions =
              ...
              return (
                ...
                >
                  ...
                  <td className="px-2 py-1.5">
                    ...
                    {row.name && !compact ? (
                      <span className="ml-1.5 text-muted-foreground">{row.name}</span>
                    ) : null}
                  </td>
                  <td className="px-2 py-1.5">
                    <RowStatus row={row} />
