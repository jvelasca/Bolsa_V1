...
export function smaAnchorPeriods(
  ...
  const periods = specs
    .filter(
      (spec) => spec.definitionId === "sma" || spec.definitionId === "ema",
    )
...
export function rsiAnchorParams(
  ...
  if (optimizeFamilyForStrategy(strategyType) !== "rsi_mean_reversion")
    return null;
  const specs = presetIndicatorSpecs(strategyType);
...
export function macdAnchorParams(
  ...
  if (optimizeFamilyForStrategy(strategyType) !== "macd_signal_cross")
    return null;
  const macd = presetIndicatorSpecs(strategyType).find(
    (spec) => spec.definitionId === "macd",
  );
...
export function buildOptimizeSeedFromExploreRow(
  ...
  const sma =
    family === "sma_crossover" ? smaAnchorPeriods(row.strategyType) : null;
  const rsi =
    family === "rsi_mean_reversion" ? rsiAnchorParams(row.strategyType) : null;
  const macd =
    family === "macd_signal_cross" ? macdAnchorParams(row.strategyType) : null;
...
export function anchorParamsFromTrialBag(
...
export function buildOptimizeSeedFromRun(opts: {
  ...
  const known = (
    BACKTEST_STRATEGIES as Record<string, { label: string } | undefined>
  )[rawType];
  const strategyType = (
    known ? rawType : "sma_crossover"
  ) as BacktestStrategyType;
  const strategyLabel =
    opts.strategyLabel ??
    known?.label ??
    (rawType ? String(rawType) : "Estrategia");
  const family = optimizeFamilyForStrategy(strategyType) ?? "sma_crossover";
  const catalogSma =
    family === "sma_crossover" ? smaAnchorPeriods(strategyType) : null;
  const catalogRsi =
    family === "rsi_mean_reversion" ? rsiAnchorParams(strategyType) : null;
  const catalogMacd =
    family === "macd_signal_cross" ? macdAnchorParams(strategyType) : null;
  const ret = opts.totalReturnPct;
  const dd = opts.maxDrawdownPct;
  const barLimit =
    opts.barCount && opts.barCount > 0 ? opts.barCount : undefined;
  return {
    instrumentId: opts.instrumentId,
    symbol: opts.symbol,
    strategyType,
    ...
    anchorFast:
      opts.anchorFast ??
      fromTrial.anchorFast ??
      catalogSma?.fast ??
      catalogMacd?.fast,
    anchorSlow:
      opts.anchorSlow ??
      fromTrial.anchorSlow ??
      catalogSma?.slow ??
      catalogMacd?.slow,
    anchorSignal:
      opts.anchorSignal ?? fromTrial.anchorSignal ?? catalogMacd?.signal,
    anchorPeriod:
      opts.anchorPeriod ?? fromTrial.anchorPeriod ?? catalogRsi?.period,
    anchorOversold:
      opts.anchorOversold ?? fromTrial.anchorOversold ?? catalogRsi?.oversold,
    anchorOverbought:
      opts.anchorOverbought ??
      fromTrial.anchorOverbought ??
      catalogRsi?.overbought,
    anchorReturnPct: ret,
    anchorMaxDrawdownPct: dd,
...
export function suggestBarLimit(
  barCount: number | undefined,
  fallback = 500,
): number {
  if (barCount == null || !Number.isFinite(barCount) || barCount <= 0)
    return fallback;
