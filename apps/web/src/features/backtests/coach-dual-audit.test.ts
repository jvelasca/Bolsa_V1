...
describe("coach dual audit", () => {
  it("vetoes dead-cat bounce and keeps solid #1", () => {
    ...
    expect(bundle.recommendations[0]?.row.strategyType).not.toBe(
      "vwap_reclaim",
    );
    expect(bundle.recommendations[0]?.row.excessReturnPct).toBeGreaterThan(0);
    expect(["consensus", "discrepancy", "weak"]).toContain(
      bundle.audit.confidence,
    );
    expect(bundle.audit.quorum.chips.map((c) => c.id)).toEqual([
      "A",
      "A2",
      "B",
      "C",
  ...
  it("LLM veto demotes type (may remain as last-resort #3 with ★≤2)", () => {
    ...
    const smaSlot = bundle.recommendations.find(
      (r) => r.row.strategyType === "sma_crossover",
    );
  ...
  it("challenge fails when #1 loses buy & hold", () => {
    ...
    expect(
      challenge.checks.some((c) => c.code === "top_beats_bh" && !c.passed),
    ).toBe(true);
  });
  ...
  it("red-team flags clone family as soft fail", () => {
    ...
    expect(
      challenge.checks.some((c) => c.code === "family_diversity" && !c.passed),
    ).toBe(true);
  });
