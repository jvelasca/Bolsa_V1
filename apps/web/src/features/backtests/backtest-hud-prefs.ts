...
export const DEFAULT_CURSOR_PANEL_POS: BacktestCursorPanelPos = { x: 12, y: 12 };
...
function lockedIds<T extends string>(options: BacktestHudFieldOption<T>[]): Set<T> {
...
function sanitizeFavorites<T extends string>(
  ...
  const list = Array.isArray(raw)
    ? (raw.filter((id): id is T => typeof id === 'string' && valid.has(id as T)) as T[])
    : [...defaults];
...
export function loadBacktestHudPrefs(): BacktestHudPrefs {
  ...
  try {
    ...
    return {
      ...
      cursorPanelPos: {
        x: typeof pos?.x === 'number' && Number.isFinite(pos.x) ? pos.x : DEFAULT_CURSOR_PANEL_POS.x,
        y: typeof pos?.y === 'number' && Number.isFinite(pos.y) ? pos.y : DEFAULT_CURSOR_PANEL_POS.y,
      },
    };
  } catch {
...
export function patchBacktestHudPrefs(patch: Partial<BacktestHudPrefs>): BacktestHudPrefs {
...
export function isFavoriteVisible<T extends string>(favorites: T[], id: T): boolean {
