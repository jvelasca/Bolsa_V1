...
describe('coach-top-save', () => {
  ...
  it('creates missing presets and keeps 3 unique types', async () => {
    const create = vi.fn(async (input: { name: string; presetKey: string }) => ({
  ...
  it('mergeStrategiesWithTopSlots fetches missing finalist ids', async () => {
    const merged = await mergeStrategiesWithTopSlots({
      ...
      slots: [{ strategyDefinitionId: 'a' }, { strategyDefinitionId: 'missing' }],
      getById: async (id) =>
        id === 'missing'
          ? {
              id: 'missing',
    ...
    expect(filterFinalistStrategies(merged, new Set(['missing']))).toHaveLength(1);
  });
});
