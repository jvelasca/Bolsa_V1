...
describe('optimizeFamilyForStrategy · coach coverage', () => {
  ...
  it('maps oscillators / pullback to RSI lab', () => {
    expect(optimizeFamilyForStrategy('stoch_oversold')).toBe('rsi_mean_reversion');
    expect(optimizeFamilyForStrategy('cci_oversold')).toBe('rsi_mean_reversion');
    expect(optimizeFamilyForStrategy('pullback_in_uptrend')).toBe('rsi_mean_reversion');
  });

  it('maps Bollinger / SMA200 via proxy lab families', () => {
    expect(optimizeFamilyForStrategy('bollinger_lower_bounce')).toBe('rsi_mean_reversion');
    expect(optimizeFamilyForStrategy('bollinger_upper_breakout')).toBe('sma_crossover');
    expect(optimizeFamilyForStrategy('price_above_sma200')).toBe('sma_crossover');
  ...
  it('anchors Death cross at 50/200 and EMA at 12/26', () => {
    ...
    expect(smaAnchorPeriods('ma_stack_bullish')).toEqual({ fast: 20, slow: 50 });
    expect(smaAnchorPeriods('price_above_sma200')).toEqual({ fast: 50, slow: 200 });
    expect(smaAnchorPeriods('bollinger_upper_breakout')).toEqual({ fast: 10, slow: 20 });
  });
  ...
  it('maps Donchian / ADX / Ichimoku / VWAP / SuperTrend to SMA lab', () => {
    expect(optimizeFamilyForStrategy('donchian_breakout')).toBe('sma_crossover');
    expect(optimizeFamilyForStrategy('adx_di_trend')).toBe('sma_crossover');
    expect(optimizeFamilyForStrategy('ichimoku_tk_cross')).toBe('sma_crossover');
    expect(optimizeFamilyForStrategy('vwap_reclaim')).toBe('sma_crossover');
    expect(optimizeFamilyForStrategy('supertrend_follow')).toBe('sma_crossover');
    expect(smaAnchorPeriods('ichimoku_tk_cross')).toEqual({ fast: 9, slow: 26 });
    expect(smaAnchorPeriods('donchian_breakout')).toEqual({ fast: 10, slow: 20 });
  ...
  it('covers all catalog presets via native or proxy lab', () => {
    const uncovered = STRATEGY_PRESET_KEYS.filter((k) => !isOptimizableStrategy(k));
