...
describe("deep technical coach · future stars", () => {
  ...
  it("resolveCoachScoreWeights reescala el resto al cambiar peso reciente", () => {
    ...
    expect(d.future + d.risk + d.edge + d.category + d.activity).toBeCloseTo(
      1,
      5,
    );
    ...
    expect(
      heavy.future + heavy.risk + heavy.edge + heavy.category + heavy.activity,
    ).toBeCloseTo(1, 5);
  });
  ...
  it("higher futureWeight increases score for late-strong rows", () => {
    ...
    const low = scoreTechnicalFit(lateStrong, {
      ...ctxBase,
      futureWeight: 0.3,
    });
    const high = scoreTechnicalFit(lateStrong, {
      ...ctxBase,
      futureWeight: 0.55,
    });
  ...
  it("ACS-like: Golden Cross can lead on total return while coach prefers SMA 20/50 on late equity", () => {
    ...
    const coach = rankTechnicalRecommendations(
      [golden, sma2050, stack],
      ctx,
      3,
    );
  ...
  it("prefers late-half winner over early historical champion", () => {
    ...
    expect(scoreTechnicalFit(recent, ctx)).toBeGreaterThan(
      scoreTechnicalFit(oldChamp, ctx),
    );
  ...
  it("BBVA-like: dead-cat bounce (early crash + mild late) must not beat solid winners", () => {
    ...
    expect(["sma_crossover", "macd_signal_cross"]).toContain(
      top[0]?.row.strategyType,
    );
