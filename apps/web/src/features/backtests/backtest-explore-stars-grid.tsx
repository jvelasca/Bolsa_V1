  ...
  return recommendations.length > 0 ? (
    <div className="space-y-2">
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        {recommendations.map((rec) => {
          ...
          return (
            <div
              key={
                rec.row.strategyDefinitionId ??
                `${rec.row.strategyType}-${rec.rank}`
              }
              className={cn(
      ...
      <p className="text-[10px] leading-snug text-muted-foreground">
        <strong className="font-medium text-foreground/80">Lab</strong> = abre
        el laboratorio con la candidata.{" "}
        <strong className="font-medium text-foreground/80">Encolar</strong> =
        lanza búsquedas en segundo plano. Guardar TOP-3 ≠ optimizar.
      </p>
    </div>
  ) : (
    <p className="text-xs text-muted-foreground">
      Aún no hay candidatas ★. Espera a que termine la batería o lanza «Probar +
      coach».
    </p>
  );
}
