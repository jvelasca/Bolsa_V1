  ...
  return (
    ...
    >
      ...
      {!model.plateau.isPlateau && !compact && (
        <p className="text-[10px] text-muted-foreground">{model.plateau.detail}</p>
      )}

      {tooDense ? (
        <p className="text-[10px] text-muted-foreground">
          Malla densa ({model.cells.length} celdas) — se muestra el top-5; abre menos trials o usa la
          tabla.
        </p>
      ) : (
            ...
            {model.yTicks.map((y) => (
              <div key={`row-${y}`} className="contents">
                ...
                {model.xTicks.map((x) => {
                  ...
                  const norm = heatmapNorm(cell?.score ?? null, model.scoreMin, model.scoreMax);
                  return (
                    <div
                      key={`${x}-${y}`}
                      title={
                      ...
                      className={cn(
                        ...
                        cell?.isBest && 'ring-1 ring-emerald-300 ring-offset-1 ring-offset-background',
                        cell?.score == null && 'bg-muted/30',
                      )}
                      style={cell?.score != null ? heatmapCellStyle(norm) : undefined}
                    />
                  );
                })}
              </div>
      ...
      <div className="space-y-0.5">
        ...
        <ol className="space-y-0.5 text-[10px] text-muted-foreground">
          {model.top5.map((t) => (
            <li key={`top-${t.rank}`} className="flex flex-wrap items-baseline gap-1.5">
              <span className="font-semibold text-foreground/90">#{t.rank}</span>
              <span className="tabular-nums text-foreground">{t.score.toFixed(2)}</span>
