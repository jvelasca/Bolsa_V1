...
describe('library-nav', () => {
  ...
  it('builds preset href', () => {
    ...
    expect(libraryHrefForPreset('sma_crossover')).toContain('preset=sma_crossover');
  });
  ...
  it('parses filter and search', () => {
    ...
    expect(parseLibraryNavFromSearch(new URLSearchParams('tab=run'))).toBeNull();
  });

  it('includes optional q', () => {
    expect(backtestLibraryHref({ library: 'all', q: ' ACS ' })).toContain('q=ACS');
  });
});
