...
describe("shouldToastRemoteEnqueue", () => {
  ...
  it("silent when already seen same or newer", () => {
    ...
    expect(
      shouldToastRemoteEnqueue(
        {
          lastRemoteEnqueueAt: "2026-08-02T11:00:00.000Z",
          lastRemoteEnqueueAdded: 1,
        },
        "2026-08-02T12:00:00.000Z",
      ).shouldToast,
  ...
  it("toasts when remote is newer than last seen", () => {
    expect(
      shouldToastRemoteEnqueue(
        {
          lastRemoteEnqueueAt: "2026-08-02T13:00:00.000Z",
          lastRemoteEnqueueAdded: 2,
        },
        "2026-08-02T12:00:00.000Z",
      ).shouldToast,
    ).toBe(true);
