 ...
 */
export function OptimizeCpcvReport({
  cpcv,
  pbo,
  labWfeHint,
}: OptimizeCpcvReportProps) {
  return (
    <div className="space-y-2 rounded-lg border border-violet-500/40 bg-violet-500/10 px-3 py-3 text-sm">
      <p className="font-medium text-foreground">
        CPCV ligero · {cpcv.pathCount} paths · {cpcv.nGroups} grupos (purge{" "}
        {cpcv.purgeBars} / embargo {cpcv.embargoBars})
      </p>
      ...
      <ul className="grid max-h-40 gap-1.5 overflow-y-auto text-[11px] text-muted-foreground sm:grid-cols-3">
        {cpcv.paths.map((path) => (
          ...
          >
            ...
            {path.oosMetrics != null
              ? ` → OOS ${path.oosMetrics.score.toFixed(1)}`
              : ""}
