...
describe("lab-adoption-memory", () => {
  ...
  it("guides SMA / RSI space around last Mejor (v0 widths sin plateau)", () => {
    ...
    expect(shouldApplyGuidedSpace(baseSmaRec(), "rsi_mean_reversion")).toBe(
      false,
    );
  });
  ...
  it("v0.1: meseta ensancha y pico estrecha el espacio SMA", () => {
    ...
    if (wide?.family === "sma_crossover" && tight?.family === "sma_crossover") {
      expect(wide.fast.max - wide.fast.min).toBeGreaterThan(
        tight.fast.max - tight.fast.min,
      );
      expect(wide.slow.max - wide.slow.min).toBeGreaterThan(
        tight.slow.max - tight.slow.min,
      );
      expect(
        formatLabAdoptionHint(
          baseSmaRec({
