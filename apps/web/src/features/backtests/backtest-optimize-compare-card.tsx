  ...
  return (
    <div className="rounded-md border border-border/60 bg-background/80 px-2 py-1.5">
      ...
      <div className="mt-0.5 flex flex-wrap items-baseline gap-2 text-xs">
        ...
        <span className="tabular-nums font-semibold text-foreground">{after}</span>
        {deltaLabel ? (
          <span
            className={cn(
              'rounded px-1 py-0.5 text-[10px] font-medium tabular-nums',
              better === 'up' && 'bg-emerald-500/15 text-emerald-800 dark:text-emerald-200',
              better === 'down' && 'bg-rose-500/15 text-rose-800 dark:text-rose-200',
...
export function BacktestOptimizeCompareCard({ snapshot, onDismiss, onBackToCoach }: Props) {
  const local = useMemo(() => buildLocalOptimizeCompareNote(snapshot), [snapshot]);
  ...
  const aiMutation = useMutation({
    mutationFn: () =>
      api.analyzeBacktestCoach({
        ...
        localSummary: [...local.summary, ...local.detailed, ...local.nextSteps].join('\n'),
      }),
    onSuccess: (res) => {
  ...
  const retBetter =
    snapshot.deltaReturnPct > 0.5 ? 'up' : snapshot.deltaReturnPct < -0.5 ? 'down' : 'neutral';
  const ddBetter =
    snapshot.deltaDrawdownPct < -0.5 ? 'up' : snapshot.deltaDrawdownPct > 0.5 ? 'down' : 'neutral';
    ...
    >
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          ...
          <p className="text-sm font-semibold text-foreground">{note.headline}</p>
        ...
        <div className="flex flex-wrap gap-1.5">
          {onBackToCoach && (
            <Button type="button" size="sm" variant="outline" className="h-7 text-[11px]" onClick={onBackToCoach}>
              Volver al coach
            </Button>
          )}
            <Button
          ...
          {onDismiss && (
            <Button type="button" size="sm" variant="ghost" className="h-7 text-[11px]" onClick={onDismiss}>
              Cerrar
            </Button>
          )}
        </div>
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        ...
        <Metric
          ...
          better={
            snapshot.deltaScore > 0.05 ? 'up' : snapshot.deltaScore < -0.05 ? 'down' : 'neutral'
          }
        />
      </div>
      <p className="text-[10px] text-muted-foreground">
        Verde = mejora operativa · ámbar = sin ventaja clara. El Lab mira sobre todo el score
        {snapshot.after.oosScore != null ? ' OOS' : ''} (no solo el retorno histórico).
      </p>
      ...
      <div>
        <p className="text-[11px] font-medium text-foreground">Análisis detallado</p>
      ...
      <div>
        <p className="text-[11px] font-medium text-foreground">Siguiente paso</p>
