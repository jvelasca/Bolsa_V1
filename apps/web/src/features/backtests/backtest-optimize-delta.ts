...
export function buildOptimizeBeforeAfter(
  ...
  const beforeFromSeed: OptimizeCompareSide | null =
    seed && seed.anchorReturnPct != null
      ? {
          ...
          maxDrawdownPct: seed.anchorMaxDrawdownPct ?? result.baseline.maxDrawdownPct,
  ...
  const before = beforeFromSeed ?? sideFromTrial(result.baseline, 'Antes (baseline lab)', family);
  ...
  if (snap.after.oosReturnPct != null) {
    summary.push(
      `OOS del mejor: retorno ${snap.after.oosReturnPct.toFixed(1)}%` +
        (snap.after.oosScore != null ? ` · score OOS ${snap.after.oosScore.toFixed(2)}` : ''),
    );
  }
  const detailed = [
...
export function mergeOptimizeCompareAi(
  ...
  return {
    ...
    summary:
      Array.isArray(llm.summary) && llm.summary.length > 0 ? llm.summary : local.summary,
    detailed:
      Array.isArray(llm.detailed) && llm.detailed.length > 0 ? llm.detailed : local.detailed,
    nextSteps:
      Array.isArray(llm.nextSteps) && llm.nextSteps.length > 0 ? llm.nextSteps : local.nextSteps,
    engineLabel,
  };
}

export function optimizeComparePromptBlob(snap: OptimizeBeforeAfterSnapshot): string {
  return [
