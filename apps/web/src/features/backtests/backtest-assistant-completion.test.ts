...
describe('assistant session progress', () => {
  ...
  it('unlocks semifinal only after universe ✓', () => {
    expect(canAutoRunStep('semifinal', emptyAssistantProgress(), true)).toBe(false);
    expect(
      canAutoRunStep('semifinal', { ...emptyAssistantProgress(), universeDone: true }, true),
    ).toBe(true);
  });
  ...
  it('withUniverseDone unlocks semifinal in the same tick (no 2º Play)', () => {
    ...
    expect(canAutoRunStep('semifinal', withUniverseDone(stale), true)).toBe(true);
  });
