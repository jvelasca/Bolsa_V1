...
export function StrategyMonitorPanel({
  ...
  useEffect(() => {
    ...
    const onShellTick = (ev: Event) => {
      ...
      if (detail.added > 0) {
        const open = useCoreRReviewQueueStore
          .getState()
          .openCount(effectiveListId);
        setCoreRSyncMsg(`Auto shell · +${detail.added} · ${open} abiertas.`);
      }
    };
    window.addEventListener(CORE_R_SCHEDULER_EVENT, onShellTick);
    return () =>
      window.removeEventListener(CORE_R_SCHEDULER_EVENT, onShellTick);
  }, [effectiveListId]);
        ...
        {effectiveListId ? (
          <div className="space-y-2 rounded-md border border-border/60 bg-muted/15 px-2.5 py-2">
            ...
            {coreRSyncMsg ? (
              ...
              <p className="text-[10px] text-muted-foreground">
                Encola juicios Lista AUTO y degradación PnL DEMO (−5% Lab / −10%
                cambio). Prefiere «{ESTUDIO_LIST_NAME}» al activar. No ejecuta
                ni pisa TOP.
              </p>
            )}
              ...
              <div className="space-y-1.5">
                ...
                <ul className="max-h-36 space-y-1.5 overflow-y-auto">
                  {coreROpen.map((item) => {
                    ...
                    const top =
                      topByInstrumentId.get(item.instrumentId) ?? null;
        ...
        {listsQuery.isLoading ? (
          ...
          <div className="rounded-md border border-dashed border-border/80 px-3 py-3 text-xs text-muted-foreground">
            <p className="font-medium text-foreground">
              No hay listas de watchlist
            </p>
            <p className="mt-1">
              Crea una en Watchlist (p. ej. IBEX) y vuelve aquí para ver TOP /
              paper / Proponer por valor.
            </p>
          </div>
        ) : !effectiveListId ? (
          <p className="text-xs text-muted-foreground">
            Elige una lista arriba.
          </p>
        ) : listDetailQuery.isError ? (
          <p className="text-xs text-destructive">
            No se pudo cargar la lista. Reintenta en un momento.
          </p>
        ) : listEmpty ? (
          <div className="rounded-md border border-dashed border-border/80 px-3 py-3 text-xs text-muted-foreground">
            <p className="font-medium text-foreground">Lista vacía</p>
            <p className="mt-1">
              Añade valores en Watchlist o elige otra lista.
            </p>
          </div>
          ...
          <div className="rounded-md border border-dashed border-border/80 px-3 py-3 text-xs text-muted-foreground">
            <p className="font-medium text-foreground">
              Sin Finalistas en esta lista
            </p>
            <p className="mt-1">
              Ningún valor tiene TOP guardado ({monitorInstruments.length} en
              cola). En Probar: Universo → Play (ciclo ON) → Lab → Coach² →
              Finalistas. O modo Lista + Play = Lista AUTO.
            </p>
          </div>
        ) : (
          <>
