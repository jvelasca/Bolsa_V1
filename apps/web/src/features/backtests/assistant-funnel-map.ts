...
export const FUNNEL_STAGES: FunnelStageDef[] = [
  ...
  },
  {
    ...
    n: 5,
    label: 'Finalistas',
    blurb: 'TOP active lab_validated si el ACK final y la política lo permiten.',
    navStep: 'finalists',
  },
];

export type FunnelStageStatus = 'pending' | 'active' | 'done' | 'skipped' | 'blocked';
...
export function resolveActiveFunnelStage(input: ResolveFunnelStageInput): FunnelStageId {
  const {
    progress,
    coachPass,
    fullCycleActive,
