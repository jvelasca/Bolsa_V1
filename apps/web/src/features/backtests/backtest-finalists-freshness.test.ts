...
describe('shouldSkipFinalistsSearch', () => {
  ...
  it('adopts any active TOP with slots and no stamp', () => {
    ...
    ).toEqual({ skip: true, reason: 'adopt_existing_top', adoptFingerprint: true });
  });
...
describe('clearLocalFreshnessFingerprint', () => {
  ...
  it('removes only the instrument|TF entry', () => {
    ...
    expect(readLocalFreshnessFingerprint('san', '1d')?.fingerprint).toBe('fp-san');
  });
});
...
describe('coachFacts roundtrip', () => {
  it('merges and reads freshness', () => {
    const stamp = buildFinalistsFreshnessStamp({ inputFingerprint: 'abc', lab: true });
    ...
    expect(formatFreshnessAge(stamp.lastSearchAt, Date.parse(stamp.lastSearchAt))).toBe('ahora');
