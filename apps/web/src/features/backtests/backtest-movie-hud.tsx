...
export function BacktestMovieHud({
  ...
  const unrealizedPct =
    lastIsOpen && stats.openEntry && markPrice != null && stats.openEntry.price > 0
  ...
  const positionChip = lastIsOpen && stats.openEntry ? (
    <StatChip
      label="Posición"
      value={
      ...
      tone={unrealizedPct == null ? 'live' : unrealizedPct >= 0 ? 'good' : 'bad'}
    />
  ) : (
  ...
  const chips = (
    <>
      ...
      {show('opsCount') && <StatChip label="Efectuadas" value={`${stats.opsCount}/${totalOps}`} />}
      {show('closedCount') && <StatChip label="Cerradas" value={String(stats.closedCount)} />}
      {show('closedNet') && (
        <StatChip
          label="Neto"
  ...
  const moneyWinPct = moneyTotal > 0 ? Math.round((stats.moneyWon / moneyTotal) * 100) : 0;
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="min-w-0">
          <p className="flex items-center gap-0.5 text-[10px] font-semibold uppercase tracking-[0.14em] text-muted-foreground">
            {playing ? 'Película en vivo' : atEnd ? 'Resultado completo' : 'Barrido'}
            {menu}
          </p>
          {show('date') && (
