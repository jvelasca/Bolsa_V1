...
export function formatPositiveFoldShare(share: number | null | undefined, nFolds?: number): string {
...
export function walkForwardStabilityFlags(input: {
  ...
  return {
    weakWfe: band === 'weak',
    unstableCv:
      input.oosCv != null && Number.isFinite(input.oosCv) && input.oosCv > OOS_CV_UNSTABLE,
    fewPositiveFolds:
