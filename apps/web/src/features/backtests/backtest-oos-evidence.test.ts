...
function runDetail(partial: Partial<BacktestRunDetailDto> = {}): BacktestRunDetailDto {
  return {
    id: 'run-1',
    instrumentId: 'inst-1',
    symbol: 'IBE.MC',
...
describe('OOS evidence for checklist / adopt', () => {
  ...
  it('reads hold-out blocks from a research trial', () => {
    const evidence = extractOosEvidenceFromTrial(
      trial({
        blocks: {
          oosMetrics: { score: 4.2, totalReturnPct: 3, maxDrawdownPct: 2, tradeCount: 3 },
        },
      }),
    );
  ...
  it('stashes and resolves evidence by strategy id', () => {
    ...
    expect(resolveOosEvidence({ trial: null, strategyId: 'missing' }).kind).toBe('none');
  });
  ...
  it('adopt stash keeps CPCV + PBO + Edge even when row has path oosMetrics', () => {
    ...
    expect(gate.checks.find((c) => c.id === 'oos_validation')?.status).toBe('pass');
    expect(gate.checks.find((c) => c.id === 'pbo')?.status).toBe('warn');
    expect(gate.checks.find((c) => c.id === 'edge_report')?.status).toBe('pass');
  });
