...
export type DiaDIdentityKind = 'same_id' | 'same_family' | 'different' | 'unknown';

export const DIA_D_RECONCILIATION_LABELS: Record<DiaDReconciliationCode, string> = {
...
export function buildCounterfactualOos(opts: {
  ...
  if (opts.status === 'unavailable' || opts.status === 'error' || opts.status === 'pending') {
    return {
...
export function buildDiaDReconciliation(
  ...
  const identity = resolveDiaDIdentity(input.experimentSlot, input.productionSlot);
  ...
  if (!input.experimentSlot || !band || band === 'incomplete' || identity === 'unknown') {
    code = 'INCONCLUSIVE';
  ...
  let cfBit = '';
  if (cf?.status === 'ready' && cf.returnPct != null && Number.isFinite(cf.returnPct)) {
    const d =
