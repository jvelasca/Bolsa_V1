...
export function BacktestStrategyMatrixPanel({
  ...
  const [layout, setLayout] = useState<StrategyMatrixColumnLayoutItem[]>(
    initial.columnLayout,
  );
  const [sortState, setSortState] = useState<StrategyMatrixSortState | null>(
    initial.sort,
  );
  const [favoriteColumnIds, setFavoriteColumnIds] = useState<
    StrategyMatrixColumnId[]
  >(initial.favoriteColumnIds);
  const [listHeightPxLocal, setListHeightPxLocal] = useState(
    initial.listHeightPx,
  );
  ...
  const [dropTargetId, setDropTargetId] =
    useState<StrategyMatrixColumnId | null>(null);
  ...
  const [carouselPrefs, setCarouselPrefs] =
    useState<StrategyMatrixFilterCarouselPrefs>(() =>
      loadStrategyMatrixFilterCarouselPrefs(),
    );
  ...
  const visibleColumns = useMemo(
    () => visibleStrategyMatrixColumns(layout),
    [layout],
  );
  ...
  const filtered = useMemo(
    () => filterStrategyMatrixRows(rows, filter),
    [rows, filter],
  );
  ...
  function onListResizeMouseDown(event: React.MouseEvent) {
    ...
    function onMouseMove(moveEvent: MouseEvent) {
      setListHeightPx(
        clampListHeightPx(
          listResizeRef.current.height +
            (moveEvent.clientY - listResizeRef.current.y),
        ),
      );
    }
    ...
    function onMouseUp(upEvent: MouseEvent) {
      const next = clampListHeightPx(
        listResizeRef.current.height +
          (upEvent.clientY - listResizeRef.current.y),
      );
      ...
      {menuOpen && (
        <OpaqueMenuPanel className="absolute right-0 top-full z-30 mt-1 min-w-[200px] p-2">
              ...
              return (
                ...
                >
                  <input
                    ...
                    onChange={() =>
                      persistLayout(
                        toggleStrategyMatrixColumn(layout, column.id),
                      )
                    }
                  />
          ...
          <p className="mt-1 border-t border-border px-2 pt-2 text-[10px] text-muted-foreground">
            ★ Favoritas en este dispositivo · Biblio / Borrar opcionales · Ver =
            detalle · Arrastra el borde inferior para altura
          </p>
        </OpaqueMenuPanel>
      )}
    </div>
  ...
  return (
    <div className="space-y-2 rounded-md border border-border/80 bg-muted/20 p-2">
      ...
      <div className="overflow-hidden rounded border border-border/60 bg-background">
        <div className="overflow-auto" style={{ height: listHeightPx }}>
                      ...
                      <>
                        <div
                          ...
                          onClick={() => {
                            if (!sortable) return;
                            persistSort(
                              cycleStrategyMatrixSort(sortState, column.id),
                            );
                          }}
                        >
                          {draggable && (
          ...
          {visible.length === 0 ? (
            ...
            <div className="divide-y divide-border/40 text-[11px]">
              {visible.map((row) => {
                ...
                const atCap =
                  !checked && selectedCount >= STRATEGY_MATRIX_MAX_SELECTED;
                return (
                  <div
                    key={row.rowId}
                    className={cn(
                    ...
                    onClick={(event) =>
                      handleRowPointerSelect(row.rowId, event)
                    }
                  >
                    <div className="flex items-center justify-center px-1.5">
                      <input
                        ...
                        onClick={(event) =>
                          handleCheckboxClick(row.rowId, event)
                        }
                        onChange={() => {
      ...
      {running ? (
        <div className="space-y-1.5 rounded-md border border-border/70 bg-background/80 px-2 py-2">
          ...
          <p className="text-[10px] leading-snug text-amber-700 dark:text-amber-400">
            Parar en cualquier momento: las pruebas no finalizadas no se guardan
            como resultado (quedan como canceladas). Las ya OK sí permanecen.
          </p>
        </div>
      ) : (
        <div className="space-y-1">
          ...
          <p className="text-[10px] leading-snug text-muted-foreground">
            ...
            {coachCount > 0 ? ` (${coachCount})` : ""}: si hay filas marcadas,
            solo esas; si no, todas las visibles. «Forzar re-sim» ignora el lote
            cacheado (útil con reutilización ON). Periodo/capital en Opciones
            avanzadas.
          </p>
        </div>
      )}
