    ...
    const findings = auditFindingsFromLlmPayload(
      {
        audit: {
          findings: [
            { strategyType: 'sma_crossover', action: 'confirm', reason: 'love it' },
            { strategyType: 'invented_alpha', action: 'veto', reason: 'halluc' },
            { strategyType: 'rsi_mean_reversion', action: 'veto', reason: 'ok' },
          ],
        },
      },
      'llm',
