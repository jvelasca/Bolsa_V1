...
export function buildAssistantStepPlan(
  ...
  if (step === 'universe') {
    ...
    if (prefs.semifinal.optimizeTop3OnEnter) {
      bullets.push('Encolar optimización de las 3 candidatas (hold-out/WF según barras)');
    } else {
      bullets.push('Solo revisar coach (sin encolar lab; marca el paso al confirmar)');
    }
    if (prefs.semifinal.autoAdvanceWhenDone) {
      bullets.push('Al terminar → te pedirá OK para Laboratorio (paso 3)');
    }
  } else if (step === 'lab') {
    bullets.push('Paso 3/4 — Laboratorio: adoptar Mejor solo si ≥ ancla (OOS/WF)');
    bullets.push('Abrir pestaña Optimizar con la semilla / jobs');
    if (prefs.lab.autoAdvanceWhenActiveTop) {
      bullets.push('Cuando el TOP pase a active → te pedirá OK para Finalistas (paso 4)');
    } else {
      bullets.push('Luego pulsa Finalistas manualmente');
    }
  } else {
    bullets.push('Paso 4/4 — Finalistas: Mis estrategias filtradas por TOP del valor');
  ...
  return {
    step,
    title: source === 'advance' ? `Siguiente · ${ordinal} ${label}` : `${ordinal} ${label}`,
    bullets,
    canExecute,
