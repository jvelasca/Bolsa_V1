...
function seed(partial: Partial<OptimizeSeed> & Pick<OptimizeSeed, 'strategyType'>): OptimizeSeed {
  return {
    instrumentId: 'inst',
    strategyLabel: 'Test',
    initialCash: 10_000,
...
describe('buildOptimizeRequestsFromSeed', () => {
  ...
  it('maps Donchian proxy to SMA H0+Optuna', () => {
    const bodies = buildOptimizeRequestsFromSeed(seed({ strategyType: 'donchian_breakout' }));
