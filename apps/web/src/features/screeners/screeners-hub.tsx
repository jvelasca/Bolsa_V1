...
export function ScreenersHub() {
  ...
  const patchScanConfig = useScreenerPreferencesStore(
    (state) => state.patchScanConfig,
  );
  const setScanConfig = useScreenerPreferencesStore(
    (state) => state.setScanConfig,
  );
  const runInBackground = useScreenerPreferencesStore(
    (state) => state.runInBackground,
  );
  const setRunInBackground = useScreenerPreferencesStore(
    (state) => state.setRunInBackground,
  );
  ...
  const mobileView = useScreenerPreferencesStore(
    (state) => state.layout.mobileView,
  );
  const setMobileView = useScreenerPreferencesStore(
    (state) => state.setMobileView,
  );
  const panelOpenState = useScreenerPreferencesStore(
  ...
  const [activeDefaultPolicyId, setActiveDefaultPolicyId] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    if (
      !listIdFromUrl ||
      !listsQuery.isSuccess ||
      appliedListIdFromUrlRef.current
    )
      return;
  ...
  const setSyncScanActive = useScreenerActivityStore(
    (state) => state.setSyncScanActive,
  );
  useEffect(() => {
  ...
  const resultsPanel = lastResult ? (
    <Card className="min-w-0">
      <CardHeader className="pb-3">
        ...
        <CardDescription>
          Señales en cierre de barra · TF {lastResult.timeframe} ·{" "}
          {lastResult.hitCount} coincidencias
        </CardDescription>
      </CardHeader>
    ...
    <div className="rounded-lg border border-dashed border-border px-4 py-8 text-center text-sm text-muted-foreground">
      Ejecuta un rastreo para ver coincidencias. Clic en símbolo o «Gráfico»
      abre Trading con contexto de lista.
    </div>
  );
      ...
      {recentJobs.length > 0 && (
        ...
        >
          <ScanJobsPanel
            embedded
            jobs={recentJobs}
            onLoadResult={handleLoadJobResult}
          />
        </ScreenerCollapsibleSection>
      )}
            ...
            {history.map((entry) => (
              ...
              >
                <span className="text-muted-foreground">
                  {new Date(entry.runAt).toLocaleTimeString("es-ES")} ·{" "}
                  {entry.result.hitCount} coincidencias
                </span>
                <Button
                  type="button"
                  size="sm"
    ...
    <div className="screener-hub-shell mx-auto flex h-full min-h-0 w-full max-w-[1600px] flex-col gap-3">
      ...
      {isSplitViewport && (
        <p className="hidden shrink-0 text-[10px] text-muted-foreground lg:block">
          Arrastra los separadores entre paneles para ajustar el layout. Se
