...
const MODE_OPTIONS: PositionExecutionMode[] = [
  "manual",
  "exit_strategy",
  "full_auto",
];
...
export function PositionPoliciesPanel({
  embedded,
}: { embedded?: boolean } = {}) {
  const queryClient = useQueryClient();
  const accountId = useActiveAccountQueryKey();
  const accountScope = accountId;
  const positionPanel = useScreenerPreferencesStore(
    (state) => state.positionPanel,
  );
  const patchPositionPanel = useScreenerPreferencesStore(
    (state) => state.patchPositionPanel,
  );
  ...
  const [lastEvalResults, setLastEvalResults] = useState<
    PositionExitEvalResultDto[]
  >([]);
  const { mode, executeTrades, exitStrategyId, executionPolicyId } =
    positionPanel;
  ...
  const availablePositions = positions.filter(
    (p) => !policyInstrumentIds.has(p.instrumentId),
  );
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      api.createPositionPolicy({
        ...
        executionPolicyId:
          mode === "full_auto" ? executionPolicyId || null : null,
      }),
    onSuccess: () => {
      setShowCreate(false);
      setInstrumentId("");
    ...
    return (
      ...
      >
        <p className="text-sm text-muted-foreground">
          Selecciona una cuenta activa para gestionar políticas de salida
          (P6/P7).
        </p>
      </ScreenerPanelShell>
    );
  ...
  return (
    <ScreenerPanelShell
      ...
      description={
        embedded
          ? undefined
          : "Política por posición abierta — evaluar salidas con rules engine P7."
      }
      icon={LogOut}
    >
      <div className="flex flex-wrap gap-2">
        ...
        <label className="flex items-center gap-1.5 text-xs text-muted-foreground">
          <input
            ...
            onChange={(e) =>
              patchPositionPanel({ executeTrades: e.target.checked })
            }
          />
          Ejecutar trades
        </label>
        ...
        <div className="space-y-3 rounded-lg border border-border bg-muted/30 p-3">
          ...
          <label className="block text-sm">
            ...
            <select
              ...
              onChange={(e) =>
                patchPositionPanel({
                  mode: e.target.value as PositionExecutionMode,
                })
              }
              className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
            >
          ...
          {mode === "exit_strategy" && (
            <label className="block text-sm">
              ...
              <select
                ...
                onChange={(e) =>
                  patchPositionPanel({ exitStrategyId: e.target.value })
                }
                className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
              >
                <option value="">Selecciona…</option>
          ...
          {mode === "full_auto" && (
            <label className="block text-sm">
              ...
              <select
                ...
                onChange={(e) =>
                  patchPositionPanel({ executionPolicyId: e.target.value })
                }
                className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
              >
                <option value="">Selecciona…</option>
      ...
      {policiesQuery.isLoading ? (
        ...
        <p className="text-sm text-muted-foreground">
          Sin políticas. Crea una para posiciones abiertas con salida
          automática.
        </p>
      ) : (
            ...
            >
              <div>
                <p className="font-medium text-sm">
                  {symbolByInstrumentId[policy.instrumentId] ??
                    policy.instrumentId.slice(0, 8)}
                </p>
