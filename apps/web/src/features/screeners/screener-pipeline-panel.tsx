  ...
  return (
    ...
    >
      <div className="space-y-2 text-xs text-muted-foreground">
        <p>
          <strong className="text-foreground">Cache</strong> — features por
          IndicatorSpec
        </p>
        <p>
        ...
        <p>
          <strong className="text-foreground">Programación</strong> — worker en
          API (cierre 1d/1wk)
        </p>
        {jobMeta?.cacheHits != null && (
          <p className="border-t border-border pt-2">
            Última tarea: caché {jobMeta.cacheHits} aciertos /{" "}
            {jobMeta.cacheMisses} fallos
          </p>
        )}
        ...
        <div className="border-t border-border pt-2">
          ...
          >
            {evaluateSchedulesMutation.isPending
              ? "Evaluando…"
              : "Evaluar programaciones P9"}
          </Button>
          {evaluateSchedulesMutation.data && (
            <p className="mt-1 text-xs text-muted-foreground">
              {evaluateSchedulesMutation.data.data.enqueuedCount} tareas
              encoladas de {evaluateSchedulesMutation.data.data.checkedCount}{" "}
              rastreadores
            </p>
          )}
        </div>
