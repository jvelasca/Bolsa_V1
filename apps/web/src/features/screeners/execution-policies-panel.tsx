...
const MODE_OPTIONS: ExecutionMode[] = [
  "inform_only",
  "alert",
  "paper_auto",
  "live_auto",
];
const KIND_OPTIONS: SignalKind[] = [
  "entry_long",
  "exit",
  "entry_short",
  "watch",
...
export function ExecutionPoliciesPanel({
  embedded,
}: { embedded?: boolean } = {}) {
  ...
  const [signalKinds, setSignalKinds] = useState<SignalKind[]>([
    "entry_long",
    "exit",
  ]);
  ...
  const [requireValidatedBacktest, setRequireValidatedBacktest] =
    useState(false);
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      api.createExecutionPolicy({
        ...
        webhookUrl:
          mode === "alert" && channels.includes("webhook")
            ? webhookUrl || null
            : null,
        emailTo:
          mode === "alert" && channels.includes("email")
            ? emailTo || null
            : null,
    ...
    >
      ...
      {policiesQuery.isLoading ? (
        ...
        <p className="text-sm text-muted-foreground">
          Sin políticas. Crea una para ejecutar coincidencias desde resultados
          de rastreo.
        </p>
      ) : (
            ...
            >
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {MODE_LABELS[policy.mode]} · {policy.signalKinds.length}{" "}
                  señales
                  {policy.requireValidatedBacktest
                    ? " · backtest requerido"
                    : ""}
                </p>
              </div>
