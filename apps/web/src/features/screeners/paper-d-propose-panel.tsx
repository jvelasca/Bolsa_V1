...
export function PaperDProposePanel({
  listId,
  lists,
  onListIdChange,
  className,
  ...
  const eligible =
    result?.candidates.filter((c) => c.status === "eligible") ?? [];
    ...
    <div className={cn("space-y-3", className)}>
      <div className="flex flex-wrap items-end gap-2">
        ...
        <Button
          ...
          disabled={!listId || mutation.isPending || (wantExecute && !policyId)}
          onClick={() => mutation.mutate()}
        >
          <Route
            className={cn("h-3.5 w-3.5", mutation.isPending && "animate-pulse")}
          />
          {mutation.isPending
            ? "…"
            : wantExecute
        ...
        <div className="space-y-2">
          ...
          {eligible.length === 0 ? (
            <p className="text-[11px] text-muted-foreground">
              Ningún elegible con el umbral actual.
            </p>
          ) : (
