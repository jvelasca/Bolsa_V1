...
export function TrackersPanel({
  ...
  const patchTrackerSave = useScreenerPreferencesStore(
    (state) => state.patchTrackerSave,
  );
  ...
  const [editScheduleKind, setEditScheduleKind] =
    useState<TrackerScheduleKind>("manual");
  ...
  useEffect(() => {
    ...
    void api.getTracker(initialTrackerId).then((response) => {
      ...
      onLoadConfig(loaded, {
        defaultPolicyId:
          response.data.definition.defaultExecutionPolicyId ?? null,
      });
      const label = trackerScheduleLabel(response.data);
      if (label) {
        setScheduleLabels((current) => ({
          ...current,
          [initialTrackerId]: label,
        }));
      }
    });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      let strategyId =
        config.scanSource === "saved" ? config.savedStrategyId : "";
  ...
  function handleLoad(trackerId: string) {
    void api.getTracker(trackerId).then(async (response) => {
      const tracker = response.data;
      const strategyResponse = await api.getStrategy(
        tracker.strategyDefinitionId,
      );
      onLoadConfig(
        scanConfigFromStrategyDefinition(
          strategyResponse.data,
          scanConfigFromTracker(tracker),
        ),
        {
  ...
  return (
    <ScreenerPanelShell
      ...
      description={
        embedded
          ? undefined
          : "Persistidos en BD — schedule automático vía worker P9 (API en marcha)."
      }
      icon={Radar}
    >
        ...
        {activeTrackerId && canRunScan(config) && (
          ...
          >
            {updateMutation.isPending
              ? "Guardando…"
              : "Sincronizar rastreo → rastreador"}
          </Button>
        )}
      </div>
        ...
        <div className="space-y-3 rounded-lg border border-border bg-muted/30 p-3">
          ...
          <label className="block text-sm">
            ...
            <select
              ...
              onChange={(e) =>
                patchTrackerSave({
                  scheduleKind: e.target.value as TrackerScheduleKind,
                })
              }
              className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
            >
          ...
          {executionPolicies.length > 0 && (
            <label className="block text-sm">
              ...
              <span className="mt-0.5 block text-[11px] text-muted-foreground">
                B1: modos «Solo informar» / «Alerta» se aplican solos tras cada
                scan. Paper auto no.
              </span>
              <select
                value={defaultPolicyId}
                onChange={(e) =>
                  patchTrackerSave({ defaultPolicyId: e.target.value })
                }
                className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
              >
          ...
          {trackers.map((tracker) => {
            ...
            const isRunning =
              ...
              (enqueueMutation.isPending &&
                enqueueMutation.variables === tracker.id);
                ...
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      ...
                      <span
                        className={
                          tracker.enabled ? "text-foreground" : "text-amber-600"
                        }
                      >
                        {tracker.enabled ? "activo" : "pausado"}
                      </span>
                      {scheduleLabels[tracker.id] && (
                        <> · {scheduleLabels[tracker.id]}</>
                      )}
                    </p>
                  </div>
                  <div className="flex flex-wrap gap-1">
                    ...
                    >
                      ...
                      runMutation.variables === tracker.id ? (
                  ...
                  <div className="mt-3 space-y-3 border-t border-border pt-3">
                    ...
                    <label className="block text-sm">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setEditScheduleKind(
                            e.target.value as TrackerScheduleKind,
                          )
                        }
                        className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm"
                      >
                        <option value="manual">Manual</option>
                        <option value="on_bar_close">
                          Automático · cierre de barra
                        </option>
                      </select>
                    </label>
                    {executionPolicies.length > 0 && (
                    ...
                    <div className="flex flex-wrap gap-2">
                      <Button
                        ...
                        onClick={() =>
                          updateMutation.mutate({ trackerId: tracker.id })
                        }
                      >
                        {updateMutation.isPending
                          ? "Guardando…"
