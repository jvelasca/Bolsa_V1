...
export function ScreenerWorkflowSteps({
  ...
  return (
    <ol className="flex flex-wrap items-center gap-1.5 sm:gap-2" aria-label="Flujo del rastreador">
      {STEPS.map((step, index) => {
        const isActive = step.id === activeStep;
        const isDone = index < activeIdx;
        const showHits = step.id === 'results' && hitCount != null && hitCount > 0;
          ...
          <li key={step.id} className="flex items-center gap-1.5 sm:gap-2">
            ...
            <span
              className={cn(
                ...
                isDone && !isActive && 'border-border bg-muted/30 text-foreground',
