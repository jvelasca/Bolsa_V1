...
} from "./instrument-daily-opinion.js";
...
export function buildOpinionChannelItems(
  ...
  for (const { opinion, symbol } of rows) {
    ...
    out.push({
      ...
      stance: opinion.stance,
      dictamenStars: opinion.dictamenStars,
      strategyStars: opinion.strategyStars ?? null,
      ioScore: opinion.ioScore ?? null,
  ...
  const rank = (level: OpinionChannelItemV1["level"]) =>
    level === "alarma" ? 0 : 1;
