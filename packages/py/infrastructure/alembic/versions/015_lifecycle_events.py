...
Tabla ausente → create. Tabla presente → ensure índices (no return temprano).
"""
...
_EVENT_ID_KEY = "lifecycle_events_event_id_key"
...
def _ensure_indexes(bind: sa.engine.Connection) -> None:
    if not _index_exists(bind, _ACC_IDX):
        op.create_index(_ACC_IDX, _TABLE, ["account_id"])
    if not _index_exists(bind, _POS_IDX):
        op.create_index(_POS_IDX, _TABLE, ["position_id", "at"])
    if not _index_exists(bind, _FILL_UIDX):
        op.create_index(
            _FILL_UIDX,
            _TABLE,
            ["fill_id"],
    ...
    if not _constraint_exists(bind, _EVENT_ID_KEY) and not _index_exists(
        bind, _EVENT_ID_KEY
    ):
        op.create_unique_constraint(_EVENT_ID_KEY, _TABLE, ["event_id"])
...
def upgrade() -> None:
    ...
    if not _table_exists(bind, _TABLE):
        op.create_table(
            ...
            sa.UniqueConstraint("event_id", name=_EVENT_ID_KEY),
        )
