...
def extract_session_protect_plan(
    payload: dict[str, Any] | None,
) -> dict[str, Any] | None:
    """Lee ``runtime.protectPlan`` (Ciclo 5.1 advisory). Sin inventar."""
    runtime = _session_runtime(payload)
    if runtime is None:
        return None
    raw = runtime.get("protectPlan")
    if not isinstance(raw, dict):
        raw = runtime.get("protect_plan")
    if isinstance(raw, dict) and raw:
        return dict(raw)
    return None
...
def extract_session_expectancy(
    payload: dict[str, Any] | None,
) -> dict[str, Any] | None:
    """Lee ``runtime.expectancy`` (Ciclo 8.0 advisory). Sin inventar."""
    runtime = _session_runtime(payload)
    if runtime is None:
        return None
    raw = runtime.get("expectancy")
    if isinstance(raw, dict) and raw:
        return dict(raw)
    return None
