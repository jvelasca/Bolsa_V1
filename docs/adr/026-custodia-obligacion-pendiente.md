...
3. **Política de cargo `custody_charge_source = DEFAULT_PORTFOLIO`** (R-11 C6, hallazgo #2 de la auditoría externa): la custodia es una obligación de la **cuenta** — el importe se calcula sobre el **`total_equity` agregado** de todas las carteras de la cuenta — pero el cobro se hace **exclusivamente desde la cartera seleccionada/default** (`scope.portfolio`; fallback la cartera `is_default`). **No** hay transferencia implícita entre carteras. Es únicamente **documentación de la regla**, sin cambio de comportamiento frente a v1.2.1/`b28e956`. _(Decidido 2026-08-21.)_
...
| 2026-08-21 | R-11 C1/Custodia multi-periodo: PK `id` autoincrement + `UNIQUE(account_id, period)` (migración `006`), `created_at`/`updated_at`; `upsert` corregido para no sobrescribir; `ApplyCustodyFees`/`RunCustodyJob` liquidan primero el PENDING más antiguo. **R-11 C6 (2026-08-21):** política de cargo **`custody_charge_source = DEFAULT_PORTFOLIO`** documentada (importe sobre equity agregado, cobro desde la cartera seleccionada/default, sin transferencia inter-cartera) — solo documenta la regla, sin cambio de comportamiento. |
