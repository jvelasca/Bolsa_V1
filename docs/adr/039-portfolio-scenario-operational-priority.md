...
- Opportunity Engine — **V1.19 Discovery:** `OpportunityFunnelV1` + `OpportunityRankingV1` ([`opportunity-ranking.ts`](../../packages/shared/src/cognitive/opportunity-ranking.ts)); Quality pura cableada a Priority (pesos 35/35/30); Mesa L3 ranking ≠ Action Queue; scan diario opt-in (`OPPORTUNITY_DAILY_SCAN_ENABLED`); Decision Board **no** es screener. **Aún pendiente V1.20+:** OpportunityScore multiplicativo, Actionability v1, endpoint persistido.
