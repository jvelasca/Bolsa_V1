...
> **AsOf:** 2026-08-25 · **ADR-031** tesis ≠ plan ≠ permiso. HEAD local **`a4762e8`** (I2 cerrado; origin aún `05e354c`). Relevo vivo: [`traspaso-relevo-ciclo-i3-shadow-honesty-2026-08-25.md`](./engineering/traspaso-relevo-ciclo-i3-shadow-honesty-2026-08-25.md) (integridad I3; D1–D8 OK; stamp SHA pendiente; **no thaw**). Alembic `010` en `bolsa_v1`.
...
- **Ciclo I3:** HTTP `paper_auto` (`/execution-policies/{id}/route`, `/scans/jobs/{id}/execute`) exige `PAPER_D_EXECUTE` (403 `paper_auto_env_blocked` si off). `inform`/`alert`/`live_auto` no. Router no fusionado. **No thaw.**
...
- Diferido ADR-031: Alembic/tabla Wyckoff / `wyckoffPhase` contrato (parked), trailing continuo / bracket / T1 parcial fill, thesis health **plena** (persistencia Confidence lifecycle), expectancy **plena**, Shadow AUTO **thaw** (ADR-023 Proposed), broker. **I1** `2bd5cd8` · **I2** `e31840d` · **I3** HTTP paper_auto gated (stamp SHA pendiente). **No** reabrir Wyckoff thin por defecto.
