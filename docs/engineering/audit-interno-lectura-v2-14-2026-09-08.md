...
- **Hallazgo P1-P2 en aislamiento de cuenta — remediado en las vías de EJECUCIÓN (2026-09-08) Y de LECTURA/estudio (2026-09-08).** Un grupo de endpoints "operador/automación/IA" leía `account_id` de query/body **sin** `require_account_access`, con el dominio (`resolve_scope`) sin re-filtrar por owner. Ya se han aplicado los gates `require_account_access` a las 4 rutas que ejecutan (confirm / evaluate-exits / execute-auto / paper-desk cycle) y `require_owned_account_if_present` a las de lectura/estudio que declaran `account_id`. Ver A1 en la tabla (estado **[cerrado]**). Condición cross-account multi-owner sigue `[runtime-aislamiento]` (hoy single-owner bootstrap).
...
| --- | ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A1  | **P1-condicional** | Endpoints operator/auto/IA leen `account_id` sin gate + dominio confía en gate HTTP (cross-account si existiera 2º owner real)                    | ejecución gateada: `ai_governance.py`(confirm), `position_policies.py`(evaluate-exits), `position_automation.py`(execute-auto), `paper_desk.py`(cycle); residual lectura/estudio: effectiveness/decision-sessions/memory/trials/edge-reports/propose/daily-report/instrument_daily_opinions | **[cerrado]** gates `require_account_access` en rutas de EJECUCIÓN (2026-09-08) + `require_owned_account_if_present` en rutas de LECTURA/estudio que declaran `account_id` (2026-09-08). Condición multi-owner `[runtime-aislamiento]` sigue sin demostrarse por runtime (single-owner bootstrap). |
...
| Aislamiento de cuenta                | 9.5/10                                              | gates ejecución + lectura/estudio aplicados (2026-09-08); cross-account demostrable solo con ≥2º owner real `[runtime-aislamiento]` |
