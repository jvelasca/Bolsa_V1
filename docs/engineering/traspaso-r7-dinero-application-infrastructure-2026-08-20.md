...
## 4l. Fase B-4 corregida — `3712e63` `fix(py-application): trade+fee idempotente en AUTO execute y confirm intent (R-7/B-4, L-M6)`

> Decisión de usuario: **Opción B** — derivar/reenviar una `idempotency_key` lógica estable en los dos caminos internos de producción para que el guard DB ya existente de `ExecuteTrade` (find_transaction_by_idempotency + UNIQUE `transactions`) se active. Defensivo, sin romper callers/API, sin `contract:gen`.

### B-4 (L-M6) — doble trade+fee sin `idempotency_key`

- **Mapeo read-only (verificado por el coordinador):** en `ExecuteTrade.execute` (`accounts.py:560-636`) la 3ª llamada `append_fee` (`accounts.py:626`) NO es "no atómica" en el sentido de transacción DB: las 3 llamadas (`execute_trade :592`, `append_trade :605`, `append_fee :626`) comparten la MISMA `AsyncSession` (`dependencies.py:413-418`, `:270-280`), se hacen flush dentro de la misma transacción y se commitean JUNTAS al final del request (`dependencies.py:277`); ante excepción se rollback todo (`:278-280`). El UNIQUE de F3 (`(account_id, reference_type, reference_id, type)`) **no toca `transaction`**: trade+fee se siguen escribiendo igual, solo difieren en `type` (como se diseñó).
- **Riesgo dinero real VIGENTE (el haz de B-4):** los DOS caminos de producción de trades no pasaban `idempotency_key` (`execution_router.py:611` AUTO paper_auto, `confirm_recommendation.py:78` confirm intent), así que el guard `find_transaction_by_idempotency` (`accounts.py:574-581`) NUNCA se activaba. Un repost/retry (doble POST, retry tras timeout, job auto re-despachado tras expirar Redis/TTL) escribía un `new_id()` fresco → trade+fee 2× → dinero movido 2×. Ni el UNIQUE de F3 ni el de `transactions(portfolio_id, idempotency_key)` (`tables.py:170`, con `idempotency_key` nullable `:181`) lo frenaban.
- **Fix (Opción B, application-only sin migración):**
  - **`execution_router.py` (AUTO paper_auto):** se pasa `idempotency_key=idem_key` al `ExecuteTrade.execute`. `idem_key` ya era `make_auto_execute_idempotency_key(...)` (día×política×kind, `auto_execute_idempotency.py:11-19`) usado para el claim Redis/TTL (`claim_auto_execute_idempotency`). Ahora el dedup AUTO (Redis, TTL) se **alinea con el guard DB persistente** (`transactions` UNIQUE) → un repost tras expirar el TTL ya no duplica trade+fee, porque el guard retorna la transacción original. Se movió el cálculo de `idem_key` a nivel de función (antes estaba dentro del `if self._enforce_cognitive_gate:`) para que el guard se active siempre, y se inicializa `claimed=False` al mismo nivel para que el `release_auto_execute_idempotency(idem_key)` del `except ValueError` no falle con `NameError` cuando el gate está desactivado (era un `NameError` latente pre-existente).
  - **`confirm_recommendation.py` (confirm intent):** se pasa `idempotency_key=rec.decision_id or session_id` — la identidad lógica de la decisión. Un doble confirm de la MISMA decisión rejuega el trade original en vez de duplicarlo.
  - **API `POST /portfolio/trade` (`portfolio.py:96-110`):** ya soporta `idempotency_key` en `TradeRequestDto` (el guard se documenta como el mecanismo del path manual). Sin cambios de DTO/ruta → sin `contract:gen`.
  - **Sin falsos positivos:** la clave identifica la _ejecución lógica_ (señal día×política×kind / decisión), NO el payload del trade → no colapsa dos trades legítimos idénticos.
- **Tests** (`packages/py/application/tests/test_execute_trade_idempotency.py`, 4, fakes en memoria, patrón `test_deposit_withdraw_idempotency`):
  1. Misma `idempotency_key` → replay de la transacción original, trade+fee NO duplicados (executions=1, ledger=2 filas `buy`+`fee`).
  2. Claves distintas (trades distintos) → ambos ejecutan (sin falso positivo; executions=2, ledger=4).
  3. Sin clave, cada llamada ejecuta (escotilla documentada para un futuro caller que omita el key).
  4. `ConfirmRecommendationIntent` reenvía `decision_id` como `idempotency_key` al trade.
- **Scope/decisiones NO tocadas:** no se reabren M-3 (puente fee-excluido), B-1/B-3/M-6/M-7; no migración; no repos DB; no API/FE. Sin `regen_full`, sin `contract:gen`.

### Batería B-4 (verde, verificada por el coordinador)

| Comprobación                                                                       | Resultado                                                                                   |
| ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------- |
| `ruff check packages/py apps/api-python --config pyproject.toml` (3 files alcance) | ✅ 0 (full scope)                                                                           |
| `mypy confirm_recommendation.py` (config raíz)                                     | ✅ Success                                                                                  |
| `mypy execution_router.py` (config raíz)                                           | ✅ solo `no-untyped-def` pre-existente `ExecuteScanJobHits.__init__` (app mypy-blind en CI) |
| pytest application (fakes, incl. 4 B-4 nuevos)                                     | ✅ 249 passed (245 + 4)                                                                     |
| pytest api-python accounts integration (endpoint summary intacto)                  | ✅ 5 passed                                                                                 |
| `git commit` + `git status`                                                        | ✅ solo 2 modified + 1 nuevo en `packages/py/application`                                   |

---

## 4m. Fase B-5 corregida — `76abe0f` `fix(py-domain): guard quantity==0 en FIFO buy + observabilidad fail-open PnL CORE-R (R-7/B-5, T-M9/T-M10)`

### T-M9 (domain, verdad) — FIFO divide sin guard `quantity==0`

- **Hallazgo read-only:** `_fifo_realized` (`packages/py/domain/src/bolsa_domain/tax_report.py:100`) computaba `unit_cost = (tx.total + fee) / tx.quantity` en la rama `buy` **sin guard** `quantity > 0`. Un buy de `quantity == 0` → `ZeroDivisionError`. Verificado: el write-path `execute_trade` (`portfolio_repository.py:263: "La cantidad debe ser mayor que cero"`) ya rechaza `quantity <= 0` antes de insertar en `TransactionRow`, y la migración `account_migration.py:225-243` escribe solo en `LedgerEntryRow` (no en `TransactionRow`), así que **hoy NO hay camino real** de `quantity==0` al FIFO → es defensa en profundidad (latente).
- **Decisión usuario:** añadir el guard (freeze-compatible, no es feature).
- **Fix:** en la rama `buy`, el lote solo se apendiza si `tx.quantity > 1e-9` (mismo umbral del resto del archivo). Un buy de quantity 0 hace `continue` sin fabricar lote → por construcción no hay división por cero ni lot de cost 0 que contamine basis/residual. Cero cambio para quantity > 0.
- **Test** `test_fifo_buy_quantity_zero_no_divide_by_zero_ni_lote` (`test_tax_report_fiscal.py`): buy qty==0 + buy válido + sell → no `ZeroDivisionError`, el sell consume SOLO el lote válido (`cost_basis = 5*100`), `realized_gain == (120-100)*5`.

### T-M10 (application, verdad) — fail-open silencioso del PnL CORE-R

- **Hallazgo read-only:** `fetch_core_r_pnl_extra_rows` (`fetch_core_r_pnl_extra_rows.py:53-118`) atribuye a CADA instrumento el `return_pct` = `account_return_pct(initialDeposit, totalEquity)` del equity **whole-account** de la cuenta paper vinculada a su top (no del instrumento). **Semántica confirmada INTENCIONAL**: espejada en el FE `apps/web/src/features/backtests/core-r-scheduler-tick.ts:40` (`fetchPnlExtraRows`, misma estructura `row.paperAccount` → `coreRAccountReturnPct(totalEquity)`). El veredicto de revisión CORE-R es un juicio de la cuenta/estrategia que ejecuta el top, no del instrumento → **NO se toca la semántica**.
- **Decisión usuario:** corregir el fail-open solo (menor superficie), sin cambiar instrumento→cuenta.
- **Fix:** se añaden 2 `logger.warning` para los skips **silenciosos** que degradaban la cola sin señal: (a) un top con `strategy_ids` no vacíos pero **sin cuenta paper vinculada** (`find_paper_for_top_slots → None`); (b) un paper con **summary live ausente** (`live is None`). El skip por `return_pct is None` (initialDeposit ≤ 0), el `return []` de lista no-existente/vacía y el `except Exception` superior (ya `logger.exception`) se dejan como están (best-effort intencional). Sin cambio de retorno.

### Batería B-5 (verde, verificada por el coordinador)

| Comprobación                                                                           | Resultado                                     |
| -------------------------------------------------------------------------------------- | --------------------------------------------- |
| `ruff check packages/py apps/api-python --config pyproject.toml` (full)                | ✅ 0                                          |
| `mypy tax_report.py` + `test_tax_report_fiscal.py` (config raíz)                       | ✅ Success (0)                                |
| pytest domain FIFO (`test_tax_report_fiscal.py` + `test_tax_report_open_positions.py`) | ✅ 13 passed (incl. nuevo)                    |
| pytest application (fakes)                                                             | ✅ 249 passed                                 |
| `git status --short`                                                                   | ✅ solo 2 modified + 1 nuevo en `packages/py` |

---

## 4n. Fase B-2 corregida — `7b3dd91` `fix(py-domain): total_unrealized_gain fail-closed con posiciones sin precio (R-7/B-2, T-M8)`

> Decisión de usuario: **Opción A (fail-closed)** — si alguna posición abierta del tax-report no tiene precio de mercado (`unrealized_gain None`), `total_unrealized_gain` pasa a `None` en vez de silenciar esa posición a 0. Coherente con la semántica "sin valor observable" de M-1.

### T-M8 (domain, verdad) — `total_unrealized_gain` silencia a 0 las posiciones sin precio

- **Hallazgo read-only (verificado):** en `build_tax_report` (`packages/py/domain/src/bolsa_domain/tax_report.py:368-372`) el total se computaba como `sum(line.unrealized_gain or 0.0 ...)`. `unrealized_gain` es `None` (`tax_report.py:262`) exactamente cuando `market_value is None`, que ocurre cuando `price is None` (`tax_report.py:259-261`) — es decir, cuando la posición abierta **no tiene precio de mercado**. Esa posición se silenciaba a 0 en `total_unrealized_gain`, infra-representando y haciendo que el FE la mostrara como un valor "conocido" (subestimado) en vez de "no disponible".
- **NO confundir con M-1** (cerrada): M-1 era el fallback mark-to-cost en `get_summary` (infrastructure, `total_market_value`/`total_equity` del RESUMEN). B-2 es la cara unrealized del **tax-report** (domain), poblada por `open_positions_with_fee_basis` (puente M-3).
- **Fix (Opción A, domain-only):** en `build_tax_report`, si `unrealized_lines` y **alguna** línea tiene `unrealized_gain is None`, `total_unrealized_gain = None`. Caso sin posiciones → `None` (respeta el shape `float | None`). El DTO/API/FE no cambian de shape (sin `contract:gen`).
- **Consumidores de `total_unrealized_gain` (mapeo verificado):**
  - **API:** `apps/api-python/src/bolsa_api/schemas/account_mappers.py:219` → schema `accounts.py:233` (`totalUnrealizedGain: float | None`).
  - **FE:** `apps/web/src/features/fiscal/tax-report-page.tsx:332-345` — tarjeta "P&L no realizado" solo renderiza si no-null. Con B-2, si una posición abierta no tiene precio, el total pasa a "—" en lugar de un 0 infra-representado.
  - **Job/analytics:** NO consumen el gain del tax-report (los jobs CORE-R consumen `totalUnrealizedPnl` del **summary**, `fetch_core_r_pnl_extra_rows.py:80`). Sin impacto.
- **Files:** `tax_report.py` · nuevo test en `test_tax_report_open_positions.py` (+3: una sin precio → total None / todas con precio → suma real 245 / mixto → total None). No se tocó application/infra/API/FE.

### Batería B-2 (verde, verificada por el coordinador)

| Comprobación                                                                       | Resultado                 |
| ---------------------------------------------------------------------------------- | ------------------------- |
| `ruff check packages/py apps/api-python --config pyproject.toml` (2 files alcance) | ✅ 0                      |
| `mypy packages/py/domain/src` (gate CI, 44 files)                                  | ✅ Success (0)            |
| pytest domain full (incl. 3 nuevos)                                                | ✅ 25 passed              |
| pytest application (fakes)                                                         | ✅ 249 passed             |
| `git status`                                                                       | ✅ solo files del alcance |

> **Nota mypy:** `test_tax_report_open_positions.py` conserva **2 errores mypy pre-existentes** (`:104, :146`, `realized[0].proceeds + line.market_value`, tests M-3 originales) — **fuera del gate CI** (CI corre mypy solo sobre `*/src`). El código nuevo de B-2 es mypy-limpio.
...
| B-2 (T-M8)       | domain      | `total_unrealized_gain` suma `unrealized_gain or 0.0` → posiciones sin precio se silencian a 0 en el total.                                                                                                                                                                                                  | verdad        | ✅ CORREGIDO (B-2, ver §4n, `7b3dd91`) — opción A fail-closed                                                                                                                      |
...
| B-4 (L-M6)       | application | fee ledger escrita en 2ª llamada NO atómica con el trade (aunque comparten la misma `AsyncSession`/`commit` final, por lo que la atomicidad DB SÍ se cumple); el app guard solo se activa si se pasa `idempotency_key` (nada lo exigía en AUTO/confirm). Riesgo real: doble trade+fee sin `idempotency_key`. | dinero        | ✅ CORREGIDO (B-4, ver §4l, `3712e63`) — opción B                                                                                                                                  |
| B-5 (T-M9/T-M10) | application | FIFO divide sin guard `quantity==0` (latente, repo lo rechaza); `fetch_core_r_pnl_extra_rows` atribuye el PnL whole-account a un instrumento (fail-open, fallback a `list[]`).                                                                                                                               | verdad        | ✅ CORREGIDO (B-5, ver §4m, `76abe0f`) — guard `quantity==0` en FIFO buy + `logger.warning` en skips silenciosos PnL CORE-R (semántica instrumento→cuenta intencional, NO se toca) |
...
4. ~~**B-2 (T-M8):** `total_unrealized_gain` silencia a 0 las posiciones sin precio. _(NO confundir con M-1, ya cerrada.)_~~ — **✅ CERRADA** (`7b3dd91`, ver §4n): opción A fail-closed — si una posición abierta no tiene precio (`unrealized_gain None`) el total pasa a `None` (FE lo muestra "—", no un 0 infra-representado). Coherente con M-1. **R-7 deuda completa.**
...
6. ~~**B-4 (L-M6):** fee ledger escrita en 2ª llamada no atómica con el trade~~ — **✅ CERRADA** (`3712e63`, ver §4l): opción B, `idempotency_key` estable en AUTO execute y confirm intent (activando el guard DB). La atomicidad DB ya se cumplía (misma `AsyncSession`/commit único); el haz real era el doble trade+fee sin clave.
7. ~~**B-5 (T-M9/T-M10):** FIFO divide sin guard `quantity==0`; `fetch_core_r_pnl_extra_rows` atribuye el PnL whole-account a un instrumento~~ — **✅ CERRADA** (`76abe0f`, ver §4m): guard `quantity==0` en FIFO buy (latente) + `logger.warning` en skips silenciosos PnL CORE-R (semántica instrumento→cuenta intencional, NO se toca).
...
| B-4/L-M6: `ruff` config CI raíz (3 files) · `mypy confirm_recommendation` · app 249 (4 nuevos) · api-python accounts 5 | ✅ 0 / Success / 249 / 5 (ver §4l)                                                                           |
| B-5/T-M9-TM10: `ruff` config CI raíz · `mypy tax_report+test` · domain FIFO 13 (nuevo) · app 249                       | ✅ 0 / Success / 13 / 249 (ver §4m)                                                                          |
| B-2/T-M8: `ruff` config CI raíz (2 files) · `mypy domain/src` (gate) · domain 25 (3 nuevos) · app 249                  | ✅ 0 / Success / 25 / 249 (ver §4n)                                                                          |
...
>
> **B-4 (L-M6) CERRADA (`3712e63`):** la 3ª llamada `append_fee` **NO era "no atómica" en sentido DB** — las 3 llamadas de `ExecuteTrade.execute` comparten la misma `AsyncSession` y se commitean juntas al final del request (rollback ante excepción). El riesgo dinero real era el **doble trade+fee sin `idempotency_key`**: los dos caminos internos (AUTO `execution_router.py:611`, confirm `confirm_recommendation.py:78`) no la pasaban, así que el guard `find_transaction_by_idempotency` nunca se activaba y un repost/retry reescribía trade+fee (dinero 2×). **Opción B:** se reenvía `idempotency_key` estable en ambos — AUTO usa el `make_auto_execute_idempotency_key` (día×política×kind, alineando el dedup Redis/TTL con el guard DB persistente `transactions` UNIQUE) y confirm usa `decision_id or session_id`. `idem_key`/`claimed` se movieron a nivel de función (evita también un `NameError` latente del `except ValueError` con el cognitive gate desactivado). Sin falsos positivos (la clave identifica la ejecución lógica, no el payload). API `POST /portfolio/trade` ya soporta `idempotency_key` (sin `contract:gen`). Sin migración/repos/API/FE. Tests `test_execute_trade_idempotency.py` (4). Batería: ruff 0 · mypy confirm_recommendation Success · app 249 (245+4) · api-python accounts 5. Ver §4l.
>
> **B-5 (T-M9/T-M10) CERRADA (`76abe0f`):** **T-M9** `_fifo_realized` (`tax_report.py:100`) dividía `(total+fee)/quantity` en la rama buy sin guard `quantity==0` → `ZeroDivisionError` latente (el write-path `execute_trade`, `portfolio_repository.py:263`, ya rechaza quantity≤0). Fix: el lote buy solo se apendiza si `quantity > 1e-9` (un buy de quantity 0 no fabrica lote/basis). **T-M10** `fetch_core_r_pnl_extra_rows` degradaba la cola de revisión CORE-R en silencio (fail-open) cuando un top con estrategias no tenía cuenta paper vinculada o el account no tenía summary live; la semántica instrumento→cuenta (atribuir el return del equity whole-account de la cuenta paper al instrumento) es **intencional** y espejada en el FE (`core-r-scheduler-tick.ts:40`), NO se toca. Fix: `logger.warning` en los dos skips silenciosos para que el fallo sea observable. Sin cambio de retorno. Tests `test_fifo_buy_quantity_zero_no_divide_by_zero_ni_lote`. Batería: ruff 0 · mypy domain Success · domain FIFO 13 (nuevo incl.) · app 249. Ver §4m.
>
> **B-2 (T-M8) CERRADA (`7b3dd91`):** `total_unrealized_gain` silenciaba a 0 las posiciones sin precio en `build_tax_report` (`tax_report.py:368-372`, `sum(line.unrealized_gain or 0.0 ...)`); `unrealized_gain` es `None` cuando no hay precio de mercado (la cara unrealized del tax-report que puebla `open_positions_with_fee_basis`, puente M-3 — **NO confundir con M-1** que era en `get_summary` de infra). **Opción A (fail-closed, decisión usuario):** si alguna posición abierta tiene `unrealized_gain None`, `total_unrealized_gain = None` (el FE `tax-report-page.tsx:332-345` lo muestra "—" en vez de un 0 infra-representado). Coherente con M-1. Shape DTO `float | None` intacto (sin `contract:gen`); consumidores: API `account_mappers.py:219` + FE tax-report (jobs CORE-R NO consumen el gain). Tests `test_tax_report_open_positions.py` (+3: sin precio → None / todas con precio → 245 / mixto → None). Batería: ruff 0 · mypy domain/src Success · domain 25 (3 nuevos) · app 249. Ver §4n.
>
...
**Estado al cerrar (verificado):** `main` limpio, sincronizado con `origin/main` · **CERRADAS: L-M3/M-5 (F3) · M-1 · M-2 · M-3 · M-6 · M-4/T-M5 · M-7 · B-1 · B-3 · B-4 · B-5** → **Alto×3 + Medio×7 + Baja B-1 + Baja B-3 + Baja B-4 + Baja B-5 de R-7 COMPLETADAS** · **T-M4 (job dedicado) DIFERIDO** por freeze · ancla `docs/engineering/backlog-trabajo-2026-08-20.md` §0 al día (LEER PRIMERO al abrir).
...
- **B-2 (T-M8):** `total_unrealized_gain` silencia a 0 las posiciones sin precio (_no confundir con M-1, ya cerrada_) → **✅ CERRADA** (`7b3dd91`, opción A fail-closed, ver §4n).
- **B-4 / B-5** — ✅ **CERRADAS** (B-4 `3712e63` §4l · B-5 `76abe0f` §4m).
...
## ✳️ ARRANQUE SIGUIENTE ETAPA: **FIN de R-7 COMPLETADO → GUARDAR + mandar auditorías externas del estado global**

> **Cómo usar:** pega este bloque (o el §8 anterior) como primer mensaje del NUEVO chat. El coordinador del nuevo chat ejecuta la secuencia en orden, una fase = un subagente acotado + batería + aprobación por commit + relevo documentado. **DECISIÓN DE USUARIO (2026-08-20):** R-7 quedó **cerrado por completo** (deuda Baja B-2 `7b3dd91` incluida) → **siguiente hito: GUARDAR + mandar auditorías externas para ver el estado global**.
>
> **B-1 (T-M7) CERRADA** (`4f43aeb`, high-water-mark max drawdown — ver §4j). **B-3 (L-M4) CERRADA** (`7cffaa7`, `transfer_cash` muerto eliminado — ver §4k). **B-4 (L-M6) CERRADA** (`3712e63`, trade+fee idempotente en AUTO/confirm, opción B — ver §4l). **B-5 (T-M9/T-M10) CERRADA** (`76abe0f`, guard FIFO qty==0 + observabilidad PnL CORE-R — ver §4m). **B-2 (T-M8) CERRADA** (`7b3dd91`, `total_unrealized_gain` fail-closed con posiciones sin precio — ver §4n).
...
**Estado al abrir:** `local main = origin/main` (cierre B-2 `7b3dd91` + docs actualizados), árbol limpio. **CERRADAS: L-M3/M-5 (F3) · M-1 · M-2 · M-3 · M-6 · M-4/T-M5 · M-7 · B-1 · B-3 · B-4 · B-5 · B-2.** Queda solo **M-4/T-M4 DIFERIDO** por freeze (job dedicado, colinda con "sin features").

**Plan de trabajo de esta etapa (FIN de R-7 → guardar + auditar):**

1. ~~**B-2 (T-M8)**~~ — ✅ **CERRADA** (`7b3dd91`, opción A fail-closed): `total_unrealized_gain` pasa a `None` si una posición abierta no tiene precio (FE muestra "—", no un 0 infra-representado). Coherente con M-1. Ver §4n. Riesgo verdad. **R-7 deuda completa.**
2. **GUARDAR + mandar auditorías externas del estado global** (decisión y alcance del usuario) — revisar también el checklist operativo manual del §4 del backlog (secret scanning UI, `TRUSTED_PROXIES`, `BP/.L`→`BP.L`, logs dev) y la higiene de ramas `stage/*`.
3. **M-4/T-M4 (diferido por freeze):** mover el cargo de custodia del path de lectura (GET summary/tax) a un job dedicado — cambio de comportamiento, requiere decisión cuando se levante el freeze.
...
**Decisiones de alcance vigentes que NO reabrir sin pedir:** M-3 = puente (storage/`avg_cost` sigue fee-excluido); M-4/T-M4 diferido por freeze (job dedicado); B-3 CERRADA = eliminar `transfer_cash` (código muerto, no reabrir); B-1 CERRADA (high-water-mark, no reabrir); B-4 CERRADA = trade+fee idempotente en AUTO/confirm (opción B, no reabrir); B-5 CERRADA (guard FIFO qty==0 + observabilidad PnL CORE-R; semántica instrumento→cuenta intencional, no reabrir); B-2 CERRADA (`total_unrealized_gain` fail-closed con posiciones sin precio, opción A, no reabrir); M-6/M-7 CERRADAS. Freeze: sin features nuevas · no reabrir Belief/H · no tocar gobernanza IA · auth JWT diferida (D4). **No `regen_full`** sin decisión. **No `contract:gen`** salvo fase pactada.
