...
>
> **SIGUIENTE ETAPA (por decisión 2026-08-20): FIN de R-7** — deuda de dinero real de R-7 **completa** (Alto×3 + Medio×7 + Baja **B-1** + Baja **B-3** + Baja **B-4** + Baja **B-5** cerradas). Queda la **Baja: B-2** (unrealized silencia sin precio) · **M-4/T-M4** (job dedicado, DIFERIDO por freeze). **Se decide NO pausar ahora:** cuando R-7 quede del todo cerrado → **guardar + auditorías externas**. **L-M3/M-5 CERRADA · M-1 · M-2 · M-3 · M-6 · M-4/T-M5 · M-7 · B-1 · B-3 · B-4 · B-5 TODAS CERRADAS.**
>
...
- **B-2 (T-M8):** `total_unrealized_gain` silencia a 0 las posiciones sin precio (_no confundir con M-1, ya cerrada_). **[SIGUIENTE]**
...
>
> **B-1 (T-M7) CERRADA** (`4f43aeb`, high-water-mark max drawdown — ver §4j). **B-3 (L-M4) CERRADA** (`7cffaa7`, `transfer_cash` muerto eliminado — ver §4k). **B-4 (L-M6) CERRADA** (`3712e63`, trade+fee idempotente en AUTO/confirm, opción B — ver §4l).
...
**Estado al abrir:** `local main = origin/main` (cierre B-5 `76abe0f` + docs actualizados), árbol limpio. **CERRADAS: L-M3/M-5 (F3) · M-1 · M-2 · M-3 · M-6 · M-4/T-M5 · M-7 · B-1 · B-3 · B-4 · B-5.** Queda la **Baja de R-7: B-2** (+ **M-4/T-M4 DIFERIDO** por freeze).
...
2. ~~**B-4 (L-M6)**~~ — ✅ **CERRADA** (`3712e63`, opción B): trade+fee idempotente en AUTO execute y confirm intent (guard DB activado). Riesgo dinero. Ver §4l.
3. ~~**B-5 (T-M9/T-M10)**~~ — ✅ **CERRADA** (`76abe0f`, ver §4m): guard `quantity==0` en FIFO buy + `logger.warning` en skips silenciosos PnL CORE-R (semántica instrumento→cuenta intencional, NO se toca). Riesgo verdad.
4. **B-2 (T-M8)** — `total_unrealized_gain` silencia a 0 las posiciones sin precio (NO confundir con M-1, ya cerrada). Riesgo verdad. **[SIGUIENTE del FIN de R-7]**
