...
> **Estado:** ✅ **R-8A PUSHEADA a `main`** (código `edf2d0c` + docs `7f327ab`) · ✅ **R-8B.1 PUSHEADA** (`ac147fe`) · ✅ **R-8B.2 PUSHEADA** (`abf3dc2`) · ✅ **R-8C PUSHEADA** (`3ad48aa`) · ✅ **R-8D PUSHEADA** (`dbd1ee5`) · ✅ **R-8B.3 PUSHEADA a `main`** (`fb95b27`) · ✅ **CONTRACT-STALE RESUELTO** (`aecbb28`, regen acotada, `contract:check` verde) — **R-8 COMPLETA** en `main`. Pendiente decisión: solo ítems `pending-delete` riesgo alto + `M-4/T-M4` diferido.
...
- R-7 completo (todos sus items cerrados; `M-4/T-M4` diferido). **R-8 COMPLETA**: R-8A (`edf2d0c`+`7f327ab`), R-8B.1 (`ac147fe`+`a1360bb`), R-8B.2 (`abf3dc2`), R-8C (`3ad48aa`), R-8D (`dbd1ee5`) — todas PUSHEADAS a `main`. **R-8B.3 COMPLETA a `main` (`fb95b27`)**: Fase A cash (`158b3db`) · FIE FUND (`756e3a1`) · B signal-alerts (`ce1d5ca`) · C dictamen (`e6dde15`) · D account-settings (`fb95b27`) — fidelidad wire en `packages/shared`. **CONTRACT-STALE RESUELTO (`aecbb28`)**: regen acotada de `openapi.json`+`schema.d.ts` (idempotencyKey Deposit/Withdraw R-8A · `authenticated` required `@default false` R-8B.2 · response+descripción `logout`) → `contract:check` **VERDE**. Pendiente no urgente: ítems pending-delete riesgo alto ("hasta purge storage") · **M-4/T-M4** (job dedicado) diferido por freeze.
...
| 2026-08-20 | CONTRACT-STALE (fase dedicada D5)   | `aecbb28`                                 | PASO 1 read-only (worktree aislado, repo limpio) · diff 5 legítimos/0 no deseado · `contract:check` **VERDE** · web typecheck 0 · lint 0 · test 714/714 · árbol acotado a 2 artefactos | ✅ **PUSHEADA a `main`**       |
