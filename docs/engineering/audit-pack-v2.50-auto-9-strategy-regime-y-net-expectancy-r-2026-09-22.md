...
4. **Quién borra los espacios, medido: `prettier`, no la ruta de escritura.** El ciclo repetido sobre el mismo documento lo aísla: reparado con el script queda en **0** huecos, pasa el `prettier` **local** (`3.8.4`, el del hook `lint-staged`) y **vuelven los mismos**, se repara y vuelve a **0**, y sin `prettier` de por medio el contador no se mueve. El diff por caracteres da la regla: **borra el espacio que sigue a un span de código** al reimprimir la línea — **44** borrados en `PROJECT_STATE:22` (la línea de índice de packs, que además normaliza añadiendo el prefijo `> `) y **~3** por fila de tabla larga. Las **dos** rutas de escritura (script y editor) escriben el texto **intacto** en todos los contextos probados (párrafo, blockquote, lista, tabla) y `prettier` respeta el espaciado en fragmentos pequeños: el daño aparece solo en el documento grande que él reimprime. **CERRADO** en el tramo de tooling posterior al sello (decisión del usuario): `*.md` queda **fuera** de `prettier` (`format`, `format:check` y `lint-staged`) y la reparación se hace definitiva con `tools/fix_md_spacing.py`, que empareja spans y **solo** inserta espacios — **251** huecos en **19** ficheros (**169** en `PROJECT_STATE`), con invariante medido: `git diff -w` muestra **solo** los dos ficheros de configuración (`package.json` y `.lintstagedrc.json`), o sea que los 251 espacios son **invisibles** a la comparación sin espacios ⇒ la reparación es **solo espacial**. `prettier` **no** está en ningún workflow de CI (`grep` en `.github/workflows`): la exclusión no cambia ninguna compuerta. `pnpm format:check` sigue fallando por **141** ficheros **pre-existentes** ajenos (`turbo.json`, `.vscode/*.json` sin trackear, `governor.json`, …), ninguno tocado aquí.
