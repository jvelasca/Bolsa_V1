...
| **D3** | ¿El Lab/Radar quedan **fuera** del spine?                                                          | (a) fuera (laboratorio, `ADR-019`) · (b) el diario puede proponer a la mesa            | ⏳ recomendada **(a)** — pendiente confirmación                                                                                                                                                                                                      |

> **D1 CERRADA por el propietario.** D2/D3 no bloquean los docs; se cierran antes de abrir F0.5. Con D1, el descargue queda definido: **mismo gate + mismo risk en SEMI y AUTO**; el único parámetro que difiere es la **autorización** (humano `/confirm` vs policy `paper_auto`).
