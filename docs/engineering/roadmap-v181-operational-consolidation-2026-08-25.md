...
> **AsOf:** 2026-08-25.
> **Estado:** consolidación **CERRADA en código** (C1–C5 + ADR-032 + C6 stamp). Tag `v1.8.1-beta` **parked**.
...
| **C2** | Alembic única autoridad | Retirar/renombrar `db:push` / `db:migrate` Prisma públicos                               | **CERRADO `952b115`**  |
| **C3** | ActionQueue             | Prioridad determinista · cola completa ≠ slice UI top-N                                  | **CERRADO `420ad37`**  |
| **C4** | Contratos / shape drift | TradePlan reader canónico (`readCanonicalTradePlan` + `planSource`) · sin `contract:gen` | **CERRADO `96d1148`**  |
| **C5** | Honesty métricas        | MFE real ≠ proxy · Expectancy sample-quality en UI                                       | **CERRADO `4e245f7`**  |
| **C6** | Stamp pack v1.8.1       | Audit pack · CHANGELOG Unreleased · **tag parked**                                       | **CERRADO documental** |
