...
| **C4** | Contratos / shape drift   | TradePlan reader canónico (`readCanonicalTradePlan` + `planSource`) · sin `contract:gen` | **CÓDIGO LISTO**      |
| **C5** | Honesty métricas          | MFE real ≠ proxy · Expectancy sample-quality en UI                                       | **CÓDIGO LISTO**      |
