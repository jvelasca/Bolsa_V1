...
| `Gitleaks` (los tres commits: `a60f72c1`, `1127d010`, `006c8a87`) | `35150808784`, `35154789021`, [`35187161098`](https://github.com/jvelasca/Bolsa_V1/actions/runs/35187161098) | success                                                                                                                                                                                                                                                             |

**El commit de sellado NO entra en el tag y NO re-dispara la CI de Python.** `main` tiene un commit
posterior al tag, `006c8a87` (`docs(v2.40.4): sellado del audit-pack...`), que toca solo `CHANGELOG.md`
y `docs/engineering/**`: ninguna de esas rutas está en el `paths` de `Python CI`
(`apps/api-python/**`, `packages/py/**`, `scripts/verify/**`, `pyproject.toml`, `Makefile`, el propio
workflow), así que ese push **no** lanza `Python CI` (medido: para `006c8a87` solo corrió `Gitleaks`,
run `35187161098`, success). El tag sigue apuntando a `1127d010`, el commit **verde**. Patrón del repo:
el **código y su evidencia CI** van en el tag; la **guía de lectura** va en el tip de `main`.
