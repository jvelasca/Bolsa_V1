...
> **Propósito:** documento **único** para auditoría externa general tras el **cierre y tag** del ciclo beta slice **U6 → DS-05 → ops → DS-03 → higiene → Research→Radar → `v1.7.0-beta`**, más el estado del **Ciclo 1/5 OrderProposal/Journal** (ADR-029 + F1 en working tree). Consolida identidad, freeze, arcos cerrados, verificación y riesgos ops.
> **AsOf:** 2026-08-24 · HEAD **`e3b943a`** · tag **`v1.7.0-beta` → `e3b943a`** verificado en `origin` · R-13 **CERRADA** · Track B **CERRADO** · Fase 0 spine **COMPLETA** · UX mesa **U0–U6 CERRADA** · **DS-05/DS-03 CERRADAS** · **ops propietario CERRADA** · **Research→Radar copy CERRADA** · **ciclo beta slice CERRADO y TAGGED** · **Ciclo activo = OrderProposal/Journal F1** (working tree, pendiente commit).
...
| Pieza                       | Estado                                                                                                                                   |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **Rama**                    | `origin/main` = **`e3b943a`** · tag **`v1.7.0-beta` → `e3b943a`** (verificado origin) · previo **`v1.6.0-beta` → `c3964fc`**             |
...
| **Ciclo activo**            | **Ciclo 1/5 OrderProposal/Journal** — ADR-029 + F1 dominio/persistencia **en working tree (pendiente commit)**                           |
| **Freeze**                  | OrderProposal/Journal **parcialmente levantado** (ADR-029); `PAPER_D_EXECUTE` off · sin broker live · Belief frozen · Track B no reabrir |

**Mensaje clave:** el núcleo financiero R-7→R-13, el Decision Spine (SEMI=AUTO risk, Fit VETO, confirm contrato, **freshness DS-05**, **mandate DS-03**), la mesa U0–U6, ops ejecutable, copy Research→Radar y el tag **`v1.7.0-beta`** están **cerrados y verificados** en **`e3b943a`**. El propietario aprobó **5 ciclos en orden**; el **Ciclo 1** (OrderProposal + DecisionJournal) tiene ADR-029 + implementación F1 **sin commit** en working tree. **BETA / no producción.**
...
HEAD **`e3b943a`** = stamp de cierre del ciclo beta (CHANGELOG + living SoT). Working tree ahead: **Ciclo 1 OrderProposal/Journal F1** (sin commit).
...
## 4. Ciclo activo — OrderProposal/Journal (Ciclo 1/5)

> **Estado al auditar:** working tree **sin commit** · base tag **`e3b943a`**.

| Ítem                          | Estado                                                                                                                                                                         |
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **ADR-029**                   | Aceptado (docs) — [`029-order-proposal-decision-journal.md`](../adr/029-order-proposal-decision-journal.md) · **pendiente commit**                                             |
| **Plan F1–F3**                | [`plan-order-proposal-journal-2026-08-24.md`](./plan-order-proposal-journal-2026-08-24.md) · **pendiente commit**                                                              |
| **F1 dominio + persistencia** | En working tree: DTOs shared · mapper · Alembic `010_decision_journal_entries` · `JournalWriter` · hooks best-effort propose/confirm/router · tests `test_decision_journal.py` |
| **F2 wire/API**               | **No abierto** — sin endpoints HTTP · sin `contract:gen`                                                                                                                       |
| **F3 UI journal**             | **No abierto**                                                                                                                                                                 |

**Alcance F1 (verificado en working tree):** OrderProposal = proyección refs-only desde `decision_sessions` `kind='propose'`; DecisionJournal = tabla append-only `decision_journal_entries`; hooks observan, **no relajan** gates D1/D2/H3.

Relevo: [`traspaso-relevo-order-proposal-journal-apertura-2026-08-24.md`](./traspaso-relevo-order-proposal-journal-apertura-2026-08-24.md).
...
| Ítem                                         | Estado                                                                         |
| -------------------------------------------- | ------------------------------------------------------------------------------ |
| OrderProposal / Journal (F1)                 | **Parcialmente levantado** — ADR-029 aprobado; F1 en working tree; sin HTTP/UI |
| Attribution / orquestador                    | **No**                                                                         |
| `PAPER_D_EXECUTE`                            | **off**                                                                        |
| Broker live                                  | **No**                                                                         |
| Track B B1–B12                               | **Cerrado** — no reabrir                                                       |
| Belief / gobernanza IA                       | **Freeze**                                                                     |
| `contract:gen`                               | Solo fase pactada (F2 requiere OK explícito)                                   |
...
**Firma release:** `git fetch && git rev-parse origin/main` → **`e3b943a`** · `git rev-parse v1.7.0-beta` → **`e3b943a`**
...
**F1 (working tree, post-commit coordinador):** `pytest packages/py/application/tests/test_decision_journal.py` · `ruff`/`mypy` ficheros tocados · `pnpm --filter @bolsa/shared typecheck`.
...
4. **Ciclo 1 sin commit** — F1 + ADR-029 en working tree; auditor debe tratar como **no desplegado** hasta commit aprobado.

---

## 8. Limitaciones conocidas (honestas; no son bugs de esta rebanada)

Copiado de [`CURRENT_SYSTEM.md`](../CURRENT_SYSTEM.md) (HEAD tag; F1 aún no commiteado):
...
- OrderProposal / Journal: **F1 en progreso** (working tree); Attribution **sin abrir**.
...
| Ciclo 1 F1        | working tree (pendiente commit) |
