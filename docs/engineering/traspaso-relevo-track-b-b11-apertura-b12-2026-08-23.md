# RELEVO / TRASPASO — Track B split backtests B11 → apertura B12 (2026-08-23)
...
> **Estado al redactar (verificado):** código B11 **local** = `50649a3` · `origin/main` al redactar = `7e2d24f` (B9; B10+B11 aún sin push) · docs stamp pendiente push · **R-13 CERRADA** · **Track B EN CURSO** · split backtests **B1–B11 HECHAS** · tag **`v1.6.0-beta` → `c3964fc`** intacto.
> **AsOf:** 2026-08-23.
...
- **HEAD/rama:** código B11 local = `50649a3` (2 commits ahead de `origin/main` `7e2d24f` B9: `bef3c9f` B10 + `50649a3` B11). SHA vivo tras push = `git rev-parse origin/main`.
- **Commits split B1–B11 (de más nuevo a más antiguo):**
...
- **Track B — split `backtests-page`:** B0 plan ✅ · **B1–B11 código ✅** · **B12 pendiente** (thin shell final).
- **LOC página (aprox. post-B11):** ~1686 (partía ~4697; post-B10 ~1706). Tab jobs ~175 LOC (JSX ~90 + VM + imports). Tab run B10 intacto.
- **Batería B11 (coordinador):** typecheck 0 · lint 0 errores · test **754/754**.
...
| B10  | `apps/web/src/features/backtests/backtests-page-run-tab.tsx`           | JSX tab `run` (wizard + result + monitor). Tipo `BacktestPageViewModel`. `BacktestsPageRunTab` presentacional (sin hooks). Cableado `{tab === "run" && <BacktestsPageRunTab vm={runTabVm} />}`                                                                                                                                                            |
| B11  | `apps/web/src/features/backtests/backtests-page-jobs-tab.tsx`          | JSX tab `jobs` (Lab · Optimizar). Tipo **propio** `BacktestPageJobsViewModel` (no ensancha el VM de B10; `runMutation` aquí es `Pick<"mutate">`). `BacktestsPageJobsTab` presentacional (sin hooks). Cableado página `:1587` `{tab === "jobs" && <BacktestsPageJobsTab vm={jobsTabVm} />}`. Reset rail `setOptimizeCompare(null)` **sigue en la página**. |

### Deuda explícita post-B11 (no auto-cerrar) — alcance B12

- Thin shell final — **B12**: criterio plan `backtests-page.tsx` ≤ 600 LOC (hoy ~1686). Tabs `strategies` (`:1589–1669` `BacktestLibraryTab`) y `history` (`:1671–1679` `BacktestHistoryTab`) **siguen en la página** (ya son componentes; el cableado queda en el shell). **Riesgo Medio.**
- `proposeFinalistMutation` sigue en la página (depende de derivados) ~`:855`.
- Queries tardías en página: `linkedTrialQuery` · `replayBarsQuery` · `drawingReplayQuery` (~`:676–762`).
- Effects matrix fingerprint / sync `setMatrixRows` siguen en página.
- Header + `BacktestAssistantRail` + zone settings siguen en el shell (~`:1451–1583`).
...
**No reabrir B7–B11.** `BacktestPageViewModel` (B10) y `BacktestPageJobsViewModel` (B11) no se mezclan salvo necesidad demostrada en B12 (y entonces con justificación file:line).
...
3. **Anti-saturación:** el chat anterior cerró tras B11 (código `50649a3` + este stamp). **Este chat nuevo** abre **solo B12**. No reabrir B7–B11. B12 es la **última** fase del split; no encadenar trabajo post-B12 en el mismo chat.
...
Smoke B12: shell (header + rail + tabs) · tab `run` B10 intacto · tab `jobs` B11 intacto · library/history cableados · keep-alive Lista AUTO (`platform-shell.tsx`) · Regla de Hooks (early return post-hooks intacto).
...
| Ítem                    | Estado / regla                                                                            |
| ----------------------- | ----------------------------------------------------------------------------------------- |
| **Split backtests B12** | **SIGUIENTE** — thin shell · plan `plan-split-backtests-page-2026-08-22.md` · 1 subagente |
...
> CONTEXTO (2026-08-23): git fetch && git rev-parse origin/main (código B11 local = 50649a3; B10 = bef3c9f; origin al stamp = 7e2d24f B9; no asumir SHA remoto si aún no hay push). Track B EN CURSO. Split backtests **B1–B11 ✅** (6271c8c · fcdc857 · bcadea9 · 5475c09 · ca13981 · 5c03ff7 · 09f908b · 6e998bd · 7e2d24f · bef3c9f · 50649a3). Research→Radar F4′–F6′ ✅ (240c846). LEE: traspaso-relevo-track-b-b11-apertura-b12-2026-08-23.md · backlog §0 · plan-split-backtests-page-2026-08-22.md. Tarea: backtests **B12** (thin shell final; página ≤600 LOC; tabs strategies/history siguen en la página). 1 subagente, sin commits sin aprobación. Riesgo **Medio**. No reabrir B7–B11. Anti-saturación: **B12 solo en este chat nuevo**.

### 4.2 Brief subagentes (patrón)

> Una fase = subagente acotado: read-first backlog §0 + plan split §3 fase B12 + archivos exactos + qué NO tocar (B7–B11, `backtests-page-run-tab.tsx`, `backtests-page-jobs-tab.tsx`) + batería esperada + **NO commits ni push** + reporte file:line. Coordinador re-verifica antes de proponer commit. Anti-saturación: no reabrir B11 ni encadenar post-B12 en el mismo chat.
...
- Relevo anterior (B10 → B11): `traspaso-relevo-track-b-b10-apertura-b11-2026-08-23.md` (histórico)
...
> **Siguiente:** backtests **B12** (thin shell final — `backtests-page.tsx` ≤ 600 LOC). Riesgo **Medio**. Nuevo chat (anti-saturación). No reabrir B7–B11.
