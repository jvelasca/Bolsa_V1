...
| 2026-08-11 | **M1 commit+push** `3d093cb`. M2 aplicado: `deduct_cash` gana `allow_partial: bool=False`; usuario→ValueError sin truncar; custodia→`allow_partial=True` y `ApplyCustodyFees` registra importe real + `balance_after` real con descripción de cargo parcial por saldo. Tests `test_deduct_cash_allow_partial.py` (3, DB-requeridos). Batería: ruff ✓ · pytest (260) ✓ · mypy sin errores nuevos (deuda previa en `accounts.py` no-untyped). Pendiente de commit+push aprobado.                                                                                                                                                                                                                                                      |
| 2026-08-11 | **M2 commit+push `113e27a`.** **Checkpoint docs traspaso** `ee44589` (traspaso + índice actualizados con M1/M2 + texto exacto §9). **M3 aplicado:** `ExecuteTrade.execute` usa `result.summary.portfolio.cash` como `balance_after` (eliminado recálculo manual que doble-contaría fees). Batería: ruff ✓ · pytest (260) ✓ · mypy 5 errores pre-existentes (sin nuevos). Pendiente de commit+push aprobado.                                                                                                                                                                                                                                                                                                                         |
| 2026-08-11 | **M3 commit+push `5128cf4`.** **M4 aplicado** (idempotencia + contrato estricto): usuario decidió **Prisma migrate** (Alembic no funcional en F1: sin `env.py`). Migración `20260811010000_trade_idempotency` (prisma + SQLAlchemy `TransactionRow.idempotency_key` + `UNIQUE(portfolio_id, idempotency_key)`); `execute_trade`/`ExecuteTrade.execute` con `idempotency_key` opcional (retry→misma tx, sin duplicar); `TradeRequestDto` estricto (`Literal["buy","sell"]`, `gt=0`, `allow_inf_nan=False`, `idempotencyKey` opcional). Aplicada la migración a la DB local (`db:migrate:deploy`). Test `test_trade_idempotency.py` (3). Batería: ruff ✓ · pytest (287) ✓ · mypy 0 errores nuevos. Pendiente de commit+push aprobado. |
| 2026-08-11 | **M4 commit+push `986399d`.** **M5 aplicado** (suite invariantes contables): `test_financial_invariants.py` (5, PG real) — cash≥0, qty≥0, balance_after coherente, idempotencia no duplica, y **anti-doble-gasto bajo concurrencia** (dos buys simultáneos: exactamente 1 éxito, cash=500≥0, qty=10; prueba que el lock de M1 funciona). Añadido fix Windows SelectorEventLoop en infra tests. Batería: ruff ✓ · pytest (292) ✓ · mypy 0 errores de código. Pendiente de commit+push aprobado. **F1 casi completa: falta aprobar M5 + `pnpm test` + CI + pnpm test.**                                                                                                                                                               |
