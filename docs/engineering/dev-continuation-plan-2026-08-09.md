...
Siguiente lote: **resto de `apps/web/src`** (fuera de `features/backtests`, que queda COMPLETO con los lotes 15-18: `6c32b06`/`a42587e`/`da2f7d9`/`5bec6ed`, y el dominio `hub`/`chart`/`misc` CERRADO). **Lote 19 hecho (`689c294`): `features/accounts`.** **Lote 20 hecho (`1081809`): `features/workspace`.** **Lote 21 hecho (`75f6595`): `features/config`+`platform`, 14 files, +601/-443.** Continuar por dominios (charts, instruments, trading, settings, screeners, research, lib, stores, components/ui, layout, etc.), con el mismo protocolo de 8 pasos y verificando el `git diff --cached --numstat`. `prettier --check` amplio (excl. `features/backtests`) ahora: **381 files** desincronizados pendientes. Verificación tras cada lote y commit propio. HEAD `75f6595` ·
sincronizado.
