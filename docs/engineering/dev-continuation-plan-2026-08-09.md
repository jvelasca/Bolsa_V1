...
Siguiente lote: otro sub-lote por dominio de `features/backtests` (p. ej. `chart`/`misc`, subdividir los restantes ~40 files; `hub` ya hecho lote 15 `6c32b06`, `coach`/`lab` lote 14 `6f8c668`), después el resto de `apps/web/src` por sub-lotes. Verificación tras cada lote y commit propio. HEAD `6c32b06` ·
sincronizado.
