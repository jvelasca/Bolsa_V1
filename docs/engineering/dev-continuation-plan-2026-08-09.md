...
Siguiente lote: **dominio `hub`/`chart`/`misc` CERRADO (lotes 15-18: `6c32b06`/`a42587e`/`da2f7d9`/`5bec6ed`)** → con esto se ha formateado por completo `features/backtests` del lote 10 en adelante (los 18 lotes cubren todos los dominios de `features/backtests` pendientes). A partir de aquí, seguir con el **resto de `apps/web/src`** por sub-lotes (components/ui, layout, features de otros dominios, lib, stores, etc.), con el mismo protocolo de 8 pasos y verificando el `git diff --cached --numstat`. Verificación tras cada lote y commit propio. HEAD `5bec6ed` ·
sincronizado.
