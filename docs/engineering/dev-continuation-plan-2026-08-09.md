...
Siguiente lote: **resto de `apps/web/src`** (fuera de `features/backtests`, que queda COMPLETO). **Lote 19 (`689c294`): `accounts`. Lote 20 (`1081809`): `workspace`. Lote 21 (`75f6595`): `config`+`platform`. Lote 22 (`cbd0fff`): `research`. Lote 23 (`4300674`): `settings`, 25 files, +1927/-1433. Lote 24 (`e38be2d`): `instruments`, 30 files, +2128/-1399.** Continuar por dominios (charts, trading, screeners, lib, stores, components/ui, layout, etc.), con el mismo protocolo de 8 pasos y verificando el `git diff --cached --numstat`. `prettier --check` amplio (excl. `features/backtests`) ahora: **316 files** desincronizados pendientes. Verificación tras cada lote y commit propio. HEAD `e38be2d` ·
sincronizado.
