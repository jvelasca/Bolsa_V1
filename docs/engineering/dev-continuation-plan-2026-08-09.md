...
| 10 | `9853e79` | `backtests/optimize` **restantes** = **16 files** (prod + tests: `backtest-optimize-compare`(+test), `backtest-optimize-delta`, `backtest-optimize-from-seed`(+test), `backtest-optimize-heatmap-panel`(+`backtest-optimize-heatmap.test`), `backtest-optimize-panel`, `backtest-optimize-progress`, `backtest-optimize-seed`(+test), `backtest-optimize-space`(+test), `backtest-optimize-validation-hint`(+test), `backtest-optimized-strategy`), diff +1333/−932 solo formato. Sin falsos positivos EOL (los 16 con diff real; solo `'→"` + line-wrapping) | typecheck ✅ · lint 0e ✅ · test 140/707 ✅ · build ✅ |
| 11 | `ee381d7` | `backtests/strategy-matrix` **restantes** = **11 files** (prod + tests: `backtest-strategy-matrix`(+test), `strategy-matrix-column-layout`(+test), `strategy-matrix-filter-carousel-prefs`(+test), `strategy-monitor`(+test), `strategy-monitor-panel`, `mine-strategies-filters`(+test)), diff +970/−785 solo formato. Sin falsos positivos EOL (los 11 con diff real; solo `'→"` + line-wrapping) | typecheck ✅ · lint 0e ✅ · test 140/707 ✅ · build ✅ |
| 12 | `9975cd3` | `backtests/list-auto` **+ `mass-compare`** = **11 files** (prod + tests: `backtest-list-auto`(+test), `backtest-list-auto-board`(+test), `backtest-list-auto-board-panel`, `backtest-list-auto-persist`(+test), `backtest-list-auto-statusbar.test`, `backtest-mass-compare`(+test), `backtest-mass-compare-panel`), diff +576/−430 solo formato. Sin falsos positivos EOL (los 11 con diff real; solo `'→"` + line-wrapping) | typecheck ✅ · lint 0e ✅ · test 140/707 ✅ · build ✅ |
| 13 | `f011918` | `backtests/finalists` **+ `top`** = **17 files** (8 prod + 9 tests: `backtest-finalists-freshness`(+test +`-restart.test`), `finalists-stability-summary`(+test), `finalist-propose-supervised`(+test), `promote-finalist-to-tracker`(+test), `instrument-strategy-top-panel`(+test), `instrument-strategy-top-promote`(+test), `instrument-top-match`(+test), `instrument-top-strategy-type`(+test)), diff +807/−667 solo formato. Sin falsos positivos EOL (los 17 con diff real; solo `'→"` + line-wrapping) | typecheck ✅ · lint 0e ✅ · test 140/707 ✅ · build ✅ |
| 14 | `6f8c668` | `backtests/coach` **+ `lab`** = **21 files** (prod + tests): `coach-dual-audit`(+test), `coach-profile-policy`(+test), `coach-llm-invariant.test`, `coach-top-save`(+test), `coach-quorum-bar`.tsx, `coach-profile-battery-scenario.test`; `lab-coach-handoff`(+test), `lab-adoption-memory`(+test), `lab-board-activity-banner`.tsx, `lab-zone-verdict`.tsx, `lab-coach-caf-smoke.test`; + los **5 pendientes verificados** que `--check` reporto y se anadieron: `backtest-lab-board`.tsx + `backtest-lab-board-types`.ts, `backtest-coach-lote`(+test), `backtest-coach-coherence.test`; diff +1326/-1048 solo formato. Sin falsos positivos EOL (los 21 con diff real) | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) · **test:coach 26/186 ok** (API smoke CORE-P = SKIP optional live, dato → area Coach/TOP) |
| 15 | `6c32b06` | `backtests/hub` (hub primero del dominio `hub`/`chart`/`misc`) = **4 files** (`backtest-hub-layout`(.tsx), `backtest-global-bar`(.tsx), `backtest-history-tab`(.tsx), `backtests-page`(.tsx, orquestador M5; solo line-wrapping, +23/-21)), diff +161/-102 solo formato. Sin falsos positivos EOL (los 4 con diff real). `hub-nav`(+test)/`hub-tabs`(+test) YA estaban formateados (no reportados por --check). No es area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
| 16 | `a42587e` | `backtests/chart` (primera parte del dominio `hub`/`chart`/`misc`; `hub` ya hecho lote 15 `6c32b06`) = **16 files** (paneles de chart/visualizacion + zone): `backtest-chart-import-panel`, `backtest-equity-chart`, `backtest-replay-chart`, `backtest-stat-donut`, `backtest-ranking-table`, `backtest-universe-picker`, `backtest-cursor-panel`, `backtest-favorites-menu`, `backtest-future-stars`(+test), `backtest-instrument-preview`, `backtest-movie-hud`, `backtest-movie-stats`; zone: `backtest-zone-settings-dialog`, `backtest-zone-prefs`(+test)), diff +721/-458 solo formato. Sin falsos positivos EOL (los 16 con diff real; 1er `--write` en background fallo por error transitorio FileSystem, reintentado ok). No es area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
| 17 | `da2f7d9` | `backtests/misc` motores (domino `hub`/`chart`/`misc`; `hub` lote 15, `chart` lote 16) = **23 files reales de 24 candidatos** (motores/logica misc): `backtest-paper-checklist`, `backtest-paper-gate`(+test), `backtest-run-context`(+test), `backtest-batch-run`, `backtest-export`, `backtest-date-format`, `backtest-split-layout`, `backtest-hud-prefs`, `use-backtest-hud-prefs`, `backtest-buy-hold`(+test), `backtest-deep-coach`(+test), `backtest-oos-evidence`(+test), `backtest-pbo`(+test), `backtest-period`(+test), `backtest-walk-forward-metrics`(+test)), diff +1389/-1087 solo formato. **1 falso +EOL**: `backtest-period-returns.ts` (contenido normalizado identico a HEAD) -> fuera del commit. Area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) · **test:coach 26/186 ok** (API smoke CORE-P = SKIP optional live) |
| 18 | `5bec6ed` | `backtests/library` + `estudio`/`ibex`/tests sueltos (CIERRE del dominio `hub`/`chart`/`misc`) = **11 files** (`library-nav`(+test), `library-strategy-buckets`(+test), `estudio-list.test`, `estudio-personal-list.test`, `ibex35-operativa-audit`(+test), `chart-strategy-bridge.test`, `drawing-replay-parity.test`, `signal-evaluate-parity.test`), diff +360/-295 solo formato. Sin falsos positivos EOL (los 11 con diff real). Cierra el dominio `hub`/`chart`/`misc` de `features/backtests` | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) · **test:coach 26/186 ok** (API smoke CORE-P = SKIP optional live) |
| 19 | `689c294` | `features/accounts` — **primer sub-lote del resto de `apps/web/src`** (fuera de `features/backtests`, que queda COMPLETO) = **17 files** (todos de produccion + `paper-lab-evidence.test`): `account-detail-panel`, `account-investor-profile-select`, `account-scope-selector`, `account-settings-dialog`, `account-settings-panel`, `account-wizard-capital-step`, `account-wizard-commissions-step`, `account-wizard-identity-step`, `account-wizard-review-step`, `account-wizard-tax-step`, `accounts-page`, `create-account-wizard-dialog`, `investor-profile-panel`, `investor-profile-picker`, `paper-lab-evidence`(+test), `use-active-account`), diff +973/-656 solo formato. Sin falsos positivos EOL (los 17 con diff real; solo `'→"` + line-wrapping). No es area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
| 20 | `1081809` | `features/workspace` = **7 files** (`use-visualization-workspace-sync`, `visualization-workspace-sync`, `workspace-auto-save`, `workspace-bootstrap`, `workspace-picker-dialog`, `workspace-remote-sync`, `workspace-ui-bridge-register`), diff +107/-78 solo formato. Sin falsos positivos EOL (los 7 con diff real; solo `'→"` + line-wrapping). No es area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
| 21 | `75f6595` | `features/config` **+ `platform`** = **14 files** (`config`: `database-config-panel`, `notification-prefs`(+test), `notifications-settings-panel`, `platform-config-dialog`; `platform`: `mandate-tenure-pnl`(+test), `operating-mandate`(+test), `operating-mandate-sync`, `product-universe`(+test), `strategy-adoption`, `universe-chip`), diff +601/-443 solo formato. Sin falsos positivos EOL (los 14 con diff real; solo `'→"` + line-wrapping). No es area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
| 22 | `cbd0fff` | `features/research` = **10 files** (`asesor-daily-ops-panel`, `asesor-opiniones-panel`, `estudio-opinion-alarm-poller`, `opinion-channel-map.test`, `research-lab-evidence-summary`, `research-lab-evidence`(+test), `research-page`, `research-trial-result-block`, `use-asesor-alarma-badge`), diff +749/-452 solo formato. Sin falsos positivos EOL (los 10 con diff real; solo `'→"` + line-wrapping). No es area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
| 23 | `4300674` | `features/settings` = **25 files** (`ai-platform-section`, `ai-platform-tracker`(+test), `backtesting-help-section`, `backtesting-tracker`(+test), `chart-platform-section`, `chart-platform-tracker`, `data-capture-section`, `data-market-tracker`, `data-sync-summary-card`, `decision-replay-panel`, `dia-d-evidence-archive-help-card`, `effectiveness-panel`, `general-settings-section`, `market-providers-status-card`, `paper-paths-copy`(+test), `settings-redirect-page`, `settings-section`, `supervised-f3-panel`, `value-analysis-section`, `value-analysis-tracker`, `watchlist-help-section`, `watchlist-lists-tracker`), diff +1927/-1433 solo formato. Sin falsos positivos EOL (los 25 con diff real; solo `'→"` + line-wrapping). No es area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
| 24 | `e38be2d` | `features/instruments` = **30 files** (`composite-leg-labels`(+test), `fundamental-card-panel`, `instrument-detail-page`, `instrument-dictamen-evolution`(+test), `instrument-narrative-editor`(+test), `instrument-sync-dialog`, `instruments-hub-column-layout`(+test), `instruments-hub-detail-panel`, `instruments-hub-enrichment`(+test), `instruments-hub-filter-bar`, `instruments-hub-model`(+test), `instruments-hub-scores`(+test), `instruments-hub-split-layout`, `instruments-hub-trackers`(+test), `instruments-page`, `use-activate-instrument-tracking`, `use-ensure-instrument-fundamentals`, `use-instrument-data-freshness`, `use-instrument-live-quotes-batch`, `use-instrument-sync`(+test ya formateado), `use-instruments-hub-enrichment`, `use-instruments-hub-scores`, `use-instruments-hub-trackers`), diff +2128/-1399 solo formato. Sin falsos positivos EOL (los 30 con diff real; `use-instrument-sync.test.ts` quedo `(unchanged)`; solo `'→"` + line-wrapping). No es area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
| 25 | `110667b` | `apps/web/src/stores` = **31 files** (`active-account-store`, `alerts-store`(+test), `auth-store`, `chart-cursor-store`, `core-r-review-queue-store`(+test), `dia-d-evidence-archive-store`(+test), `dia-d-trading-session-store`(+test), `estudio-membership-store`, `estudio-process-running-store`, `instruments-hub-preferences-store`, `list-auto-activity-store`, `list-chrome-layout-store`, `list-tracker-results-store`, `notification-prefs-store`, `screener-activity-store`, `screener-preferences-store`, `supervised-f3-queue-store`(+test), `tracker-alarm-inbox-store`(+test), `trade-preferences-store`, `trading-layout-store`, `trading-ui-store`, `ui-store`, `visualization-store`, `workspace-store`, `workspace-ui-bridge`), diff +1911/-1207 solo formato. Sin falsos positivos EOL (los 31 con diff real; solo `'→"` + line-wrapping). No es area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
| 26 | `465137a` | `apps/web/src/lib` sub-batch A = **18 files** (`api-base-url`, `api`(+808/-574), `chart-list-membership`(+test), `chart-list-snapshot`, `chart-tab-uniqueness`(+test), `close-chart-on-list-removal`, `datetime-input`, `default-lists`, `draw-tool-favorites-storage`, `draw-tool-session-storage`, `instrument-search.test`, `list-carousel-config`, `list-column-layout`, `list-hub-column-layout`, `list-selection-guard`, `list-sort-with-recommendation.test`), diff +1299/-897 solo formato. Sin falsos positivos EOL (los 18 con diff real; solo `'→"` + line-wrapping). No es area Coach/TOP. Quedan 17 files de `lib` (sub-batch B) | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
| 27 | `0b8f04b` | `apps/web/src/lib` sub-batch B (**CIERRE** del dominio lib, 17 de 35) = **17 files** (`list-sort-with-recommendation`, `list-sync`(+test), `list-utils`, `query-client`, `query-invalidation`, `routes`, `scan-results-column-layout`(+test), `screener-split-layout`(+test), `scroll-list-instrument-into-view`(+test), `search-ranking`, `use-media-query`, `utils`, `workspace-payload`), diff +518/-388 solo formato. Sin falsos positivos EOL (los 17 con diff real; solo `'→"` + line-wrapping). No es area Coach/TOP. **Dominio lib CERRADO (lotes 26-27, 35 file)** | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
| 28 | `882082c` | `features/screeners` (**CIERRE** del dominio screeners) = **27 files** (dominio completo; todos de produccion excepto `tracker-alarms.test.ts`): diff +2431/-1621 solo formato. Sin falsos positivos EOL (los 27 con diff real; los 27 del dominio en el glob). No es area Coach/TOP. **Dominio screeners CERRADO (lote 28, 27 files)** | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
| 29 | `998ed1d` | `features/charts` sub-batch A = **29 files** (`ai-indicators-panel` ... `chart-draw-tool-utils.ts`), diff +2784/-1540 solo formato. Sin falsos positivos EOL (los 29 con diff real). No es area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
| 30 | `87e47e5` | `features/charts` sub-batch B = **29 files** (`chart-finalist-top1-switch` ... `chart-time-sync.ts`), diff +1148/-759 solo formato. Sin falsos positivos EOL (los 29 con diff real). Area potencial Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) · **test:coach 26/186 ok** (API smoke CORE-P = SKIP optional live) |
| 31 | `ce981e7` | `features/charts` sub-batch C (**CIERRE** del dominio charts) = **29 files** (`chart-toolbar-chart-bar` ... `use-inspector-bar-shortcut-favorites`), diff +2178/-1134 solo formato. Sin falsos positivos EOL (los 29 con diff real). **Dominio charts CERRADO (lotes 29-31, 87 files)**. Area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) · **test:coach 26/186 ok** (API smoke CORE-P = SKIP optional live) |
| 32 | `7367447` | `features/trading` sub-batch A = **28 files** con diff real (`charts-zone` ... `estudio-process-status.test.ts`); 1 falso +EOL excluido (`dia-d-session-report-panel.tsx`). No es area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
| 33 | `3b92e76` | `features/trading` sub-batch B = **29 files** con diff real (`estudio-process-status.ts` ... `lists-tab/list-recommendation-columns.test.ts`). Area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) · **test:coach 26/186 ok** (API smoke CORE-P = SKIP optional live) |
| 34 | `cafba4c` | `features/trading` sub-batch C = **28 files** con diff real (`lists-tab/list-recommendation-scores-context` ... `propose-instrument-supervised.test.ts`); 1 falso +EOL excluido (`lists-tab/list-selection.ts`). Area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) · **test:coach 26/186 ok** (API smoke CORE-P = SKIP optional live) |
| 35 | `49c7fac` | `features/trading` sub-batch D (**CIERRE** del dominio trading) = **27 files** con diff real (`propose-instrument-supervised.ts` ... `use-trade-notional.ts`, incl. `trading-coach-rail`). **Dominio trading CERRADO (lotes 32-35, 112 files con diff real + 2 falsos EOL)**. Area Coach/TOP | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) · **test:coach 26/186 ok** (API smoke CORE-P = SKIP optional live) |
| 36 | `5b47f60` | auxiliares + root app/main = **19 files** con diff real (root `app`+`main`, `ai`×2, `alerts`×4, `auth`×2, `dashboard`, `fiscal`, `help`×4, `history`, `operations`, `sync`); 1 falso +EOL excluido (`vite-env.d.ts`). No es area Coach/TOP backtests, test:coach 26/186 ok de forma conservadora | typecheck ok · lint 0e ok · test 140/707 ok · build ok (chunk = M7) |
