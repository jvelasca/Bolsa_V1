...
Siguiente lote: otro sub-lote por dominio de `features/backtests` (p. ej. `list-auto`/`mass-compare`, `coach`/`lab`, `finalists`/`top`, `strategy-matrix` restantes, `hub`/`chart`/`misc`), después el resto de `apps/web/src` por sub-lotes. Verificación tras cada lote y commit propio. HEAD `9853e79` ·
sincronizado.
