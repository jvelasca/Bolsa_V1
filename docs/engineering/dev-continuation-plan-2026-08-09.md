...
Siguiente lote: **resto de `apps/web/src`** (fuera de `features/backtests`, que queda COMPLETO con los lotes 15-18: `6c32b06`/`a42587e`/`da2f7d9`/`5bec6ed`, y el dominio `hub`/`chart`/`misc` CERRADO). **Lote 19 (`689c294`): `features/accounts`. Lote 20 (`1081809`): `features/workspace`. Lote 21 (`75f6595`): `features/config`+`platform`. Lote 22 (`cbd0fff`): `features/research`, 10 files, +749/-452.** Continuar por dominios (charts, instruments, trading, settings, screeners, lib, stores, components/ui, layout, etc.), con el mismo protocolo de 8 pasos y verificando el `git diff --cached --numstat`. `prettier --check` amplio (excl. `features/backtests`) ahora: **371 files** desincronizados pendientes. Verificación tras cada lote y commit propio. HEAD `cbd0fff` ·
sincronizado.
