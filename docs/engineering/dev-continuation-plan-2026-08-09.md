...
Siguiente lote: otro sub-lote por dominio de `features/backtests` (p. ej. `hub`/`chart`/`misc`, subdividir ~44 files; `coach`/`lab` ya hecho lote 14 `6f8c668`), después el resto de `apps/web/src` por sub-lotes. Verificación tras cada lote y commit propio. HEAD `6f8c668` ·
sincronizado.
