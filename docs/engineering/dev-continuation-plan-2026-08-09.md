...
Siguiente lote: **resto de `apps/web/src`** (fuera de `features/backtests`, que queda COMPLETO con los lotes 15-18: `6c32b06`/`a42587e`/`da2f7d9`/`5bec6ed`, y el dominio `hub`/`chart`/`misc` CERRADO). **Lote 19 hecho (`689c294`): dominio `features/accounts`, 17 files, +973/-656 solo formato.** **Lote 20 hecho (`1081809`): dominio `features/workspace`, 7 files, +107/-78 solo formato.** Continuar por dominios (charts, instruments, trading, settings, screeners, research, platform, config, lib, stores, components/ui, layout, etc.), con el mismo protocolo de 8 pasos y verificando el `git diff --cached --numstat`. `prettier --check` amplio (excl. `features/backtests`) ahora: **395 files** desincronizados pendientes. Verificación tras cada lote y commit propio. HEAD `1081809` ·
sincronizado.
