...
Siguiente lote: otro sub-lote por dominio de `features/backtests` (p. ej. `coach`/`lab`, `finalists`/`top`, `hub`/`chart`/`misc`), después el resto de `apps/web/src` por sub-lotes. Verificación tras cada lote y commit propio. HEAD `9975cd3` ·
sincronizado.
