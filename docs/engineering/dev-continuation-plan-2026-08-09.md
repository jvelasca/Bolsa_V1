...
Siguiente lote: **`backtests/misc`** (segunda parte del dominio `hub`/`chart`/`misc`; `hub` hecho lote 15 `6c32b06`, `chart` hecho lote 16 `a42587e`) = los ~35 files restantes del dominio `chart`/`misc` que `--check` reportó (motores/logica misc + tests sueltos: `paper-checklist`, `paper-gate`(+test), `run-context`(+test), `batch-run`, `export`, `date-format`, `split-layout`, `hud-prefs`, `use-backtest-hud-prefs`, `buy-hold`(+test), `deep-coach`(+test), `oos-evidence`(+test), `pbo`(+test), `period`(+test), `period-returns`, `walk-forward-metrics`(+test), `chart-strategy-bridge.test`, `drawing-replay-parity.test`, `signal-evaluate-parity.test`, `library-nav`(+test), `library-strategy-buckets`(+test), `estudio-list.test`, `estudio-personal-list.test`, `ibex35-operativa-audit`(+test)) — si excede ~30, subdividir en dos sub-lotes. Verificación tras cada lote y commit propio. HEAD `a42587e` ·
sincronizado.
