...
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Tag de release R-11** (p.ej. `v1.3.0`)                                                                                                                                                        | cierre R-11              | **PENDIENTE decisión del propietario.** R-11 se cerró **sin tag nuevo**; `v1.2.1` sigue siendo el último tag sobre `2093296`. Si se quiere tag de release: añadir entrada `CHANGELOG` con versión, tag anotado sobre el commit de cierre (p.ej. `870fb21`) y push de rama+tag. NO auto-taggear sin aprobación. |
...
| **Test infra `test_execute_trade_con_fees_reconcilia` roto** (`ExecuteTrade.execute(...)` pide `idempotency_key` tras F1/C2)                                                                    | R-10 F1                  | Deuda conocida ajena a R-11. Corregible actualizando el test para pasar la key. **NO reintroducido** por R-11; ajeno al gate.                                                                                                                                                                                  |
...
> **R-11 NO taggeada** (decidir `v1.3.0` si se cierra release). Último tag release: **`v1.2.1`** sobre `2093296`.
