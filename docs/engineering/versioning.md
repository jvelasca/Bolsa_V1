...
| ----------- | --------------------------------------------------------------------- | ----------------------------------------- | --------------------------- |
| **Product** | Nombre de producto / slice de UX o dominio que lee el auditor         | AsOf de `CURRENT_SYSTEM.md`, relevos      | `V1.41.3-beta`              |
| **Git tag** | Tip certificado para auditoría / CI-by-tag                            | `git tag` · GitHub Releases               | `v1.41.3-beta` → `a8101ab7` |
...
1. **El tip certificado es el git tag**, no el `version` de npm. Un auditor cita `v1.41.3-beta` → `a8101ab7`.
