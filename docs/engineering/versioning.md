...
| ----------- | --------------------------------------------------------------------- | ----------------------------------------- | ------------------------- |
| **Product** | Nombre de producto / slice de UX o dominio que lee el auditor         | AsOf de `CURRENT_SYSTEM.md`, relevos      | `V1.46-beta`              |
...
2. **Package congelado a propósito** durante la serie UX V1.36–V1.46: no bumpir `package.json` en cada parche. El desfase `1.35.0-beta` vs producto `V1.46-beta` **no es un bug de runtime**.
3. **Bump de package** solo al cerrar una **versión estable**. V1.46 es Paper Desk foundation (ciclo de sesión), no estable.
