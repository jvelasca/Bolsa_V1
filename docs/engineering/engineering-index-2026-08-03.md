...
│   ├── traspaso-higiene-formato-legacy-salida-2026-08-11.md ← M0/§6.2 higiene Prettier legacy: SALIDA/RELEVO (punto de entrada del siguiente hilo) · protocolo 8 pasos + EOL check · avance 1-27 (379 files) HEAD `0b8f04b` · `features/backtests` COMPLETO (lotes 2-18) · **`accounts` 19 · `workspace` 20 · `config`+`platform` 21 · `research` 22 · `settings` 23 · `instruments` 24 · `stores` 25 · `lib` sub-batch A 26 · `lib` sub-batch B 27 (CIERRE `lib`) HECHOS (resto `apps/web/src`)** · siguiente: dominios charts/trading/screeners/components · ver §7.6.i
