...
│   ├── traspaso-higiene-formato-legacy-salida-2026-08-11.md ← M0/§6.2 higiene Prettier legacy: SALIDA/RELEVO (punto de entrada del siguiente hilo) · protocolo 8 pasos + EOL check · avance 1-28 (406 files) HEAD `882082c` · `features/backtests` COMPLETO (lotes 2-18) · **`accounts` 19 · `workspace` 20 · `config`+`platform` 21 · `research` 22 · `settings` 23 · `instruments` 24 · `stores` 25 · `lib` sub-batch A 26 · `lib` sub-batch B 27 (CIERRE `lib`) · `screeners` 28 (CIERRE `screeners`) HECHOS (resto `apps/web/src`)** · siguiente: `charts` (3 sub-lotes) → `trading` (4 sub-lotes) → auxiliares · ver §7.6.i
