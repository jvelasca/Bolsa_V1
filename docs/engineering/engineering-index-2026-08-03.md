...
│   ├── traspaso-higiene-formato-legacy-salida-2026-08-11.md ← M0/§6.2 higiene Prettier legacy: SALIDA/RELEVO (punto de entrada del siguiente hilo) · protocolo 8 pasos + EOL check · avance 1-21 (248 files) HEAD `75f6595` · `coach`/`lab` HECHO lote 14 · `hub` HECHO lote 15 · `chart` HECHO lote 16 · `misc`/motores HECHO lote 17 · cierre misc HECHO lote 18 · **`features/backtests` COMPLETO** · **`accounts` HECHO 19 · `workspace` HECHO 20 · `config`+`platform` HECHO 21 (primeros sub-lotes del resto `apps/web/src`)** · siguiente: otro dominio del resto `apps/web/src` · ver §7.6.i
