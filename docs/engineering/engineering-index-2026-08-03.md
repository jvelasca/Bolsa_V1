...
16. **Auditoría consolidada interna+externas + plan hardening (2026-08-11):** [audit-consolidado-internas-externas-2026-08-11.md](./audit-consolidado-internas-externas-2026-08-11.md) — 4 fuentes cruzadas (interna · externa 1 · externa 2 · externa 3) · mapa P0/P1/P2 · plan F1–F5 · decisiones D0–D5 pactadas · checkpoint git `audit-checkpoint-2026-08-11` · NO tocar código
    ...
    >             └── **F5a-fidelidad — traspaso de fase:** [traspaso-f5a-fidelidad-gate-2026-08-12.md](./traspaso-f5a-fidelidad-gate-2026-08-12.md) — **COMPLETADO 2026-08-12** · rama `stage/f5a-fidelidad-gate-2026-08-12` desde base `d892b3f` (tip F5b-drift) · **gate `contract-check.ts` BIDIRECCIONAL** (nuevo tipo `CoversContract<BE,FE>` = `Exclude<keyof BE, keyof FE>`) cubriendo FE⊆contrato Y contrato⊆FE · **sentinelas 5→11** (G1–G11: +`InstrumentDto`/`OhlcvBarDto`/`PortfolioDto`/`PositionDto`/`InvestmentPortfolioDto`/`AccountSummaryDto`) · si el BE emite un campo nuevo, el typecheck rompe (antes no se detectaba) · **cero cambios de DTOs shared** (todos ya con igualdad de claves) · solo `contract-check.ts` modificado · probe de drift contrato→FE verificado positivo (G9 rompe con `driftProbe`) y restaurado · batería: contract:check **VERDE** ✓ reproducible · web typecheck✓ · lint 0 errores · shared typecheck✓ · test **714✓** (141 f) · deuda: `openapi-fetch` completo (fase propia), reconciliar ~87+ DTOs restantes / consolidar tipos web-only → P2.6, fidelidad de tipos del valor (`manifest`/`number`↔`integer`) fuera de claves · ver §8
