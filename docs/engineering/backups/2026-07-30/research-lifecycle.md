...
**Sync:** 2026-07-30 · **Baseline v1.5** ([ADR-017](../adr/017-baseline-v1-5-research-observatory.md)) · Fase 1 + 1.5 · **Fase 2 P2.A–P2.F cerrada** ([ADR-018](../adr/018-fase2-evidence-store-v0.md)) · Batería `pnpm test:fase2` · Track lab UI P3–P9 cerrado · Embudo coach A–D + **Play ciclo** + **Lista AUTO** (frescura v1.1 · keep-alive) · Finalistas → paper (A) / Supervisado (C) · **Monitor Finalistas MVP** · Handoff: [session-handoff-2026-07-30.md](./session-handoff-2026-07-30.md) · ops [list-auto-ops-2026-07-29.md](./list-auto-ops-2026-07-29.md).
...
5. Desde **Finalistas**: **Checklist** = paper manual (A); **Proponer** = Supervisado F3 con FA+perfil (C). Distintos; ninguno es auto.
6. En Ayuda → Backtesting: **Monitor Finalistas** = estado TOP/paper/Proponer (solo lectura). Auto-paper completo (D) congelado.

Detalle: Ayuda (?) → **Backtesting**. Cierre de sesión: [handoff 2026-07-29](./backtesting-funnel-handoff-2026-07-29.md).
...
Siguiente fuera de este embudo: auto-paper D (congelado) exige ranking TA+FA+perfil + ejecución. **Monitor Finalistas MVP** (Ayuda → Backtesting) = tablero de estado solo lectura.
...
| **Frescura v1.1** | Huella local+DB; espera perfil; run-context persistido; omitir tras reinicio si igual |
...
#### CORE-R — Reevaluación continua de estrategia (**CRÍTICO · v0+v1 · 2026-07-30**)
...
| **Pendiente** | Scheduler auto · degradación OOS · paper PnL · narración LLM |
...
| **Filas** | TOP status + evidenceLevel + #1; paper vinculado (`strategyDefinitionId` + `labEvidence`); último Proponer F3 origen Finalistas |
| **Enlaces** | Finalistas · Checklist (`?openAnalysis=1` → Detalle + checklist) · F3 (si hay ítem en cola) |
| **No es** | Auto-paper D, scheduler, deploy, execute, unificación A/B |
