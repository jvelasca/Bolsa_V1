...
| **F-SEG-3**    | CORS mínimo privilegio + `X-Forwarded-For`/TrustedHost en rate-limit                                                                        | 🟠 PENDIENTE                       | Bajo       | 🔵 **activa ahora** |
...
2. **CORS `allow_methods=["*"]`/`allow_headers=["*"]`** → F-SEG-3.
3. **Mojibake `workspace-store-core.ts`** (halzg encontrado por auditoría interna propia) → F-HLTH-1.
...
> `f595761` F-FIN-1, `9a35405` F-FIN-2, `4b7a984` F-SEG-1 y `dcb8a37` F-SEG-2 commited/pusheados, pend. merge). Árbol limpio. CI verde.
>
...
> cuenta), F-FIN-2 (ejercicio fiscal en `GetTaxReport`), F-SEG-1 (fail-closed production + `compare_digest`) y
> F-SEG-2 (rotación de logs + redacción de secretos) **hechas** (ver §3).
>
> PRÓXIMA FASE pactada: **F-SEG-3** — CORS mínimo privilegio +
> `X-Forwarded-For`/TrustedHost en rate-limit. Riesgo Bajo. Después: F-HLTH-1, F-DEBT-1, F-WORKER-1 (ver §3).
>
