...
| **F-SEG-1**    | Fail-closed production + comparaciones constantes (`secrets.compare_digest`) — **auth JWT sigue diferida (D4)**                             | 🟠 PENDIENTE                       | Bajo       | 🔵 **activa ahora** |
...
2. **Fail-closed production ausente** → F-SEG-1 (`config.py`, no valida `ENVIRONMENT=prod`+sin password).
3. **Comparaciones `==` no constantes** → F-SEG-1 (`tokens.py:17`, `auth.py:42`).
4. **Repo público + 155,8 MB de logs sin rotación** → F-SEG-2.
5. **CORS `allow_methods=["*"]`/`allow_headers=["*"]`** → F-SEG-3.
6. **Mojibake `workspace-store-core.ts`** (halzg encontrado por auditoría interna propia) → F-HLTH-1.
...
> `f595761` F-FIN-1 y `9a35405` F-FIN-2 commited/pusheados, pend. merge). Árbol limpio. CI verde.
>
...
> cuenta) y F-FIN-2 (ejercicio fiscal en `GetTaxReport`) **hechas** (ver §3).
>
> PRÓXIMA FASE pactada: **F-SEG-1** — Fail-closed production + comparaciones constantes (`secrets.compare_digest`),
> auth JWT sigue diferida (D4) (`config.py` no valida `ENVIRONMENT=prod`+sin password; `tokens.py:17`/`auth.py:42`
> usan `==`). Riesgo Bajo. Después: F-SEG-2, F-SEG-3, F-HLTH-1, F-DEBT-1, F-WORKER-1 (ver §3).
>
