...
| **F-SEG-2**    | Auditoría historial git (repo público) + rotación de logs + tests negativos de redacción                                                    | 🟠 PENDIENTE                       | Bajo-Medio | 🔵 **activa ahora** |
...
2. **Repo público + 155,8 MB de logs sin rotación** → F-SEG-2.
3. **CORS `allow_methods=["*"]`/`allow_headers=["*"]`** → F-SEG-3.
4. **Mojibake `workspace-store-core.ts`** (halzg encontrado por auditoría interna propia) → F-HLTH-1.
...
> `f595761` F-FIN-1, `9a35405` F-FIN-2 y `4b7a984` F-SEG-1 commited/pusheados, pend. merge). Árbol limpio. CI verde.
>
...
> cuenta), F-FIN-2 (ejercicio fiscal en `GetTaxReport`) y F-SEG-1 (fail-closed production + `compare_digest`)
> **hechas** (ver §3).
>
> PRÓXIMA FASE pactada: **F-SEG-2** — Auditoría historial git (repo público) + rotación de logs (155,8 MB) +
> tests negativos de redacción. Riesgo Bajo-Medio. Después: F-SEG-3, F-HLTH-1, F-DEBT-1, F-WORKER-1 (ver §3).
>
