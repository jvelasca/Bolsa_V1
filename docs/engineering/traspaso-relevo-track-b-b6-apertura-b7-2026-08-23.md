# RELEVO / TRASPASO — Track B split backtests B6 → apertura B7 (2026-08-23)
...
> **Estado al redactar (verificado):** `local main = origin/main = 5c03ff7` (B6) · docs stamp pendiente push · **R-13 CERRADA** · **Track B EN CURSO** · split backtests **B1–B6 HECHAS** · tag **`v1.6.0-beta` → `c3964fc`** intacto.
> **AsOf:** 2026-08-23.
...
- **HEAD/rama:** código B6 = `5c03ff7`. SHA vivo tras push docs = `git rev-parse origin/main`.
- **Commits split B1–B6 (de más nuevo a más antiguo):**
...
- **Herencia previa (intacta):** docs stamp relevo B5 `5b7c4e9` · Research→Radar F4′–F6′ `240c846` · plan B0 `b2ee0f2` · tag `v1.6.0-beta` → `c3964fc`.
- **Track B — split `backtests-page`:** B0 plan ✅ · **B1–B6 código ✅** · **B7–B12 pendientes**.
- **LOC página (aprox. post-B6):** ~4029 (partía ~4697; post-B5 ~4146).
...
| B6   | `apps/web/src/features/backtests/lib/backtest-page-navigation.ts`      | Factory `createBacktestPageNavigation`: `patchSearchParams`/`openLibrary`/`setTab`/`selectRun`/`selectInstrument`/`openInstrumentInValor`; `lastBatteryFingerprintRef` subió al bloque de refs |

### Deuda explícita post-B6 (no auto-cerrar)

- Lista AUTO aún en página — extracción **B7** (`lib/backtest-list-auto-controller.ts`).
- Asistente / play cycle — **B8** (no paralelizar con B7).
- Lab handlers — **B9**.
...
- Smoke manual pendiente: `?runId=` / `?focus=coach` · keep-alive `patchSearchParams` fuera de `/backtests` · Lista AUTO pause/resume.
...
3. **Anti-saturación:** el chat anterior cerró tras B6. **Este chat nuevo** abre B7; B7 es **Muy alto** (Lista AUTO / keep-alive) — **no encadenar B7+B8** (estado compartido) ni B7+B8 en el mismo chat sin relevo.
...
Smoke B7: Lista AUTO pause/resume fuera de `/backtests` (PlatformShell keep-alive) · campaña activa con página oculta · deep-links intactos.
...
| Ítem                       | Estado / regla                                                                        |
| -------------------------- | ------------------------------------------------------------------------------------- |
| **Split backtests B7–B12** | **SIGUIENTE** — plan `plan-split-backtests-page-2026-08-22.md` · 1 fase = 1 subagente |
...
> CONTEXTO (2026-08-23): main = git rev-parse origin/main → 5c03ff7 (código B6; docs stamp si ya pusheado). Track B EN CURSO. Split backtests **B1–B6 ✅** (6271c8c · fcdc857 · bcadea9 · 5475c09 · ca13981 · 5c03ff7). Research→Radar F4′–F6′ ✅ (240c846). LEE: traspaso-relevo-track-b-b6-apertura-b7-2026-08-23.md · backlog §0 · plan-split-backtests-page-2026-08-22.md. Tarea: backtests **B7** (Lista AUTO) — 1 subagente, sin commits sin aprobación. Riesgo **Muy alto**. No paralelizar con B8.

### 4.2 Brief subagentes (patrón)

> Una fase = subagente acotado: read-first backlog §0 + plan split §3 fase Bx + archivos exactos + qué NO tocar + batería esperada + **NO commits ni push** + reporte file:line. Coordinador re-verifica antes de proponer commit. Anti-saturación: **no encadenar B7+B8** en el mismo chat (estado compartido) sin relevo.
...
- Relevo anterior (B5 → B6): `traspaso-relevo-track-b-b5-apertura-b6-2026-08-23.md` (histórico)
...
> **Siguiente:** backtests **B7** (Lista AUTO — `lib/backtest-list-auto-controller.ts`). Riesgo **Muy alto**.
