...
> **AsOf:** `origin/main` = **`e106133`**. Tag **`v1.5.0-beta` → `5e52bd6`**. Gates R12-ACCOUNTS + R12-AUTH **F1–F10 + F8b–F8e** · **F7b script** · **JWT-only**. D4 **Opción C** · ADR-027 **Aceptado**. Pending-delete E8 **N** (sin purge). Ventana purge V2 + kit log **abierta 2026-08-22** (`0763700`).
...
- D4 JWT (Opción C): **F4** `cdda80d` · **F5** `02c86fc` · **F6+F7a** `98b4986` · **F8** `5e7c67b` · **F9** `26494d8` · **F8b** `2cd20b0` · **F8c** `b44cf52` · **F8d** `c697bae` · **F8e** `905acee` · **F10** `837ec85`. **F7b script + JWT-only gate** (working tree, pendiente commit). Stamp docs previo **`e106133`**.
...
| kit monitor V2    | (pendiente commit) | log local `bolsa-legacy-storage-metrics-log` cap 200; E8 N; **sin purge** |
| R12-AUTH F7b      | (pendiente commit) | script offline dry-run default; **sin apply prod**                        |
| JWT-only gate     | (pendiente commit) | SHA-256/HMAC → 401; `tokens.py` eliminado; env `APP_PASSWORD` = flag      |
...
> Firma: `git fetch` + `git rev-parse origin/main` · `git status`. **`origin/main` `e106133`**. Tag **`v1.5.0-beta` → `5e52bd6`** · tag `v1.3.0` → `b778292`. Partida R-12 `f7a86cc`.
