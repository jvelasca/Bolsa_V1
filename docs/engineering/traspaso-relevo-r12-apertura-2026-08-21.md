...
> **AsOf:** `origin/main` = **`e565cf4`**. Tag **`v1.5.0-beta` → `5e52bd6`**. Gates R12-ACCOUNTS + R12-AUTH **F1–F10 + F8b–F8e** cerrados. D4 **Opción C** · ADR-027 **Aceptado**. Pending-delete E8 **N** (sin purge). Ventana purge V2 métricas **abierta 2026-08-22** (`0763700`).
...
- D4 JWT (Opción C): **F4** `cdda80d` · **F5** `02c86fc` · **F6+F7a** `98b4986` · **F8** `5e7c67b` · **F9** `26494d8` · **F8b** `2cd20b0` · **F8c** `b44cf52` · **F8d** `c697bae` · **F8e** `905acee` · **F10** `837ec85`. Stamp docs **`e565cf4`**.
...
`pending-delete` alto (purge) · gobernanza IA · `PAPER_D_EXECUTE` · `contract:gen` (salvo fase pactada) · split `backtests-page` · fusión research/screeners · money motor (`ExecuteTrade`, custodia apply) · retirar fallback `APP_PASSWORD` / `tokens.py` (solo F10 pactado).
...
> Firma: `git fetch` + `git rev-parse origin/main` · `git status`. **`origin/main` `e565cf4`**. Tag **`v1.5.0-beta` → `5e52bd6`** · tag `v1.3.0` → `b778292`. Partida R-12 `f7a86cc`.
