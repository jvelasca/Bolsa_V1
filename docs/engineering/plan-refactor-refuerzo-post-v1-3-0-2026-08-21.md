...
| P-B1      | **Fase 4 — Crash-consistency (chaos): kill entre cash y ledger**     | Verificar atomicidad REAL: `BEGIN → UPDATE cash → KILL` → ROLLBACK, cash intacto, sin ledger.                                                                                        | No hay ninguna prueba de atomicidad real bajo kill. Es la "prueba fundamental" de la auditoría.                | Medio      | ✅ (chaos, ver §3.4)                                                                                                    |
| P-B2      | **Fase 5 — Concurrencia masiva / carga (chaos)**                     | cientos de depósitos/retiros/BUY/SELL simultáneos + verificación Σ=chain=cash tras cada escenario.                                                                                   | Comportamiento bajo estrés no validado (auditoría recomienda como siguiente fase).                             | Medio      | ✅ (chaos, ver §3.5)                                                                                                    |
...
- **R000 (descubierta en Relevo UNO) — tests que no limpian la DB compartida:** `test_m7_custody_single_charge_f3_guard.py` y `test_m2_ledger_cash_reconciliation.py` no llaman a `close_account`/`delete_simulated_account`, por lo que cada ejecución sobre la DB compartida regenera cuentas `simulated` huérfanas que hacen que `verify_ledger_balance_chain.py` (ahora con invariante A) NO dé EXIT 0. **Corregir en esta fase**: hacer que esos tests limpien/aislen sus cuentas para que el EXIT 0 del verify sea estable.
...
- **Relevo DOS (Fases 4–5, chaos):** crash-consistency + concurrencia masiva. Requiere **infraestructura de tests aislada** (docker PG). Máx. 2 subagentes por la complejidad.
- **Relevo TRES (Fase 6, higiene):** docstrings + mypy scope + limpieza E8 + `pending-delete` (solo bajo aprobación).
