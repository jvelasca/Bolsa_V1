# Traspaso M5 — CIERRE PARCIAL del frente `backtest-optimize-panel.tsx` (ola 1 · data-only) · 2026-08-10

**Fecha:** 2026-08-10 · **Rama:** `stage/estudio-membership-operativa-2026-08-04`
**HEAD de referencia:** `889570f` (cierre de la ola 1) · árbol limpio · sincronizado con origin
...
(data-only, bajo riesgo)** está ejecutada y pusheada; quedan los **bloques de alto acoplamiento** pendientes, que
requieren plan específico + aprobación explícita si se retoman.
...
**`backtest-optimize-panel.tsx`: 2.168 → 1.935 líneas (−233).**
...
Los helpers de formateo migraron a los componentes; los imports quedan retirados del orquestador.
...
6. **Botones de adopción + banner de estado del Mejor vs ancla**: `bestVsAnchor`, `handleSave`, `saveStrategyMutation`,
   `savedRowId`.
7. *(adicional)* **banner "Prueba origen"** (`seed` + `onClearSeed` + `optimizeFamilyProxyNote`) — riesgo medio (§1.2
   del traspaso de entrada).
...
- **FASE 1 → FASE 2 → FASE 3.** FASE 1 y la ola 1 de FASE 2/3 ya hechas. **Sin aprobación explícita no se toca código
  ni se commitea.**
...
## 4. Estado de referencia para validar batería (HEAD `889570f`)

- Rama limpia y sincronizada con `origin/stage/estudio-membership-operativa-2026-08-04`.
- Batería verificada en cada paso de la ola 1: typecheck exit 0 · lint 0e/0w · test **140/707** · build exit 0.
- Fichero a modificar (si se continúa): `apps/web/src/features/backtests/backtest-optimize-panel.tsx`. NO es área
  Coach/TOP; la regla `coach-top-quality.mdc` **NO** aplica.
