...
| `INFRA`      | Infrastructure Domain _(conceptual; no ADR propio aún)_ | Docker, PG, workers, cache, broker adapters, logs, telemetry  | Hipótesis, Beliefs, Strategies                   |
...
| **EIG**                             | Expected Information Gain _ex ante_                                                    | Curiosity, Budget                      | \(\mathbb{E}[\mathrm{PnL}]\)  | ADR-013             |
...
| **Meta-Knowledge**                  | Conocimiento sobre _cómo investigar_                                                   | Research Knowledge                     | Market Knowledge              | ADR-011 D14         |
...
| `U_SAMPLE` / `U_CONFLICT` / `U_AMBIGUOUS` / `U_REGIME` / `U_DESIGN` / `U_CAUSAL` | Scientific | _Why uncertain?_ (ADR-013)          |
...
| **Mandato operativo**                     | Playbook vigente en TRADING para un instrumento×cuenta; con **tenure** (desde/hasta)                                                   | Finalistas LAB; tag _setup_ de un trade suelto        |
...
| **En estudio**                            | _(legacy)_ ver **Estudio**                                                                                                             | —                                                     |
| **Estudio**                               | Lista API canónica (`estudio`) = universo supervisable TRADING (membresía explícita) = **universo Daily Ops** (Hoy / Opportunity / IO) | Catálogo IBEX/S&P; ≠ pestañas de gráfico              |
| **Descubrir**                             | Mercado / screeners — señales fuera o dentro de Estudio                                                                                | BUY automático                                        |
| **Supervisar**                            | Añadir a Estudio + Supervisión ON                                                                                                      | Abrir gráfico (→ Visualizados)                        |
| **Operar**                                | Mercado: contexto del valor (gráfico + plan + acción) → Confirm. Hoy solo prioriza/atiende.                                            | Ranking ≠ BUY; Hoy como segundo terminal              |
| **Descubierto**                           | Interesante fuera de Estudio; CTA Analizar / Añadir a Estudio                                                                          | Oportunidad operable diaria                           |
| **Plan operativo**                        | Proyección visual única (entry / stop vigente / T1 / T2)                                                                               | Tres paneles de stop rivales                          |
| **Stop vigente**                          | `PositionState.currentStop` post-fill                                                                                                  | Pending limit · thin protect · plan UI como autoridad |
| **AdminRail**                             | Overview · Cuentas · Fiscal · Consola (admin, colapsable)                                                                              | Nav L1 diaria · menú ⚙                                |
...
| **Coach en vivo**                         | _(legado)_ contenido absorbido por **Panel Operativa**                                                                                 | Reabrir embudo dentro del desk                        |
