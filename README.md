...
**Producto (V2.14.2 Financial Execution Core · cierre A1 con account-isolation en rutas de LECTURA/estudio · tip `main` → `da181b76`/código V2.14.1 · package `1.43.2-beta` · 2026-09-08):** Plataforma PAPER/paper-auto → **Operativa LIVE VIRTUAL** → motor **LIVE Execution Core** (gated). Financial Execution 2026: `live_orders` persistente en PostgreSQL (Alembic `023`) · recuperación de órdenes `UNKNOWN` **sin re-POST** · `ExecutionEvent` idempotente · `OperationalIncident` con dedup OPEN multi-worker · reconciliación continua de posiciones sin auto-heal · `Decimal` al boundary financiero · Decision Spine + Lifecycle Event Store / Outbox + Financial Integrity. Auth JWT multi-user (ADR-027) · stack React 19 + FastAPI Python. **No** LIVE capital · `LIVE_EXECUTION_UNLOCKED` default off · cancel XTB **PARKED** (cero POST bridge). Éxodo LIVE no certificado. Provenance auto-reportada en `GET /api/health`. Estado canónico: [`docs/CURRENT_SYSTEM.md`](./docs/CURRENT_SYSTEM.md).
