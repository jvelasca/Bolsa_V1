...
**Producto (V2.15 hardening · primer ciclo PREVENCIÓN backups `bolsa_v1` + provenance/readiness · tip `main` → `808432aa` (merge PR #59) / package `1.44.0-beta` · 2026-09-08):** Plataforma PAPER/paper-auto → **Operativa LIVE VIRTUAL** → motor **LIVE Execution Core** (gated). Financial Execution 2026: `live_orders` persistente en PostgreSQL (Alembic `023`) · recuperación de órdenes `UNKNOWN` **sin re-POST** · `ExecutionEvent` idempotente · `OperationalIncident` con dedup OPEN multi-worker · reconciliación continua de posiciones sin auto-heal · `Decimal` al boundary financiero · Decision Spine + Lifecycle Event Store / Outbox + Financial Integrity. Auth JWT multi-user (ADR-027) · stack React 19 + FastAPI Python. **Prevención**: `pnpm db:dump/db:restore/db:backup:list/db:backup:cron:win` (copias locales gitignored de `bolsa_v1`). **Readiness**: `GET /api/health/live` + `/api/health/ready` · provenance `PRODUCT_VERSION`/`API_CONTRACT_VERSION` obligatorias en producción. **No** LIVE capital · `LIVE_EXECUTION_UNLOCKED` default off · cancel XTB **PARKED** (cero POST bridge). Éxodo LIVE no certificado. Estado canónico: [`docs/CURRENT_SYSTEM.md`](./docs/CURRENT_SYSTEM.md).
